...
- 教師あり学習\
訓練データとして、ラベル(正解)付きデータを使用する学習方法。入力に対する正しい出力の例を与えることで、入力と出力の関係を学習させる。問題と期待する答えをセットで与える。\
例 : 気温以外のデータから気温の予測、手書きの文字を認識する、迷惑メールを判別する

- 教師なし学習\
訓練データとして、ラベルなしデータを使用する学習方法。クラスタリングなどのためにデータ構造を学習させる。答えは与えず、分類・抽出させる\
ワインの特徴で分類する(時限削減)
...
- 通信速度\
上りは、パソコンや携帯電話からインターネット上へファイルを転送する際の通信。ブログに写真を載せる際の通信や、メールの送信など\
下りは、インターネット上からファイルをパソコンや携帯電話に受け取る際の通信。インターネットからの音楽や画像のダウンロード、メールの受信など
...
- メモリ\
PCが作業をする時に使う机のようなもの。何かを記憶しておくための部品。読み書き可能なメモリ(RAM)と、読めるが書込み不可のメモリに分かれる

- RAM(*Random Access Memory*)\
パソコンのメモリなど、読み書き可能なメモリ。揮発性メモリと不揮発性メモリに分かれる

- 揮発性メモリ\
電気が流れていないと記憶した内容を忘れてしまうメモリ。(電気は関係無く)時間の経過でデータが消える揮発性メモリ(DRAM(*Dynamic Random Access Memory*))、消えない揮発性メモリ(SRAM(*Static Random Access Memoey*))に分かれる。\
DRAMは「読み書きが遅い・消費電力が大きい・容量を大きくしやすい」「安い」、SRAMは「読み書きが速い・消費電力が小さい・容量を大きくしにくい・高い」という特徴がある

- ファイアウォール\
予め設定したルールに従って、通してはいけないデータを止める機能のこと。不正アクセスによるデータの改ざんを防ぐ、セキュリティ対策では必須の機能

- ゼロトラスト\
認証なしには何も信用しないという前提で、適切な認証を受けた端末とそのユーザだけが、許可されたデータやアプリケーションにアクセスできるという概念

- SASEサービス\
*Secure Access Service Edge*の略。IT環境におけるセキュリティ機能とネットワーク機能を1つのクラウドサービスに統合させる、新たなセキュリティフレームワークの考え方
