...
---
10進数の演算式$\frac{7}{32}$の結果を2進数で表したもの
...
```math
\frac{2}{16}+\frac{4}{256}+\frac{8}{(256*16)}\\
=\frac{(64+8+1)}{512}\\
=\frac{73}{512}
```

---
16進小数3A.5Cを10進数の分数で表したもの

- A.**$\frac{3735}{64}$**
```math
(3*16)+(10*1)+(\frac{5}{16})+(\frac{12}{256})\\
=58+\frac{92}{256}\\
=58+\frac{23}{64}\\
=\frac{3735}{64}
```
...
---
調歩同期式(スタートストップ方式)のデータ伝送において偶数パリティによる誤り検出方式を用いて、文字T(JIS7単位符号 1010100)を送出した。正しく受信したときのビット列はどれか。送出はスタートビット(0)、文字の低位ビットから高位ビット、パリティビット、ストップビット(1)の順とし、受信したビットを左から順に記す

- A.**0001010111**\
送信するデータの一定長のビット列に1ビットの検査ビットを付加し、受信側で受信データをパリティビットを照合することで誤りを検出する方式。ビット列とパリティビットを合わせて1のビット数が偶数になるようにパリティビットを付加するのが偶数パリティ。文字Tは1のビットが3個あり、偶数パリティを用いるので付加するパリティビットは1\
送出は、スタートビット(0) -> 文字列Tを右から順に送出 -> パリティビット(1) -> ストップビット(1)のこれら受信したビットを左から順に並べる

---
7ビットの文字コードの先頭に1ビットの偶数パリティビットを付加する時、文字コード30, 3F, 7Aにパリティビットを付加したものはどれか。文字コードは16進数で表している

- A.**30, 3F, FA**\
それぞれを16進数に変換すると、\
30 -> 0011 0000、1の数が偶数なのでパリティビットは付加しない\
3F -> 0011 1111、1の数が偶数なのでパリティビットは付加しない\
7A -> 0111 1010、1の数が奇数なので先頭に1を付加してFA

---
16ビットの2進数nを16進数の各桁に分けて、下位の桁から順にスタックに格納するために、次の手順を4回繰り返す。a, bに入る適切な語句の組み合わせはどれか。 $xxxx_{16}$ は16進数のxxxxを表す\
1.`a`をxに代入する\
2.xをスタックにプッシュする\
3.nを`b`論理シフトする
...
- A.**a : $n AND 000F_{16}$**
- A.**b : 右に4ビット**\
16ビットの2進数がある時、4桁毎にビット列を取り出すことで16進数の各桁に分けられる。下位の桁から順にスタックに格納するので、aには下位4ビットを取り出すための演算が入る。あるビットから特定位置のビットの値を取り出す時には、取得したいビットのみ1とand演算を取る(000Fとand演算)。下位から5-8ビット目に位置するビット列を1-4ビット目に移動したいので、右に4ビット論理シフトする

---
スタック1, 2がある。関数fはスタック1(a, b, c, d)からポップしたデータをスタック2にプッシュする。関数gはスタック2からポップしたデータを出力する。b, c, d, aの順に出力するために実行すべき関数の順番

- A.**f, f, g, f, g, f, g, g**\
f, f -> スタック2にデータをプッシュする(b, a)\
g -> bをプッシュする\
f -> スタック2にcをプッシュする(c, a)\
g -> cをプッシュする\
f -> スタック2にdをプッシュする(d, a)\
g, g -> d, aの順でプッシュする

---
通信回線のパリティチェック方式(垂直パリティ)に関する記述

- A.**1ビットの誤りを検出できる**\
垂直パリティを用いると1ビット(奇数個)の誤りを検出できる。データ通信やメモリチェックにおいてデータのビット誤りを検出方法。一定長のビット列(通常7-8ビット)毎に1ビットの検査ビット(パリティビット)を付加し、検査側が受信データとパリティビットを照合し誤りを検出する。ビット列とパリティビットを合わせて1の個数が奇数(偶数)になるようにパリティビットを付加する奇数(偶数)パリティがある。付加位置は、送信されるデータビット列に付加する垂直パリティと、一定のデータブロック毎に特定位置のビットをまとめたビット列に対して付加する水平パリティ、両者併用の垂直水平パリティがある。基本的に誤り訂正機能はないため、検出時には送信元に再送要求を行う。誤りが奇数個なら検出できるが、偶数個だと検出できない。2方向のパリティを組み合わせた垂直水平パリティ方式だけは、1ビットの誤り訂正、2ビットの誤り検出が可能

<img width="500" alt="" src="./images/パリティチェック.png">

- 1ビットの誤りを訂正でき、2ビットの誤りを検出できる\
垂直水平パリティの説明。垂直パリティ単独では奇数個の誤りの検出しかできない

- 奇数パリティならば1ビットの誤りを検出できるが、偶数パリティは1ビットの誤りも検出できない
- 奇数パリティなら奇数個のビット誤りを、偶数パリティならば偶数個のビット誤りを検出できる\
どちらも奇数個の誤りは検出できるが、偶数個の誤りは検出できない

---
ECC(*Error Check and Correct memory*)メモリで、2ビットの誤りを検出し、1ビットの誤りを訂正するために用いるもの

- A.**ハミング符号**\
情報ビットに検査ビットを付加することで2ビットまで誤りを検出し、1ビットの誤りを訂正できる。RAID2の誤り訂正符号、データ通信などに利用されている。エラーの検出にXOR演算を使う

- 偶数パリティ\
データを構成するビット全体の中でビット"1"の数が偶数になるようにパリティビットを付加する。1ビットの誤りを検出できる

- 垂直パリティ\
水平方向と組み合わせた垂直水平パリティチェックは1ビットの誤り位置を特定し訂正できるが、垂直パリティだけでは1ビットの検出のみが可能

- チェックサム\
データの合計値を検査用に付加し、データに誤りが生じていないか検査する方式

---
送信側では、ビット列をある生成多項式で割った余りをそのビット列に付加して送信し、受信側では、受信したビット列が同じ生成多項式で割り切れるか否かで誤りの発生を判断する検査方式

- A.**CRC(*Cyclic Redundancy Check*)方式**\
送信データから生成多項式によって誤り検出用のデータを付加して送信する。受信側では送信側と同じ生成多項式を用いて受信データを除算し、送信されてきた誤り検出用のデータと比較することで誤りの有無を判断できる。パリティチェックでは検出できない偶数個の誤りやバースト誤りを検出できる特徴がある

- ハミング符号方式\
情報ビットに対して検査ビットを付加することで、2ビットの誤り検出と1ビットの自動訂正機能のある方式

- 垂直パリティチェック方式
- 水平パリティチェック方式\
データに検査用のパリティチェックを付加することで誤りを検出する方式

---
整数x, y( $x > y \geqq 0$ )に対し、次のように定義された関数F(x, y)がある。F(231, 15)の値は幾らか\
F(x, y) = x(y = 0の時)、F(y, x *mod* y)(y > 0の時)

- A.**3**\
y>0なので、関数はF(15, 231 mod 15)\
231を15で割った余りは6なので、F(15, 6)、15を6で割った余りは3
...
---
同じ属性から成る関係R, Sがある。RとSの属性値の一部が一致する場合、関係演算 $R-(R-S)$ と同じ結果が得られるもの

- A.**R $\cap$ S**\
R部分からR部分のみ($R-S$)を取り除くので、RとSの共通部分のみを表す

---
X及びYは0又は1の値をとる変数である。X`?`YをXとYの論理演算とした時、次の真理値表が得られた時の、X`?`Yの真理値表
...
<img width="500" alt="" src="./images/真理値表.png">

- A.**ウ**\
X or (X`?`Y) : 演算結果が1、Xが1なので、X`?`Yは1でなければばらない\
X and (X`?`Y) : 演算結果が1、Xが1なので、X`?`Yは1でなければならない
...
---
1ビットの数A, Bの和を2ビットで表現した時、上位ビットCと下位ビットSを表す論理式の組合せ
...
- A.**$S : (A * \overline{B})+(\overline{A}*B)$**\
排他的論理和と一致する。片方が共通部分以外、それぞれの和は、共通部分以外のベン図部分を表している
...
- A.**$\overline{A} - B$**\
A以外の部分からBを引いたらA, B以外の部分を示す

- $(\overline{A} \cup \overline{B}) - (A \cap B)$\
A, Bの共通部分以外から、A, B共通部分を引いても変化はない

- $(S-A) \cup (S-B)$\
A以外とB以外の全体部分なのでAかつB以外の範囲が該当する
...
- A.**0260**\
11進数55550を10進数に変換する\
-> $11^1 \times 5+11^2 \times 5+11^3 \times 5+11^4 \times 5$\
$=55(1+11+121+1331)$\
$=55*1464=80520$\
下4桁0520に0.5を乗じて**0260**

---
配列A[i](i = 1, 2, …, n)を、次のアルゴリズムで整列する。行2 - 3の処理が初めて終了した時、必ず実現されている配列の状態\
[アルゴリズム]\
1.iを1から(n - 1)まで1ずつ増やしながら2 - 3を繰り返す\
2.jをnから(i + 1)まで減らしながら3を繰り返す\
3.もしA[j] < A[j - 1]なら、A[j]と A[j - 1]を交換する

- A.**A[1]が最小値になる**\
"7315"の文字列(n = 4)の場合、\
1.i = 1 - 3まで繰り返す\
2.jを4から2まで減らしながら繰り返す\
3-1.j = 4の時、5 < 1ではないので交換しない(7315)\
3-2.j = 3の時、1 < 3なので交換する(7135)\
3-3.j = 2の時、1 < 7なので交換する(1735)\
これで行2 - 3の処理の1回目が終了。文字列の中の最小値が先頭に格納される

---
PCM伝送方式によって音声をサンプリング(標本化)して8ビットのデジタルデータに変換し、圧縮処理をしないで転送したところ、転送速度は64,000ビット/秒であった。この時サンプリング間隔

- A.**125**\
1秒毎に64,000ビットのデータが生成されているという意味。1回のサンプリング(標本化)で生成されるデータは8ビットなので、1秒間にサンプリングされている回数は、\
$64,000/8=8,000回$\
1秒間に8,000回のサンプリングを行うためには、以下周期でデータを取得する必要がある\
$1秒/8,000回=125マイクロ秒$

---
音声を標本化周波数10kHz、量子化ビット数16ビットで4秒間サンプリングして音声データを取得した。この音声データを、圧縮率1/4のADPCMを用いて圧縮した場合のデータ量(kバイト)

- A.**20**\
4秒間の標本化周波数(サンプリング回数)は、40,000回。各サンプリングデータを16ビット(2バイト)で量子化するため、量子化後のデータ量はサンプリング回数に2を乗じて80,000バイト。ADPCMで1/4に圧縮するため、圧縮後のデータ量は、\
$80,000/4=20,000(バイト)=20(kバイト)$

---
4Tバイトのデータを格納できるようにRAID1の外部記憶装置を構成する時、フォーマット後の記憶容量が1Tバイトの磁気記憶装置は少なくとも何台必要か

- A.**8**\
RAID1は同じデータを2台の補助記憶装置のそれぞれに記録することで信頼性を高める構成(ミラーリング)。同じデータを2台に書き込むのでディスクの使用効率は1台の時の半分。4Tバイトのデータを記録する場合は、その2倍の8Tバイト分のディスクが必要

---
音声のサンプリングを1秒間に11,000回行い、サンプリングした値をそれぞれ8ビットのデータとして記録する。$512*10^6$バイトの容量をもつフラッシュメモリに記録できる音声は最大何分か

- A.**約775分**\
8ビット = 1バイトなので、1秒間で11kバイト、1分間で660kバイトの容量が必要。フラッシュメモリの容量は$512*10^6$バイトなので、\
($512*10^6$) / ($66*10^4$) = $7.75*10^2$秒 = 約775分

---
格納アドレスが1 - 6の範囲の直接編成ファイルにおいて次の条件でデータを格納した場合、アドレス1に格納されているデータのキー値\
...
- A.**13**\
3 -> 3%5は3なので1を加えて格納アドレスは`4`、4は同様の流れで格納位置は`5`\
8 -> 8%5は3だが、すでにデータがあるので格納位置は`6`\
13 -> 13%5は3、格納位置は本来は4だが、すでにデータが存在するため、最終アドレスの次は先頭に戻って`1`に格納\
14 -> 14%5は4、格納位置は本来は5だが既にデータが存在しているため格納位置は`2`\
18 -> 18%5は3、格納位置は本来は4だが既にデータが存在しているため格納位置は`3`
...
---
2分木の各ノードがもつ記号を出力する再帰的なプログラムProc(ノードn)は、次のように定義される。このプログラムを、図の2分木の根(最上位のノード)に適用した時の出力
...
- A.**abc-d\*+**\
Proc(+) : 左に"a"があるので、Proc(a)を呼び出す\
Proc(a) : 左右にノードがないので`aを出力`し、proc(+)に戻る\
Proc(+) : 右に"*"があるので、Proc(\*)を呼び出す\
Proc(\*) : 左に"-"があるので、Proc(-)を呼び出す\
Proc(-) : 左に"b"があるので、Proc(b)を呼び出す\
Proc(b) : 左右にノードがないので`bを出力`し、proc(-)に戻る\
Proc(-) : 右に"c"があるので、Proc(c)を呼び出す\
Proc(c) : 左右にノードがないので`cを出力`し、proc(-)に戻る\
Proc(-) : `-を出力し`し、proc(\*)に戻る\
Proc(\*) : 右に"d"があるので、Proc(d)を呼び出す\
Proc(d) : 左右にノードがないので`dを出力`し、proc(\*)に戻る\
Proc(\*) : `*を出力`し、Proc(+)に戻る\
Proc(+) : `+を出力`し、処理が終了する
...
<img width="300" alt="" src="./images/ディレクトリ.png">

カレントディレクトリを¥A¥B -> .. -> ..¥B -> .¥Aの順に移動した時、最終的なカレントディレクトリはどこか。ディレクトリの指定方法は下記の通り\
・ディレクトリは"ディレクトリ名¥...¥ディレクトリ名"のように、計路上のディレクトリを¥で区切って指定する\
・¥で始まる時は、左端にルートが指定されているものとする\
・カレントディレクトリは"."で表す\
・1階層上のディレクトリは".."で表す
...
指定のカレントディレクトリの位置から、\
1階層上のディレクトリに移動(¥A)、\
1階層上のディレクトリに移動してから¥Bに移動する、\
カレントディレクトリから¥Aに移動し、最終的なディレクトリは¥B¥A

---
ある工場では表に示す3製品を製造している。実現可能な最大利益は何円か。各製品の月間需要量には上限があり、組み立て工程に使える工場の時間は月間200時間までとする。複数種類の製品を同時に並行して組み立てることはできないものとする

||製品X|製品Y|製品Z|
|-|-|-|-|
|1個当たりの利益(円)|1,800|2,500|3,000|
|1個当たりの組立て所要時間(分)|6|10|15|
|月間需要際上限(個)|1,000|900|500|
...
製品Y : $2,500/10=250$\
製品Z : $3,000/15=200$\
最も利益が大いのは製品Xを優先させて製造させることが、最大利益を上げるために必要な条件。そのため製品Xを1,000個製造する\
利益は $1,800*1,000=1,800,000$\
所要時間は $0.1時間*1,000=100h$\
製造時間はあと100時間あるため、製品Yを製造する\
100時間で製造できる性品Yの数は、\
$100時間/(1/6時間)=600個$\
製品Yの製造で得られる利益は、 $600*2,500=1,500,000$\
利益の合計は、 $1,800,000+1,500,000=3,300,000$

---
A社では、優良顧客について調査することになった。優良顧客は、最近購入実績があり、かつ購入金額の多い顧客とする。優良顧客の選定基準を定めるために、最近の期間について、期間毎に購入金額毎の顧客数を求めて、顧客購入分析表を作成することにした。最も適切な顧客購入分析表はどれか

<img width="500" alt="" src="./images/顧客購入分析表.png">

- A.**ウ**\
分割表により全体の分布が明らかになり、優良顧客の対象グループが左上に集まる。これにより選定基準を決めるのに観やすい図となる

- ア\
購入金額が多い順に並んでいる。優良顧客となるべきグループが離れて配置されるので分析表としては使いにくく不適切

- イ\
購入時期が近いグループが上位に並ぶように配置されており、優良顧客となるべきグループが離れて配置されるので分析表としては使いにくく不適切

- エ\
顧客数の多いグループが上位になっており、購入金額や実績が関係なく配置されている。優良顧客となるべきグループがバラバラに配置されている

---
クラスタ分析の結果を表す図

<img width="500" alt="" src="./images/分析.png">

- A.**ア**\
複数の変数(項目、属性、次元数)を持つデータを利用し、変数間の相互の関係性を捉えるために使われる統計手法。対象となる複数の異なる性質のものが混ざっている中から、類似したものを集めてグループを作り、分離する過程と結果をデンドログラム(樹状図)として表現する

- イ\
ヒストグラムを表す図

- ウ\
1次式を満たす変数の値の中で最大化(最小化)する値を求める線形計画法の結果を表す図

- エ\
特性(結果)と影響を及ぼしたと思われる要因(原因)の関係を体系的に表す特性要因図

---
図は特性要因図(フィッシュボーンダイアグラム)の一部を表したものである。a, bの関係

<img width="300" alt="" src="./images/特性要因図.png">

- A.**bはaの原因である**\
現れた特性(結果)と影響を及ぼしたと思われる要因の関係を体系的に表した図。多くの要因が複雑に絡み合っている時、直接的な原因と間接的な原因を分別したり、真の問題点を明らかにできる。矢印の一番先端に結果をおき、それに対する原因や、さらに重要な原因が生じた原因というように、いくつかある要因の階層関係を視覚的に表すことができる

---
図に示す構成で、表に示すようにキャッシュメモリと主記憶のアクセス時間だけが異なり、他の条件は同じ2種類のCPU XとYがある\
プログラムをCPU XとYでそれぞれ実行したところ、両者の処理時間が等しかった。キャッシュメモリのヒット率は幾らか。CPU処理以外の影響はないものとする

<img width="500" alt="" src="./images/キャッシュメモリ.png">

- A.**0.90**\
CPUはまずキャッシュメモリにアクセスし、キャッシュメモリに目的データが無ければ主記憶にアクセスする。メモリへの平均アクセス時間は、キャッシュメモリのアクセス時間、ヒット率、主記憶のアクセス時間から以下のように求められる\
`キャッシュメモリのアクセス時間 * ヒット率 + 主記憶のアクセス時間 * (1-ヒット率)`\
CPU Xの平均アクセス時間は、`40 * P + 400 * (1-P)`\
CPU Yの平均アクセス時間は、`20 * P + 580 * (1-P)`\
両者の処理時間は等しいので、
```math
...
---
毎分6,000回転、平均位置決め時間20ミリ秒で、1トラック当たりの記憶容量20kバイトの磁気ディスク装置がある。1ブロック4kバイトのデータを1ブロック転送するのに要する平均アクセス時間は何ミリ秒か。磁気ディスクコントローラーのオーバヘッドは無視できるものとし、1kバイト = 1,000バイトとする
...
---
回転速度5,000回転/分、平均シーク時間が20ミリ秒の磁気ディスクがある。1ラック当たりの記憶容量は15,000バイトである。この時、1ブロックが4,000バイトのデータを、1ロック転送するために必要な平均アクセス時間

- A.**29.2ミリ秒**\
平均アクセス時間は、シークタイム(位置決め時間)と回転待ち時間とデータ転送時間の総和\
・シークタイムは`20ミリ秒`で固定\
回転待ち時間(サーチタイム)は、ディスクが1回転するのにかかる時間の半分なので、`60s/5000回転=0.012s`\
・平均回転待ち時間は1回転するのにかかる時間の半分のため、`6ミリ秒`\
・データ転送時間は1トラック(回転)が15.000バイトなので、4,000バイトを読み取るために要する時間は、`12ミリ秒*4,000/15,000=3.2ミリ秒`\
総和は、 $20+6+3.2=29.2(ミリ秒)$

---
回転数が4,200回/分で、平均位置決め時間が5ミリ秒の磁気ディスク装置がある。この磁気ディスク装置の平均待ち時間は約何ミリ秒か。平均待ち時間は、平均位置決め時間と平均回転待ち時間の合計である

- A.**12**\
回転待ち時間は、ディスクが1回転するのにかかる時間の半分なので、`60s/4,200=600ms/(42*2)=7.1428...ms`\
平均位置決め時間を足すと約12ミリ秒

---
データ探索時に使用される、理想的なハッシュ法の説明

- A.**データを特定のアルゴリズムによって変換した値を格納アドレスとして用いる、高速でスケーラビリティの高いデータ検索技術である**\
ハッシュ関数を用いてデータやデータのキー値からデータの格納位置を直接計算する方法。線形探索法や2分探索法と異なり、常に一定の時間(探索回数は1回)で目的のデータを探索できる
...
---
探索方法とその実行時間のオーダーの正しい組合せはどれか。探索するデータ数をnとし、ハッシュ値が衝突する(同じ値になる)確率は無視できるほど小さいものとする。実行時間のオーダーが $n^2$ であるとは、n個のデータを処理する時間が $cn^2$ (cは定数)で抑えられることを言う

- A.**2分探索 : $\log_2n$, 線形探索 : n, ハッシュ探索 : 1**\
アルゴリズムの計算量が実行時に処理するデータ量によってどのように増加するかやアルゴリズムの実行時間の長さを示す。時間研鑽量を表すオーダー記法によってアルゴリズムの複雑さが分かり、理論的比較をできる

---
昇順に整列されたn個のデータが配列に格納されている。探索したい値を2分探索木で探索する時の、おおよその比較回数を求める式

- A.**$\log_2n$**\
線形探索法 : 探索対象のデータ群の先頭から順に値を比較する方法。データ群が整列されていなくても探索できるが、使用頻度が高い順に整列されていると少ない回数で目的のデータを探索することが可能\
平均比較回数 : $(N+1)/2$、最大比較回数 : n\
2分探索法 : 探索データが昇順(降順)に整列されている時に用いる方法。データ中央値と探索対象のデータを比較し、大小により探索範囲を半分に狭めていくことで線形探索と比べて効率的に探索が可能\
平均比較回数 : $[\log_2N]$、最大比較回数 : $[\log_2N]+1$

---
コンピュータで連立一次方程式の解を求めるのに、式に含まれる未知数の個数の3乗に比例する計算時間がかかるとする。あるコンピュータで100元連立一次方程式の解を求めるのに2秒かかったとすると、その4倍の演算速度をもつコンピュータで1,000元連立一次方程式の解を求めるときの計算時間

- A.**500**\
10倍の連立一次方程式を解く\
3乗に比例するので、1,000倍の時間がかかるが、演算速度が4倍なので、\
$1,000 * 2秒 / 演算速度4倍 = 500$

---
図のメモリマップで、セグメント2が開放された時、セグメントを移動(動的再配置)し、分散する空き領域を集めて1つの連続領域にしたい。1回のメモリアクセスは4バイト単位で行い、読取り、書込みが30ナノ秒とすると、動的再配置をするのに必要なメモリアクセス時間は合計何ミリ秒か。1kバイトは1,000バイトとし、動的再配置に要する時間以外のオーバーヘッドは考慮しないものとする

<img width="500" alt="" src="./images/メモリマップ.png">

- A.**12.0**\
セグメント2が解放されると、空き領域がセグメント3の前後に存在する。最少回数で空き領域を1つにまとめるため、セグメント3(800kバイト)を前方に寄せるのに要する時間を考える\
1回のメモリアクセスは4バイト単位なので、
$800,000 / 4 = 200,000(回)$\
読取り -> 書込みの処理を繰り返さないといけないため、総メモリアクセス回数は2倍になる\
$200,000 * 2 * 30 = 12,000,000ナノ秒 = 12ミリ秒$

---
文字列"ET"をASCIIでコード化したものを16進数表記したものはどれか。文字コードの8ビット目には、偶数パリティビットが付く\
[ASCIIコード表の一部]

<img width="400" alt="" src="./images/文字コード.png">

- A.**C5D4**\
Eは100 0101、偶数パリティビットが付くので、`1100 0101` -> C5\
Tは101 0100、偶数パリティビットが付くので、`1101 0100` -> D4
...
- A.**dとx**\
ASCIIコード上のアルファベットは並び順通りに連続した位置に配置されている。ハッシュ表の大きさが10で、ASCIIコードを10進数表記法で表した時の1の位の数を格納位置として使うので、その数字が同じとなるアルファベット同士で衝突が発生する。今回ではアルファベットは順番に並んでいるので、2つの文字の距離が10の倍数になっているものが衝突が起こる組み合わせ

- aとi\
a(1)とi(9)で距離は8なので衝突は起きない

- bとr\
b(2)とr(18)で距離は16なので衝突は起きない

- cとl\
c(3)とl(12)で距離は9なので衝突は起きない

---
横1,600画素、縦1,200画素で、24ビットのカラー情報をもつ画像が撮影できるデジタルカメラがある。このカメラに8Mバイトの記録用カメラを使用すると、何枚の画像が撮影できるか。画像は圧縮しない

- A.**1**\
画像1枚の画素数は、 $1,600*1,200=1,920,000画素$\
1画素は24ビット=3バイトなので、画像1枚のデータ量は、 $1,920,000*3=5,760,000=5.76M$ バイト\
カメラの記憶メモリは8Mバイトなので記録できる画像枚数は1枚のみ

---
1ピクセル当たり24ビットのカラー情報をビデオメモリに記憶する場合、横1,024ピクセル、縦768ピクセルの画面表示に必要なメモリ量は、約何Mバイトか。1Mバイトは $10^6$ バイトとする

- A.**2.4**\
画面内の総ピクセル数は、\
$1024*768=786,432$(ピクセル)\
1ピクセル当たりは3バイト(=24ビット)なので、必要なメモリ量は、\
$786,432*3=2,359,296 \fallingdotseq 2.4M$バイト
...
---
次の一連の3アドレス命令で得られる結果xを表す式はどれか。3アドレス命令では、3つのオペランドを用いた命令`c = a op b`を`op(a, b, c)`として表記する。opは1つの演算子を表し、結果xを表す式においては優先順位の高い順に＊, /, +, -とする\
/(c, d, w1)\
+(b, w1, w2)\
/(e, f, w3)\
-(w3, g, w4)\
...
---
800×600ピクセル、24ビットフルカラーで30フレーム / 秒の動画像の配信に最小限必要な帯域幅はおよそ幾らか。通信時にデータ圧縮は行わないものとする

- A.**350Mビット / 秒**\
1秒当たりのデータ量を算出する。1フレームの画像に含まれるピクセル数は480,000ピクセル。それぞれのピクセルが24ビットの色情報をもつので、1フレーム当たりのデータ量は、\
$480,000ピクセル * 24ビット = 11,520,000ビット = 11.52Mビット$\
1秒の動画像は30フレームで構成されるので、1秒間のデータ量は、\
...
---
96dpiのディスプレイに12ポイントの文字をビットマップで表示したい。正方フォントの縦は何ドットになるか。1ポイントは1/72インチとする

- A.**16**\
12ポイントをインチに変換すると、\
$12(ポイント)*1/72(インチ)=1/6(インチ)$\
12ポイントの文字を描画すると、ディスプレイ上の表示サイズは1/6インチになる。1インチ96ピクセル(ドット)の構成なので、\
$96*1/6=16(ドット)$
...
---
出力待ちの印刷要求を、同一機種の3台のプリンタA - CのうちAから順に空いているプリンタに割り当てるシステム(Cの次はAに戻る)がある。印刷要求の印刷時間が出力待ちの順に5, 12, 4, 3, 10, 4(分)の場合、印刷に要した時間の長い順にプリンタを並べたものはどれか。初期状態はプリンタは全て空いているものとする

- A.**A, B, C**\
Aで5分、Bで12分、Cで4分を処理する(5, 12, 4)\
Cで3分を処理する(5, 12, 7)\
Aで10分を処理する(15, 12, 7)\
Cで4分を処理する(15, 12, 11)

---
3つのタスクの優先度と、各タスクを単独で実行した場合のCPUと入出力装置(I/O)の動作順序と処理時間は、表の通り。優先度方式のタスクスケジューリングを行うOSの下で、3つのタスクが同時に実行可能状態になってから、全てのタスクの実行が終了するまでの、CPUの遊休時間は何ミリ秒か。CPUは1個であり、1CPUは1コアで構成され、I/Oは競合せず、OSのオーバヘッドは考慮しないものとする。表の()内の数字は処理時間を示す
...
- A.**3**\
<img width="500" alt="" src="./images/タスク.png">\
CPUが空き状態になるのは6-8ミリ秒と10-11ミリ秒の間で、全タスクが完了するまでに3ミリ秒の遊休時間が発生する
...
---
次の状態遷移図をもつシステムの状態がS1である時に、信号をt1, t2, t3, t4, t1, t2, t3, t4の順に入力すると最後の状態はどれになるか。空欄は状態が変化しないことを表す
...
---
ある地方の日単位の天気の移り変わりを示したものであり、数値は翌日の天気の変化の確率を表している。ある日の天気が雨の時、2日後の天気が晴れになる確率

<img width="300" alt="" src="./images/天気.png">

- A.**0.33**\
未来の挙動が現在の値だけで決定され、過去の挙動と無関係であるという性質を持つ確率過程をマルコフ過程と呼ぶ。3つの過程が発生する確率を足し合わせる\
晴れ -> 晴れ : 0.12\
曇り -> 晴れ : 0.15\
雨 -> 晴れ : 0.06
...
---
図は70円切符の自動販売機に硬貨が投入されたときの状態遷移を表している。状態 $Q_4$ から状態Eへ遷移する事象はどれか。状態 $Q_0$ は硬貨が投入されていない状態であり、硬貨が1枚投入されるたびに状態は矢印の方向へ遷移するものとする\
状態Eは投入された硬貨の合計が70円以上になった状態であり、自動販売機は切符を発行し、釣銭が必要な場合には釣銭を返す。自動販売機は10円硬貨、50円硬貨、100円硬貨だけを受け付けるようになっている

<img width="400" alt="" src="./images/状態遷移.png">

- A.**50円硬貨または100円硬貨が投入された**\
投入された合計金額が70円以上になった時点でEの状態に遷移し終了する。状態 $Q_0 - Q_6$は現在までの投入額が0 - 60円の状態に対応し、1つずつ状態を遷移するのが10円ずつ投入された場合の状態遷移、5つ離れた状態に遷移するのが50円が投入された場合。状態 $Q_4$ は10円が4回続けて投入され、現在額は40円の状態を示す\
10円 : $Q_5$ に遷移する\
50円 : 投入された合計額が90円となりEに遷移する\
100円 : 投入された合計額が140円となりEに遷移する
...
<img width="400" alt="" src="./images/電流機器.png">

- A.**1,500Wh**\
電力量(W) = 電流(A) * 電圧(V)の関係が成り立つ。単位のWhは電力量の単位で、1Wの電力を1時間使用した場合1Whとなる。力率は、流れる電流が実際に使用された割合を示す数値(今回は100%)。100V電源で動作し、10Aの電流が流れる機器において、電力(W)は、\
$10A * 100V = 1,000W$\
$1,000W * 1.5h = 1,500Wh$
...
---
浮動小数点表示された数値の演算結果における丸め誤差の説明

- A.**数表現のけた数に限度があるので、最下位けたより小さい部分について四捨五入や切上げ、切捨てを行うことによって生じる誤差である**\
数値を有限の桁数で表す際に、それ以降の桁を切り捨て・切り上げすることで生じる誤差。表現できる桁数に限りがあるため、正確な値をそのまま保持できない場合がある。有効桁数以上の本来の値と表現された値に差が生じる

- 演算結果がコンピュータの扱える最大値を超えることによって生じる誤差である\
オーバーフローの説明。演算結果がそのコンピュータが扱える最大値を超えた際に発生する。8ビットの符号なし整数型に256を格納しようとすると、桁あふれを起こした最上位ビットが無視され結果が0になる計算誤差が起こる

- 乗除算において、指数部が小さい方の数値の仮数部の下位部分が失われることによって生じる誤差である\
情報落ちの説明。非常に大きい数と非常に小さい数の計算において、小さい数が無視される現象

- 絶対値がほぼ等しい数値の加減算において、上位の有効数字が失われることによって生じる誤差である\
桁落ちの説明。仮数部の大半が打ち消し合い、計算結果の有効桁数が少なくなることで生じる誤差
...
- A.**1インチ(2.54cm)当たりのドット数と1分間に印刷できるページ数**\
ページ単位で印刷を行うレーザプリンタの性能指標

- 1文字を印字するのに使われる縦横のドット数と1秒間に印字できる文字数\
複写型伝票の印刷などに使われるドットインパクトプリンタの性能指標

- 印字する行の間隔と1秒間に印字できる行数\
1行単位での印刷を行うラインプリンタの性能指標
...
---
デジタルカメラの画像データや携帯音楽プレーヤの音楽データの記憶媒体として利用されているもの

- A.**フラッシュメモリ**\
書換え可能・電源を切ってもデータが消えない半導体メモリ。EEPROMの一種で、ROMに分類されるが、書換え可能なためROMでもRAMでもない存在として別に分類されている場合もある。デジタルカメラや携帯音楽プレーヤの記憶媒体として普及しているのほあ、SDメモリカードやメモリースティックなどで、書き込みが高速で高集積化がしやすいNAND型フラッシュメモリに分類される。小型で大容量・安価なので以前のフロッピーディスクに代わるデバイスとして手軽なデータの持ち運びにも使われている

- DRAM\
主に主記憶に用いられるメモリ(揮発性メモリ)

- SRAM\
主にキャッシュメモリに用いられる(揮発性メモリ)

- マスクROM\
電源を切っても記憶内容を保持する不揮発性のメモリだが、内容の追記・書換えができないので不適切

---
DRAMのリフレッシュ動作の説明

- A.**データを保持するために、一定時間毎にアクセスする**\
データを記憶しているコンデンサで、時間が経つと電荷を放電しデータが消えてしまう。DRAMは消えかけた電荷をコンデンサに充電するリフレッシュ動作を常に一定間隔で行いデータを保持している

- 一定時間毎に内容を外部記憶装置に書き込む\
外部記憶装置に書き込む動作ではない
...
- A.**リフレッシュ動作が必要なメモリであり、PCの主記憶として用いられる**\
コンデンサに電荷を蓄えることにより情報を記憶し、電源供給が無くなると記憶情報も失われる揮発性メモリ。集積度を上げることが比較的簡単なためコンピュータの主記憶装置としてしようされている。コンデンサに蓄えられた電荷は時間経過で失われてしまうため、DRAMは記憶内容を保持するためのリフレッシュ動作を随時行う必要がある

- 1バイト単位でデータの消去及び書込みが可能な不揮発性のメモリであり、電源遮断時もデータ保持が必要な用途に用いられる\
EEPROMに関する記述(バイト単位で書き込み・消去する)。DRAMは揮発性メモリ

- 不揮発性のメモリでNAND型又はNOR型があり、SSDに用いられる\
フラッシュメモリに関する記述。DRAMは揮発性メモリ

- メモリセルはフリップフロップで構成され、キャッシュメモリに用いられる\
SRAMに関する記述

---
プロセッサの省電力技術の1つであるパワーゲーティングの説明

- A.**動作していない回路ブロックへの電源供給を遮断する**\
半導体の静的電力を大幅に削減する低消費電力技術。電源OFFの回路の素子から僅かに電気が漏れてしまう現象(リーク電流)が、電力消費を考える上での課題となっている。非アクティブな回路ブロックを物理的に電源から切り離すことでリーク電流を大幅に削減し、システム全体の消費電力低減を実現する

- 仕事量に応じて、プロセッサへ供給する電源電圧やクロック周波数を変える\
DVFS(Dynamic Voltage Frequency Scaling)の説明
...
- A.**消費電力を抑えながら、プロセッサ全体の処理性能を高められる**\
複数のプロセッサを並列に動作させることで高い処理能力や信頼性を得る形態。単一の主記憶を共有し単一のOSで複数のCPUを制御する密結合型と、CPU毎に独立した主記憶とOSをもつ疎結合型がある

- コアの個数をn倍にすると、プロセッサ全体の処理性能は $n^2$ 倍になる\
コア数をn倍にすると、実際はコアを協調させて動作させるためオーバヘッドが多くなりn倍を下回る

- 複数のコアが同時に動作しても、共有資源の競合は発生しない\
特別な制御を行わず、1つのタスクを複数のプロセッサで処理するケースなどでは競合が発生しやすくなる

- プロセッサのクロック周波数をシングルコアより高められる\
1つのコアのクロック数はシングルコアプロセッサと同様

---
メモリセルにフリップフロップ回路を利用したもの

- A.**SRAM**\
フリップフロップ回路は、2つの安定状態をもつことで1ビットの状態を表現可能な順序回路で、キャッシュメモリを構成するSRAMの記憶セルに使用されている。DRAMと比較してリフレッシュ動作が不要、消費電力が少ないなどの理由で読み書きを高速化しやすい長所がある

- DRAM\
コンデンサに電荷を蓄えることで情報を記憶するメモリ。コンピュータの主記憶装置として使用されている

- EEPROM\
利用者が利用内容を電気的に書込み・消去できる不揮発性メモリ。フラッシュメモリはEEPROMの一種
...
---
プログラム中の図の部分を判定条件網羅(分岐網羅)でテストする時のテストケース

<img width="300" alt="" src="./images/判定条件網羅.png">

- A.**{A, B} = {偽, 偽},  {真, 真}**\
ホワイトボックスレベルにおける網羅性のレベルの分岐網羅は、プログラム中の判定条件で結果が真の場合と偽の場合を少なくとも1度は実行するようにテストケースを設計する手法。偽の場合の処理と真の場合の処理それぞれ進むため、分岐網羅のテストケースとして適切
...
- {A, B} = {偽, 真},  {真, 偽}
- {A, B} = {偽, 真},  {真, 偽},  {真, 真}\
どちらか一方が真であれば真となり、全てが真となる場合の処理しか含まない

---
プログラムの流れ図で示される部分に関するテストデータを、判定条件網羅(分岐網羅)によって設定した。テストデータを複数条件網羅による設定に変更した時、加えるべきテストデータで適切なものはどれか。()部分は1組のテストデータを表すものとする\
判定条件網羅(分岐網羅)によるテストデータ :\
(A=4, B=1), (A=5, B=0)

<img width="300" alt="" src="./images/分岐網羅.png">

- A.**(A=7, B=0), (A=8, B=2)**\
ホワイトボックステストにおける網羅性のレベルである判定条件網羅は、プログラム中の判定条件で結果が真となる場合、偽となる場合を少なくとも1回は実行するテストケースを設計すること\
`命令網羅` : 全ての命令を少なくとも1回は実行するテストケースを設計する\
`分岐網羅(判定条件網羅)` : 判定条件の真偽を少なくとも1回は実行するテストケースを設計する\
`条件網羅` : 判定条件が複数ある場合、それぞれの条件が真偽の場合を組み合わせたテストケースを設計する\
`判定条件・条件網羅` : 判定条件網羅と条件網羅を組み合わせてテストケースを設計する\
`複数条件網羅` : 判定条件の全ての可能な結果の組み合わせを網羅し、全ての命令を少なくとも1回は実行するようテストケースを作成する\
(A=4, B=1), (A=5, B=0)はABともNo、AだけNo。残る2つの組み合わせのどちらもYes、BだけNoを満たすテストデータが必要
...
- A.**A : 排他的論理和, B : 論理積**\
z : 0同士と1同士の場合は0、1と0の組み合わせは1なので排他的論理和(xor)\
c : 1同士の組み合わせの時のみ繰り上がりが発生するので論理積(and)

---
全加算器において、図中のxとzに1、yに0を入力するした時の、出力となるc(桁上げ数)とs(和)の値

<img width="300" alt="" src="./images/全加算器.png">

- A.**c : 1, s : 0**\
2進数の1桁分の加算を下位からの桁上がりも含めて計算できる。x, y, zの3つの入力を合計すると2、2進数だと10となり、桁上げ数は1、和は0が出力される
...
---
データに検査文字(チェックディジット)を付加する。データにエラーが含まれていない場合、 $N_2=7, N_3=6, N_4=2, C=4$ の時、 $N_1$ の値は幾らか\
元のデータ : $N_1N_2N_3N_4$\
検査文字 : C = $mod((N_1+2N_2+3N_3+4N_4), 10)$\
検査文字を付加したデータ : $N_1N_2N_3N_4C$

- A.**4**\
検査文字 $C=mod((N_1+40), 10)=4$\
$N_1$ を10で割った余りが4なので、 $N_1=4$

---
与えられたデータから一定の規則に従って数値を算出し、この数値から検査文字を定めて与えられたデータに付加し、これを用いて入力データの検査を行う。次の規則を用いた場合、4桁の数値データ"2131"に付加する検査文字で正しいもの\
1.与えられたデータの各桁に、先頭から係数4, 3, 2, 1を割り当てる\
2.各桁の数値と割り当てた係数とのそれぞれの積の和を求める\
3.2で求めた和を11で割って余りを求める\
4.3で求めた余りの数字を検査文字とする。余りが10の時は"X"を検査文字とする

- A.**7**\
それぞれの積の和は、 $8+3+6+1=18$\
11で割った余りは、 $18\%11=7$
...
- A.**スーパースピードと呼ばれる5Gビット / 秒のデータ転送モードをもつシリアルインターフェイスである**\
現在最も普及している周辺機器接続のためのシリアルインターフェイスのバージョン3に当たる規格。USB2.0から3.0への進化で最大転送速度が大幅に向上した。2.0ではハイスピードモードの480Mbpsが最大だったが、3.0では5Gbpsのスーパースピードが加わっている。バージョン3.1ではさらに2倍のスーパースピードプラスが登場している

- 1クロックで2ビットの情報を伝送する4対の信号線を使用し、最大1Gビット / 秒のスループットをもつインターフェイスである\
1000 BASE - Tの説明

- PCと周辺機器とを接続するATA仕様をシリアル化したものである\
SATA(*Serial ATA*)の説明

- 音声、映像などに適したアイソクロナス転送を採用しており、ブロードキャスト転送モードをもつシリアルインターフェイスである\
IEEE 1394の説明。USBにもアイソクロナス転送はあるがブロードキャスト転送はない

---
USB(*Universal Serial Bus*)の説明

- A.**ハブを介してツリー状に機器を接続できるシリアルインターフェイスである**\
周辺機器接続のためのインターフェイス。シリアルバス規格の1つで、ハブを使うことで最大127台まで接続可能、ホットプラグ・プラグアンドプレイが可能、バスパワー方式に対応しており小電力のデバイスならコードを介しての電源供給が可能。

- PCに内蔵さっるCD-ROM装置、DVD装置などを接続するためのパラレルインターフェイスである\
パラレルATAの説明

- 磁気ディスク、プリンタなどをデイジーチェーンで接続するパラレルインターフェイスである\
SCSI(*Small Computer System Interface*)の説明

- プリンタなどに赤外線を使ってデータを転送するシリアルインターフェイスである\
IrDA(*Infrared Data Association*)の説明
...
---
XML文書を構成する最小単位である要素の定義方法に関する記述

- A.**データを開始タグと終了タグで囲んで構成するが、データがないこともある**\
タグに囲まれた要素が存在しない空要素がある

- 開始タグと終了タグが対になって構成され、どちらのタグも省略できない\
空要素記述は開始タグと終了タグを合わせたタグ(`<XX/>`)を使用するので、対になっているわけではに

- 1つのXML文書には、階層構造を表すために複数のルート要素を定義できる\
全体が1つの木構造で表現されるので複数のルート要素は記述できない

- 要素の種別を表すために注釈情報を付加して、これを要素名として識別する\
要素名を記述するタグの中に注釈情報(コメント)は記述できない

---
XMLの特徴

- A.**利用者独自のタグを使って、文書の属性情報や論理構造を定義することができる**\
ユーザが定義したタグや属性を用いて文書後続を記述できるマークアップ言語。HTMLは予め定義されたタグを用いてWebページを記述するための言語に対し、XMLはデータ交換をするための汎用のデータ形式で、ユーザ独自のタグで記述が可能

- HTMLを基にして、その機能を拡張したものである\
文書構造をマークアップで定義するSGML(*Standard Generalized Markup Language*)を拡張して策定された

- XML文書を入力するためには専用のエディタが必要である\
XMLはテキスト形式のため、OSに付属している通常のテキストエディタで作成・編集が可能

- 文書の論理構造と表示スタイルを統合したものである\
HTML(*HyperText Markup Language*)の説明

---
タグを使って文書の論理構造や属性を記述する方法を定めた規格であって、電子的な文書の管理や交換を容易に行うための文書記述言語

- A.**SGML**(*Standard Generalized Markup Language*)\
文書を電子的に変換するための汎用マークアップ言語で、文書の中にタグを埋め込んで、書体や文字の大きさ、段落などを記述できる。タグを用いたマークアップ言語のHTMLやXMLはSGMLをベースに開発された

- DML(*Data Manipulation Language*)\
DBに対してデータの検索・新規登録・削除・更新を行うためのデータ操作言語
...
---
あるプログラムAの処理が終了していない時に、別のプログラムから再度呼び出されても正しく動作する時、このプログラムAの性質
...
- 再使用可能(リユーザブル)\
プログラム終了後、再ロードせずに使用しても正しい結果を返すことができるプログラムの性質

- 再配置可能(リロケータブル)\
プログラムをメモリ上のどこに配置しても実行できるようにした性質

---
複数のプログラムから同時に呼び出された時、互いに干渉することなく並行して処理できるプログラムの性質を表すもの

- **リエントラント(再入可能)**\
同時に複数のタスクが共有して実行しても、正しい結果が得られるプログラムの性質

- リカーシブ(再帰)\
実行中に自分自身を呼び出すことを指し、再帰呼び出しを行っても正しい結果を返すことができるプログラムの性質

- リユーザブル(逐次再使用可能)\
1度実行した後、ロードし直さずに再び実行を繰り返しても、正しい結果が得られるプログラムの性質

- リロケータブル(再配置可能)\
主記憶上のどこのアドレスに配置しても、正しく実行できるプログラムの性質

---
ER(*Entity-Relationship Diagram*)図の説明

- A.**対象とする世界を実体と関連の2つの概念で表現する図である**\
DB化の対象となる実体(エンティティ)と実体の持つ属性(アトリビュート)、実体間の関連(リレーションシップ)を表現する図。データをモデリングする方法で、関係DBの設計によく利用される
...
---
ブラックボックステストにおけるテストケースの設計方法

- A.**プログラムの機能仕様やインターフェイスの仕様に基づき、テストケースを設計する**\
システムの内部構造を考慮せず、システムに与える入力を対応する出力だけに着目し、入力に対する出力が仕様書通りになっているか検証する。内部処理を無視する。同値分割、限界値分析、原因-結果グラフ、エラー推測などの手法がある。内部構造や処理に注目して実施するのはホワイトボックステストで、開発者自身が作成したプログラムが仕様書通りに実装されているかを検証する目的で実施される
...
---
オブジェクト指向における、開かれた(ホワイトボックス型)再利用とは、基底クラスに対して、サブクラスを作ることによって、基底クラスのデータや機能を再利用することである。この方式のオブジェクト指向の再利用技術に関する記述

- A.**基底クラスで定義したデータや機能に対する差異をサブクラスに記述すればよく、開発効率が良い**\
差分プログラミングでは、基底クラスと共通した部分の定義は不要で、付加する部分のみの定義だけで良いためプログラミングの効率が向上する

- 基底クラスで定義したデータが保護されるので、安全性の高いプログラムが開発できる\
サブクラスは基底クラスのデータを再定義して仕様を追加できる

- 基底クラスの変更は、サブクラスに影響しない\
全てのサブクラスに影響する

- 基底クラスを複数のアプリケーション開発に利用することはできるが、そのサブクラスを再利用することはできない\
サブクラスを基底クラスとして更にそのサブクラスを作成・再利用できる
...
---
オブジェクト指向言語のクラスに関する記述

- A.**オブジェクトに共通する性質を定義したものがクラスであり、クラスを集めたものがクラスライブラリである**\
クラスライブラリは、関連する機能を提供する複数のクラスを集めてひとまとまりにしたもの

- インスタンス変数には共有データが保存されているので、クラス全体で使用できる\
インスタンス変数は、型であるクラスをもとにして作成したオブジェクトの実体。他のインスタンス変数と共有されるデータはクラスに定義され、インスタンス変数にはオブジェクト毎の個別データが保存される

- オブジェクトはクラスによって定義され、クラスにはメソッドと呼ばれる共有データが保存されている\
クラスで定義され、同一クラスと派生クラスで共有されるデータはクラス変数。クラス変数は、プログラミング言語によっては静的フィールドや静的メンバ変数とも呼ばれる

- スーパークラスはサブクラスから独立して定義し、サブクラスの性質を継承する\
サブクラスはスーパークラスの性質を継承して定義する。スーパークラスが親クラス、サブクラスが子クラスに相当する

---
オブジェクト指向でシステムを開発する場合のカプセル化の効果
...
- A.**操作, 属性, ロール名**\
クラス、属性、クラス間の関係からシステムの構造を記述する静的な構造図。クラス間のリンク、関連、集約、汎化、特化、実現などの関係を表現できる
...
---
シェルのリダイレクト機能による実現の可否に関する記述

- A.**標準入力と標準出力をファイルに切り替えることができ、標準出力をファイルに追加することもできる**\
*shell*はOS機能の一部で、ユーザからの指示を受け解釈し、プログラム起動や制御などを行うプログラム。OSの中核(カーネル)とユーザの橋渡しを行う。リダイレクトは、標準入力(キーボード)・標準出力(ディスプレイ)を変更する機能で、キーボードからの入力の代わりにファイルを入力元にしたり、出力先をファイルにする、または出力をしないなどの変更が可能
...
- A.**相互参照の解決などを行い、複数の目的モジュールなどから1つのロードモジュールを生成する**\
複数個のコンパイル済プログラムや、プログラムで使用するライブラリを連結・結合し、1つの実行可能なプログラムファイルとして出力するソフトウェア
...
- プログラムの実行を監視し、ステップ毎に実行結果を記録する\
デバッガ(*Debugger*)の機能

---
タスクのディスパッチの説明

- A.**実行可能なタスクに対してプロセッサの使用権を割り当てること**\
OSによるタスク管理の制御機能の1つ。実行可能状態のタスクの中から優先度順などによって次に実行すべきタスクを選択して、CPUの使用権を割り当てる。ディスパッチを行うプログラムはディスパッチャ(*Dispatcher*)と呼ぶ

- 各タスクの実行順序を決定すること\
タスクスケジューリングの説明

- タスクの実行に必要な情報であるコンテキストのこと\
タスクの内部情報、置かれた状況、タスクIDや優先度などを保持するTCB(*Task Control Block* : タスク制御ブロック)の説明

- 1つのプロセッサで複数のタスクを同時に実行しているかのように見せかけた状態のこと\
マルチタスクまたはコンカレント処理の説明
...
---
エラー埋込み法による残存エラーの予測において、テストが十分に進んでいると仮定する。この時、当初の埋込みエラー数は48個である。テスト期間中に発見されたエラーの内訳は、埋込みエラー数が36個、真のエラーが42個である。この時、残存する真のエラー数は何個と推定されるか

現在見つかったエラーは埋込み36個、真のエラーが42個。残存している埋込みエラーは12個。真のエラーはその間に14個見つかる見込み\
$36:42=12:x$\
$36x=6*7*6*2$\
$x=14$
...
- A.**プログラムの命令の実行順序、実行結果などの履歴情報を出力する**\
プログラムを命令単位で実行し、その直後のメモリやレジスタの内容などの履歴情報を時系列に出力するツール。プログラムのステップ毎に誤りが無いかどうか検証する時に有効

- 磁気テープファイルや磁気ディスクファイルなどの内容を出力する\
ダンプの説明

- プログラムの実行中にエラーが発生した時、メモリの内容を出力する\
メモリダンプ(静的ダンプ)の説明

- プログラムの特定の命令を実行する毎に、指定されたメモリの内容を出力する\
スナップショット(動的ダンプ)の説明
...
- A.**アプレット(*applet*)**\
Webページと同時にサーバからダウンロードされ、クライアントのWebブラウザ上で動作するコンパイル済みのプログラム。利用者の知らない間にダウンロードされて実行されるため、ユーザ環境のセキュリティを高めるために一般のアプリケーションプログラムにはない機能制限が課せられている

- サーブレット(*servlet*)\
サーバ上でデータ処理やWebページの動的生成などを行うプログラム

- スクリプト(*script*)\
処理手順をテキストで記述する簡易的なプログラム前半を指す言葉。実行時にソースコードを1行毎に解釈しながら実行するインタプリタ方式を採用しているため、コンパイル作業が不要。*JavaScript, VBScript*、サーバ上で動作する*Perl, PHP*が該当する

- スレッド(*thread*)\
プログラムの並列処理を実現するために、タスクをさらに細分化したCPUの処理単位
...
---
主記憶と管理方式のマルチプログラミングでのプログラムの多重度の組み合わせで、スラッシングが発生しやすいもの

- A.**主記憶の管理方式 : 仮想記憶方式, プログラムの多重度 : 大きい**\
仮想記憶を利用するシステムにおいて主記憶が少ない状態で多くのプログラムを実行すると、ページング処理が多発しシステムのオーバヘッドが増加する。その結果アプリケーションのCPU使用率が極端に下がる
...
---
50MIPSのプロセッサの平均命令実行時間

- A.**20ナノ秒**\
1秒間の命令実行回数を100万単位で表すCPU性能の指標。50MIPSは、1秒間に5,000万回の命令を実行できる\
$1s/5*10^7回$\
$=2*10^{-8}秒$\
$=2*10ナノ秒$\
$=20ナノ秒$
...
- A.**主記憶装置と低速の入出力装置との間のデータ転送を、補助記憶装置を介して行うことによって、システム全体の処理能力を高める**\
プリンタなどの低速な入出力装置に対するデータ転送を磁気ディスク装置などを介して実行する機能。入出力装置とCPUでは動作速度の差が大きく、CPUが処理の途中で入出力命令を出すと、入出力動作が終了するまでの間は待ち時間が増加し、スループットが低下する。スプーリングでは、入出力装置とやり取りするデータを外部記憶装置などへ転送し、外部記憶装置と入出力装置の間でデータをやり取りする方法をとる。CPUは低速な入出力装置の動作の完了を待たずに次の処理に移ることができ、スループットを大幅に向上させることができる
...
- A.**最初に格納されたデータが最初に取り出される**\
コンピュータのデータ構造の1つで、先に入力したデータが先に出力される構造。何らかの処理の待ち行列を実現する際によく使われる構造で、プリンタの印刷待ち、CPUの処理待ちなどがキュー構造で処理されている
...
- 2つ以上のポインタを用いてデータの階層関係を表現する\
リスト構造に関する記述

---
スプーリングの説明

- A.**プリンタなどの低速出力装置へのデータを一旦高速な磁気ディスクに格納しておき、その後に装置に出力する**

- キーボードからの入力データを主記憶のキューに一旦保存しておく\
ユーザが行った操作などをアプリケーションプログラムが受け取る仕組みのメッセージループの説明

- システムに投入されたジョブの実行順序を、特性や優先順位に応じて決定する\
タスク管理の説明

- 通信データを直接通信相手の装置に送らず、予め登録しておいた代理の装置に送る\
プロキシの説明

---
タスク管理の役割

- A.**マルチプログラミングの制御を行い、CPUを有効に利用する**\
CPUからみた仕事の単位(プロセス)。そのタスクで実行するプログラムと、タスクの情報を格納しているTCB(*Task Control Block*)で構成されている。タスク管理は、CPU使用時間や主記憶などのコンピュータ上の資源を有効活用しスループットを向上させる目的で、タスク生成から消滅までの管理を適切に行うOS機能の1つ
...
---
スケジューリングに関する記述のうち、ラウンドロビン方式の説明

- A.**各タスクに、均等にCPU時間を割り当てて実行させる方式である**\
OSが各タスクにい一定のCPU時間を順に割り当てるタスクスケジューリング方式。実行可能状態の各タスクは待ち行列で管理され、一定時間内に処理が終わらなかったタスクは、待ち行列の最後尾に移され先頭になるのを待つ。ユーザから見ればどのタスクも同時並列に実行されているように見えるのでレスポンスが向上したように感じるが、タスク切り替えにオーバヘッドが掛かるため、適切なタイムクウォンタムを設定することが重要。各タスクに均等にCPU時間が与えられるため、1つのコンピュータを複数のユーザが同時に利用するタイムシェアリングシステムに適している

- 各タスクに、ターンアラウンドタイムに比例したCPU時間を割り当てて実行させる方式である\
処理時間順方式の説明

- 各タスクの実行イベント発生に応じて、リアルタイムに実行させる方式である\
リアルタイム(イベントドリブンプリエンプション)方式の説明
...
---
コンピュータアニメーションの技法のうち、モーフィング(*Morphing*)の説明

- A.**画像A, Bを対象として、AからBへ滑らかに変化していく様子を表現するために、その中間を補うための画像を複数作成する**\
CGにおいてある物体から別の物体へ変形していく間を、コンピュータ演算によって補完することで、自然な変形をする映像として作成する技法

- 実際の身体の動きをデジタルデータとして収集して、これを基にリアルな動きを持つ画像を複数作成する\
モーションキャプチャの説明

- 背景とは別に、動きがある部分を視点から遠い順に重ねて画像を作成することによって、奥行きが感じられる2次元アニメーションを生成する\
セル画を用いて制作されるセルアニメーションの説明

- 人手によって描かれた線画をスキャナで読み取り、その閉塞域を同一色で彩色処理する\
セル画を使わずにコンピュータ上で動画データを作成するデジタルアニメーションの説明
...
- A.**図形や線などを部品として、始点、方向、長さの要素によって描画していく。また、これらの部品の変形や組合せで効率的に図形を描画していくことができる**\
*Adobe Illustrator*に代表されるベクター形式の画像を作成・編集するためのソフトウェア。線の起終点の座標(位置)、曲線なら曲がり方、太さ、色、囲まれた面の色、変化の仕方などを数値で表すことで、コンピュータで扱うデータとして扱う形式。画像を点の集合で表現しようとするラスターイメージ(ビットマップなど)と比べ、拡大縮小、その他の変形を施しても図形イメージが基本劣化しない特性を持つ。イラストやロゴなどの作成に向くが、写真など細かく構成されているものを扱うには不向き
...
RIA(*Rich Internet Application*)の説明
...
- A.**PCが参照するDNSサーバに誤ったドメイン情報を注入して、偽装されたWebサーバにPCの利用者を誘導する**\
DNSサーバから名前解決要求があった場合に正常な応答に加えて偽の名前解決情報(ドメイン情報)を付加して送信し、サーバのキャッシュに偽の情報を登録させる攻撃手法。汚染されたDNSサーバを利用したユーザが、偽のキャッシュ情報をもとに悪意のあるサイトに誘導され、機密情報を盗まれるなどの被害が発生する可能性がある

- DNSサーバのソフトウェアのバージョンを入手して、DNSサーバのセキュリティホールを特定する\
攻撃のための情報を事前に収集するフットプリンティングの説明。サーバソフトウェアのバージョン情報等の収集には、ポートスキャニングツールが用いられることがある

- 攻撃対象のサービスを妨害するために、攻撃者がDNSサーバを踏み台にして再帰的な問い合わせを大量に行う\
...
---
あるオンラインリアルタイムシステムでは、20件/秒の頻度でトランザクションが発生する。このトランザクションはCPU処理と4回の磁気ディスク入出力処理を経て終了する。磁気ディスク装置の入出力処理時間は40ミリ秒/回であり、CPU処理時間は十分に短いものとする。磁気ディスク装置が均等にアクセスされるとした時、トランザクション処理には最低何台の磁気ディスク装置が必要か

- A.**4**\
トランザクションによる入出力時間は1回あたり40ミリ秒、磁気ディスク1台が1秒の間に入出力処理は`25回`できる\
入出力処理は1秒間でトランザクションが20件、1トランザクションで入出力処理が4回あるので、`80回`の入出力処理が要求される\
80回/25回=3.2となり最低4台の磁気ディスク装置が必要

---
あるシステムのサーバ処理において、次の条件で1年後の処理時間が現在より長くならないようにするためには、CPUの1ステップ当たりの処理時間を少なくとも何%にしなければならないか\
1.データ1件の処理は、CPU命令100万ステップと磁気ディスクへの入出力20回で構成されている\
2.現在は、CPU処理が1ステップ当たり平均1マイクロ秒で、入出力は1回 25ミリ秒で処理されている\
3.1年後は、入出力装置の性能改善によって、データ1件を処理する際の入出力時間が現在の80%に短縮される\
4.1年後に、1日当たりデータの処理件数は現在の150%に増加する\
5.待ち時間及びその他のオーバーヘッドは考慮しないものとする

- A.**60**\
CPU処理は100万マイクロ秒=1000ミリ秒、入出力は500ミリ秒、合計1500ミリ秒\
CPU処理時間をmとして、入出力時間80%、件数150%を加味すると、\
{100万m+(25ミリ秒*0.8)*20}*1.5\
=(100万m+400ミリ秒)*1.5\
=150万m+600ミリ秒\
この2つを方程式にする\
```math
150万m+600ミリ秒 \leqq 1500ミリ秒\\
150万m \leqq 900ミリ秒\\
1万m \leqq 6ミリ秒\\
m \leqq 6/10000ミリ秒\\
m \leqq 0.6マイクロ秒\\
```
1年後のCPU処理時間が0.6マイクロ秒であれば現在よりも処理時間が長くならないため、現在と1マイクロ秒の60%以下にしなくてはならない

---
オンラインリアルタイム処理における1つのトランザクションについて、端末側で応答時間、回線伝送時間、端末処理時間が測定できるとき、サーバ処理時間を求める式として適切なものはどれか。他のオーバヘッドは無視するものとする

- A.**サーバ処理時間 = 応答時間 - 回線伝送時間 - 端末処理時間**\
オンラインリアルタイム処理では、下記手順で処理が行われる\
1.利用者が端末に情報を入力する(端末処理)\
2.入力された情報を端末がサーバにリクエストを送信する\
3.通信回線を経由してリクエストがサーバに届く(回線伝送)\
4.サーバがリクエストに対して処理を行い、結果を端末に返す\
5.通信回線を経由して結果が端末に届く(回線伝送)\
6.端末は結果を表示する(端末処理)\
応答時間はリクエストを送信した時刻`2`からサーバから結果が返ってくる`6`までの時間。回線伝送時間は`3`, `5`、端末処理時間は`1`, `6`、サーバ処理時間は`4`の時間を示す。サーバ処理時間は応答時間(2 - 6)から回線伝送(3, 5)と端末処理(1, 6)に要した時間を引いた時間になる
...
---
出庫記録表に対するSQL文のうち、最も大きな値が得られるもの
...
商品番号が一致するレコードの2行の平均値の2が得られる

- `select max(数量) from 出庫記録`\
レコードの中で数量の最大値の3が得られる

- `select sum(数量) from 出庫記録 where 日付 = "2025-10-11"`\
日付が一致するレコードの2行の値を合計した3が得られる
...
- クライアント側で業務処理を行い、サーバ側ではDB処理に特化できるので、ハードウェア構成の自由度も高く、拡張性に優れている\
...
- A.**プロセス間通信機能を利用し、一連の処理を、サービスを受けるプロセスと、そのサービスを提供するプロセスとに分離したものである**\
サービスを要求する側(クライアント)と、サービスを提供するサーバの機能を分離した垂直分散システムの一形態。クライアントの環境を変更することなくシステムに機能を追加したり、サーバの機能を専用化することで比較的容易に個々の性能を向上させられる利点がある

- パソコンとワークステーションをLANで接続したシステムの構成方式の総称であり、パソコンをクライアント、ワークステーションをサーバと呼ぶ\
サーバとクライアントはシステムにおける役割を表し、区別は関係ない

- ホストコンピュータの一部の機能と処理対象のデータファイルを端末に分散することによって、負荷の分散を図ることを主目的としたものである\
2層C/S、3層C/Sのどちらの構成でも、処理対象のデータファイルはDBサーバ側に配置される

- ホストコンピュータの代わりに配置した複数のワークステーションと端末を接続し、負荷の水平分散を図ることを主目的としたものである\
垂直分散を目的としている

---
クライアントサーバシステム(*client-server system:C/S*)の特徴

- A.**ファイルサーバ、プリントサーバ、などのようにサーバが専用化されている場合、比較的容易に個々のサーバの性能を向上させることができる**\
サービスを要求する側のクライアントと、サービスを提供するサーバにアプリケーションの機能を分離した垂直分散システムの一形態

- クライアントで業務処理の主な部分を実行することによって、業務手続の変更に容易に対応することができる\
業務処理の主な部分はサーバ側で実行される

- 複数のサーバに機能を分散させることによって、データやプログラムが1ヶ所に集まる集中型のシステムよりも容易にセキュリティを高めることができる\
1台のホストコンピュータで複数の業務を処理する集中システムと比較すると、情報資源が分散する分だけ機密保護やセキュリティの確保は難しくなる

- 複数のサーバに業務処理の各ステップを機能分散させている場合には、特定のサーバに負荷が集中した時にもシステム全体への影響を少なくすることができる\
ある特定のステップに処理が集中した場合には、特定のサーバに処理が集中した場合と同様にシステム全体への影響が出る

---
社内業務システムをクラウドサービスへ移行することによって得られるメリット

- A.**PaaS(*Platform as a Service*)を利用すると、プラットフォームの管理やOSのアップデートは、サービスを提供するプロバイダが行うので、導入や運用の負担を軽減することができる**\
サービス提供事業者は開発環境などのミドルウェア以下を提供する。利用者は用意したアプリケーションをミドルウェア上で稼働させる。ミドルウェアにおける設定(カスタマイズ)が可能。\
アプリケーション開発 / 動作のための基盤がサービスの形で提供される。OS及び提供されるプラットフォームの保守はサービス提供者側で行われる

- オンプレミスで運用していた社内固有の機能を有する社内業務システムをSaaS(*Software as a Service*)で提供されるシステムヘ移行する場合、社内固有の機能の移行も容易である\
サービス提供事業者はアプリケーション以下を提供する。利用者は機能を使い、アプリケーションにおける設定可能範囲内でカスタマイズが可能。\
SaaSで提供されるアプリケーションは限られた範囲内でしかカスタマイズできない。独自機能を含むシステムを移行する際にはPaaS以上の利用が望まれる

- 社内業務システムの開発や評価で一時的に使う場合、SaaSを利用することによって自由度の高い開発環境が整えられる\
開発環境は提供されないため、開発環境の提供を望むのであればPaaSの利用が望まれる

- 非常に高い可用性が求められる社内業務システムをIaaS(*Infrastructure as a Service*)に移行する場合、いずれのプロバイダも高可用性を保証しているので移行が容易である\
サービス提供事業者は、OS以下(ハードウェア・ネットワーク)を提供する。利用者はミドルウェア(OS)以上を自前で用意し、OSにおけるカスタマイズが可能。\
サービス提供事業者によって提示している可用性は異なり、SLAの合意は、サービスレベルを保証するものではない。未達時には補償を受けられるが、サービスレベルを下回らないということではない。高可用性が求められるシステムをIaaSに移行するのは容易ではない

---
GPLの下で公開されたOSSを使い、ソースコードを公開しなかった場合にライセンス違反となるもの

- A.**OSSの入手、改変、販売を全て自社で行っている**\
オープンソースであるプログラムはソースコードを含まなければならず、コンパイル済形式と同様にソースコードの頒布(インターネットを通じた無料ダウンロードなどの方法)も許可されていなければならない。改変を行ったOSSを販売・公開する場合は、同時にそのソースコードの頒布もしなくてはならない

- OSSの改変を他社に委託し、自社内で使用している\
改変したのは他社であり、変更したソースコードは他社所有のため公開義務は委託先の他社にある

- OSSを利用して性能テストを行った自社開発ソフトウェアを販売している
- OSSとアプリケーションソフトウェアのインターフェイスを開発し、販売している\
OSSに改変はないので、ソースコードの公開は不要

---
多数のサーバで構成された大規模な分散ファイルシステム機能を提供し、*MadReduce*による大規模データの分散処理を実現するOSS

- A.***Apache Hadoop***\
ペタバイト級の大規模データの蓄積・処理の分散処理を実現するミドルウェア。*Google*が論文として発表した分散処理フレームワーク(*MapReduce*)および分散ファイルシステム(*Google File System*)を基盤技術に、オープンソースとして*Java*で実装したもの
...
---
ヒューマンインターフェイスの機能のうち、定型的な作業を頻繁に行う利用者の操作数を少なくする目的で用いるもの

- A.**一連のコマンドをひとまとめにしたマクロ機能**\
複数の処理と手順を定義し、必要時に呼び出して使える機能。よく使用される定型の処理手順を登録しておくことで、複数の操作を1度で行えるようにできたり、簡単に間違いなく決められた処理を実行できるなどの効果がある。表計算ソフトやDB、エディタやワープロソフトなどに用意され、便利な機能としてよく使用されている
...
- A.**免許不要の2.4GHz帯の電波を利用して通信する**\
電波を使い最大24Mbpsの無線通信を行う規格

- 1台のホストは最大127台のデバイスに接続することができる\
公式の規格では、同時に最大7台の対応デバイスと接続できる

- 規格では、1,000m以上離れた場所でも通信可能であると定められている\
有効通信範囲は最大でも100m程度

- 通信方向に指向性があるので、接続対象の機器同士を向かい合わせて通信を行う\
通信の指向性はなく、機器のレイアウトや向きを自由に変更できる
...
---
図に示すテスト工程品質管理の解釈と今後の対応

<img width="400" alt="" src="./images/テスト工程管理図.png">

- A.**作り込み品質が悪いと判断できるので、誤り多発箇所の重点対策が必要である。前工程の品質状況を見直し、必要であれば前工程をやり直す**\
・未消化テスト項目数が予定より減っていない -> テストがスケジュール通りに進んでいない\
・累積誤り検出数が予定より多い -> テスト開始初期段階から問題が多発している\
テスト対象の品質が悪いためテストが予定通り進んでいない状態と判断できる。品質が悪ければ今後も同じ状態が続き、プロジェクト完了に大きな支障をきたす可能性が高いため、一旦前工程に戻り品質を作り込む対策も必要となる

- テスト項目の消化が遅く、今のうちにテスト環境の不備や開発要因の不足について、対策を検討しておくべきである。品質については、予測した以上に誤りを検出しており、問題はない\
テストの消化数と比較して誤り数が多いので品質が悪いと考えられる
...
誤り数の修正に時間がかかりテストの進行が遅れているので、テスト工程の効率は悪くなっている
...
---
ソフトウェアのテスト工程において、バグ管理図を用いて、テストの進捗状況とソフトウェアの品質を判断したい。この時の考え方

- A.**テスト項目消化の累積件数、バグ検出の累積件数及び未解決バグの件数の推移が全て横ばいばった場合は、解決困難なバグに直面しているかどうかを確認する必要がある**\
縦軸に検出バグ数、横軸にテストに要した時間(テスト消化件数)をとり、一般的な信頼度曲線と比較することで、プログラムの品質を判断するために用いられるグラフ

- テスト工程の前半で予想以上にバグが検出され、スケジュールが遅れたので、スケジュールの見直しを行い、数日遅れでテスト終了の判断をした\
予想以上のバグの検出は、ソフトウェアの品質の悪さを示す。スケジュールを短縮してしまったことで、本来行われる予定であったテストが省略されることになりソフトウェア内にかなりの数の残存バグが残ることになる

- テスト項目がスケジュール通りに消化されていれば、バグ検出の累積件数が増加していなくても、ソフトウェアの品質は高いと判断できる\
バグが少ない原因に、テストの品質の悪さも考えられる。その場合は品質が悪くてもバグが検出されないため安易な判断は危険

- バグ検出の累積件数の推移とテスト項目の未消化件数の推移から、テスト終了の時期をほぼ正確に予測できる\
テスト終盤で解決困難なバグに直面することもあり、終了の時期を段的できるわけではない
...
---
PCで電子メールの送受信を行う際に、電子メールの送信とメールサーバからの電子メールの受信に使用するプロトコルの組み合わせ

- A.**送信プロトコル : SMTP, 受信プロトコル : IMAP4**\
`SMTP` : クライアントからサーバにメールを送信したり、サーバ間でメールを転送するのに用いられる\
`POP3` : 利用者のコンピュータ上で動作するメールソフトが、メールサーバから自身のメールを取り出す処理において使用するメール受信用プロトコル\
`IMAP` : 電子メールの管理をクライアントPC上のメールソフトではなくメールサーバ上で行うことで、複数の端末から利用する場合のメール状態の一元管理やメールの選択受信などの機能を実現したメール受信用プロトコル
...
- A.**電子メールの本文の内容の漏洩の防止**\
本文は共通鍵により暗号化され、復号に使用する共通鍵も公開鍵で暗号化されている。本文の複合化には共通鍵が必要、共通鍵を入手するには受信者の秘密鍵が必要なため、盗聴されても通信内容が漏洩することはない\
...
通信データが改ざんされていないかを受信側で検査するために、通信データから生成する固定長のコード(メッセージダイジェスト)。送信側は、通信データを共通鍵暗号方式またはハッシュ関数で変換したコードを送信データに付加して送信し、受信側も同一の方式で受信データを変換する。コードの一致を確認し、一致していれば改ざんがないと判断する\
デジタル署名は公開鍵暗号方式を応用した技術、MACは共通鍵暗号方式やハッシュ関数を用いた仕組み
...
---
AさんがBさんの公開鍵で暗号化した電子メールを、BさんとCさんに送信した結果のうち、適切なものはどれか。Aさん、Bさん、Cさんはそれぞれの公開鍵は3人全員が持ち、秘密鍵は本人だけが持っているものとする

- A.**暗号化された電子メールを、Bさんだけが自身の秘密鍵で復号できる**\
暗号化と復号に異なる鍵を使用する暗号方式。暗号化鍵は誰もが使用できるように公開し(公開鍵)、復号鍵は受信者が厳重に管理する(秘密鍵)。暗号化鍵と復号鍵は対のペアで生成され、正しく復号できるのは先頭な受信者のみ
...
- A.**有機EL**\
視野角やコントラスト、応答速度の面で優れている

- CRT(*Cathode Ray Tube*)\
ブラウン管を使用したディスプレイ。有機ELと比較して消費電力は多い

- PDP(*Plasma Display Panel*)\
透明な電極を平行に並べ2枚のガラス板の薄い隙間に希ガスを封入し、電極感に電圧をかけることで発生する紫外線で赤青緑の蛍光体を光らせて表示を行うディスプレイ。有機ELと比較して消費電力は多い

- TFT(*Thin Film Transistor*)液晶\
薄膜トランジスタを使用した液晶ディスプレイ。バックライトが必要
...
---
ネットワーク機器の1つであるスイッチングハブ(レイヤ2スイッチ)の機能

- A.**受信したパケットを、宛先MACアドレスが存在するLANポートだけに転送する**\
データリンク層(第2層)で動作し、通過するデータフレームの宛先MACアドレスを見てデータフレームを適切な送信ポートに転送する機能を有する機器。内部にMACアドレスとネットワーク内のノードの対応を記録したMACアドレステーブルを保持しており、これを基にデータフレームの転送を行う。ブリッジはソフトウェアで宛先制御を行うのに対し、スイッチングハブはハードウェア機構を行うためより高速に動作する違いがある

- LANポートに接続された端末に対して、lPアドレスの動的な割当てを行う\
DHCP(*Dynamic Host Configuration Protocol*)が提供する機能
...
ネットワークを流れる電気信号の増幅や整流を行う機器。信号の減衰を防ぎ、ネットワークの物理的な距離を延長するために使用される。物理層(第1層)で動作する

- ゲートウェイは、OSI基本参照モデルにおける第1～3層だけのプロトコルを変換する\
異なるプロトコルやアーキテクチャのネットワークを接続し、データ形式やプロトコルの変換を行う装置。7層全ての層の接続を行う

- ブリッジは、IPアドレスを基にしてフレームを中継する\
異なるネットワークセグメント同士を接続し、データフレームのMACアドレスを基に通信を中継する機器。データリンク層(第2層)で動作する

- ルータは、MACアドレスを基にしてフレームを中継する\
異なるネットワーク同士を接続し、パケットのIPアドレスを基に通信制御を行う機器。ネットワーク層(第3層)で動作する
...
`外部設計` : ユーザからのシステム要件をもとにシステムの機能を確定する作業工程。サブシステムの定義と機能分割、論理データモデル設計、画面・帳票・コードの設計などが実施される\
`内部設計` : 外部設計の要件をコンピュータまたはシステム上で効率よく動作させるかをシステム開発の視点で行われる設計工程。機能のプログラム単位への割り振り、物理データ設計、入出力画面・帳票への出力条件・チェック条件の詳細化、内部処理の詳細設計などが実施される

- 外部設計ではシステムを幾つかのプログラムに分割し、内部設計ではプログラムごとのDFDを作成する\
外部設計はDFDなどを使用してシステムを幾つかのサブシステムに分割し、内部設計ではサブシステムをプログラム単位に分割する

- 外部設計と内部設計の遂行順序は、基本計画におけるユーザの要求に基づいて決定される\
内部設計は、外部設計の成果物を基に行うので、外部設計が先で遂行順序は常に同じ

- 外部設計はコンピュータ側から見たシステム設計であり、内部設計はユーザ側から見たシステム設計である\
ユーザ側から見たシステム設計は外部設計、コンピュータ側から見たシステム設計が内部設計
...
---
スクラムチームにおけるプロダクトオーナーの役割

- A.**生み出されるプロダクトの価値を最大化するために、プロダクトバックログのアイテムを作成し、並び替える**\
プロダクトオーナー : チームに最も価値の高いソフトウェアを開発してもらうために、プロダクトに必要な機能を定義し、プロダクトのバックログの追加・削除・順位付けを行う。開発への投資に対する効果を最大にすることをに責任を持つ

- 完成の定義を忠実に守ることにより品質を作りこみ、利用可能なインクリメントを完成させる
- スプリントの計画を作成する\
開発チーム : 実際に開発を行うチーム(開発者チーム)。ビジネスアナリスト・プログラマ・テスター・アーキテクト・デザイナーなどの機能横断的な様々な技能を持った人がプロダクトを中心に集まり、自律的に行動する。開発チームはバックログに入っている高m句を完了状態にし、プロダクトの評価を高めることに責任を持つ

- チームのリーダーとして、自己管理型で機能横断型のチームのメンバーをコーチする\
スクラムマスタ : スクラムチーム全体が自律的に協働できるように、場作りをするファシリテータ的な役割を担う。コーチとなってメンバ全体の相談に乗ったり、開発チームが抱えている問題を取り除いたりする。スクラム全体を上手く回すことに責任を持つ
...
- 1度の開発で全てを作るのではなく、基本的なシステムアーキテクチャの上に機能の優先度に応じて段階的に開発する\
インクリメンタルモデルの説明
...
---
ソフトウェア開発のプロセスモデルのうち、開発サイクル毎にリスクを最小にしながら、開発サイクルを繰り返すことによって、システムの完成度を高めていくプロセスモデル

- A.**スパイラルモデル**\
情報システムやソフトウェアの開発モデルの1つで、開発の初期段階でシステムを複数の独立性の高いサブシステムに分割し、サブシステム毎に設計からテストまでのサイクルを繰り返しながらシステムを構築する開発モデル

- ウォーターフォールモデル\
開発プロジェクトを時系列に、要件定義・外部設計(概要設計)・内部設計(詳細設計)・開発(プログラミング)・テスト・運用などの作業工程(局面、フェーズ)に分割して行う開発モデル

- 成長モデル\
システムの核となる部分を開発し、引き続きユーザの要望を取り入れながら機能を追加することでシステムを完成させる開発モデル

- プロトタイピングモデル\
システム開発の早い段階で目的とするシステムの試作品を作り、ユーザにイメージを理解させ、要件の確認と承認を得たうえで本格的な開発プロセスに進む開発モデル
...
---
埋込み終了後に割込みによって中断された処理を割り込まれた場所から再開するために、割込み発生時にプロセッサが保存するもの

- A.**プログラムカウンタ**\
`ユーザモードから特権モードへの移行` : 割込みが発生すると自動的に特権モード(スーパバイザモード)に切り替わる\
`プログラムレジスタ(プログラムカウンタ)などの退避` : 割込み終了後に元の命令位置に戻れるように現在の**プログラムカウンタ**の値をスタックに退避させる\
`割込み処理ルーチンの開始番地の決定` : 所定の割込み処理の開始アドレスを取得して、プログラムカウンタにセットする\
`割込み処理ルーチンの実行` : 処理の割込み処理\
プログラムカウンタは、次に実行すべき命令の格納先である主記憶上のアドレスを保持するレジスタ。割込み発生時にひあプログラムカウンタの値をスタックに退避する。割込み処理が終わるとスタックは退避していたアドレスをプログラムカウンタに戻し、中断時の場所から処理を再開する
...
プロセッサの構成要素の1つで、論理演算・加算・減算を行う装置
...
---
パソコンの主記憶の効率的な使用に関する記述

- A.**主記憶領域の使用率を下げるために、デスクトップ上の利用頻度が少ないアイコンを削除したり、不要なウィンドウを閉じたりする**\
不要なウィンドウを閉じたり常駐プログラムを少なくするなどのほか、主記憶領域にデータが展開されているデスクトップアイコンも数が多い場合は必要最低限に減らすことで主記憶領域の使用率を減らすことが可能

- 各種のアプリケーションの処理中に異常終了が何回か発生した時は、デフラグメンテーションを実行する\
スキャンディスク(ハードディスク内の不備の修正)の実行が適切

- 主記憶領域を確保するために、アーカイブ機能で書庫を整理する\
書庫を整理しても補助記憶装置の領域が広くなるだけで主記憶領域には無関係

- データの記録と消去を頻繁に行った結果、処理速度が遅くなった場合は、スキャンディスクを実行する\
デフラグメンテーション(まとまった状態に整理する)の実行が適切

---
主記憶装置の高速化の技法として、主記憶を幾つかのアクセス単位に分割し、各アクセス単位をできるだけ並行動作させることによって実効アクセス時間を短縮する方法

- A.**メモリインタリーブ**\
物理上はひとつである主記憶領域を、同時アクセス可能な複数の論理的な領域(バンク)に分け、それぞれのバンクに対してデータの読み書きを並列で行うことにより、メモリアクセスの高速化を図る技術。奇数アドレスはバンク1、偶数アドレスはバンク2というように、連続したアドレスを複数のバンクに割り振っていく。通常は連続するアドレスに次々とアクセスされることが多いため、見かけ上並列アクセスしているようになり、実効アクセス時間が短くなる。「主記憶に並列アクセス」がキーワード
...
主記憶とキャッシュメモリの同期方法の1つのライトスルー方式の説明

- CPUと主記憶のアクセス速度の違いによるボトルネックを解消するために、高速かつ小容量のメモリを配置する\
キャッシュメモリの説明

- パイプライン処理を乱す要因をなくすために、キャッシュメモリを命令用とデータ用の2つに分離する\
パイプラインキャッシュメモリの更新回路の説明。キャッシュミスヒット時に、データの更新処理と、後続のスレッドからのアクセス処理との競合を回避し、パイプライン動作の乱れを防止する効果がある

---
メモリインタリーブの説明

- A.**主記憶を幾つかの区画に分割し、連続したメモリへのアクセスを高速化する**
...
---
リアルタイムシステムをハードリアルタイムシステムとソフトリアルタイムシステムに分類した時、ハードリアルタイムシステムに該当するもの

- A.**エアバッグ制御システム**\
即時性が求められる処理に対する仕組み。ハードの方は目的の時間内に処理が終了しなければシステムに致命的な影響を与えることになるリアルタイムシステム。処理の遅れにより人命が左右される

- Web配信システム
- 座席予約システム
- バンキングシステム\
処理の時間が多少遅れても致命的な影響を与えるとまでは言えない

---
エンジン制御、ハードディスク制御などの制御系ハードリアルタイムシステムでリアルタイムOSを活用する理由

- A.**定められた時間内にイベントに対応した処理を完了させる機構が必要だから**\
リアルタイムシステムは、使える資源に限りがある状態で、ジョブの実行が命令された時、その処理を決められた時刻(デッドライン)までに終了することに着目した制御工学における概念の1つ(リアルタイム処理)。制御系の組込みシステムなどのリアルタイムシステムのために、時間資源の保護及び実行時間の予測可能性の提供に特化したもの。特にハードリアルタイムシステムにおけるデッドラインミスはシステムに致命的な影響を及ぼすため専用化されたOSが活用される\
`ハードリアルタイムシステム` : システムに課せられた処理がデッドライン内に終了しなかった時、システム全体にとって致命的なダメージが生じる\
`ファームリアルタイムシステム` : デッドラインミスが起こった時、システム全体に致命的なダメージを与えることは無いが、その処理自体の価値は即座に0となる\
`ソフトリアルタイムシステム` : デッドラインミスが起こっても、システム全体に致命的なダメージを与えることはなく、その処理自体の価値も終了時間などにより徐々に落ちていく

---
様々なサイズのメモリ資源を使用するリアルタイムシステムのメモリプール管理において、可変長方式と比べた場合の固定長方式の特徴

- A.**メモリ効率が悪く、獲得及び返却の処理速度は速く一定である**\
`固定長方式` : 主記憶を幾つかの固定長の区画に分割しておき、タスクが実行される時にタスクが必要とする区画数を割り当てる方式。区画のサイズが同じなので、使用されない空き領域が生じる一方、アドレス計算やサイズ計算などのオーバヘッドが減り、読み書き速度は安定化する\
`可変長方式` : タスクが実行される時に、主記憶をそのタスクが必要とする大きさに区切って割り当てる方式。固定長方式と比較してメモリの使用効率はよいが、アドレス計算やサイズ計算などのオーバヘッドは増加する\
リアルタイムシステムは、使えるリソースに限りがある状態で、ジョブの実行が命令された時、決められた時刻(デッドライン)までに終了することに着目した制御工学における概念の1つ。速度が要求されるので、メモリ領域の有効活用よりも読み書き速度を優先した固定長方式のメモリ管理が採用される
...
---
割込み(*Interrupt*)に関する記述

- A.**複数の割込みの発生に備え、個々の割込み原因には優先順位が付けられる**\
すぐに対処しなくてはならない問題などがシステムに生じた時に、実行中のプログラム処理を一旦停止し、別プログラムの実行に切り替えることを指す。割込み処理プログラムが終了し、割込み原因に対する処理が完了すると、割込み直前のデータを復帰し、元のプログラムの実行が再開される

- CPUは割込みを受け付けると実行中のプログラムを中断し、プログラムの再開に必要な情報を磁気ディスクの特定の領域に格納する\
プログラム・カウンタの内容(主プログラムの戻りアドレス)をスタックに退避する

- アプリケーションは、常に割込みの発生を感知する必要がある\
割込み要求が発生すると実行中のプログラムはOSによって一時的に退避されるため、アプリケーション側で割込みの発生を感知する必要はない

- 入出力装置からの動作完了の通知は、内部割込みに分類される\
外部割込みに分類される

---
コンピュータシステムにおけるジョブスケジューリングの特徴

- A.**対話型処理とバッチ処理が混在するシステムでは、対話型処理の優先度を高くすることによって、対話型処理の応答性能の向上が期待できる**

- CPUに割り当てるジョブをOSが強制的に切り替えるタイムスライス方式では、タイマー割込みが多発するので、スループットが低下する\
OSがジョブの切り替えを行うことで、CPUの有休時間を可能な限り減らすことができ、スループットは向上する

- FCFS(*first-come first - served*)方式のジョブスケジューリングは、ジョブ間にCPUを公平に割り当てるので、スループットや応答時間の保証が可能となる\
先頭で到着したジョブから順に割り振られていくので、ジョブの待ち行列に並んでいるジョブ数によって、スループットや応答時間は変化する

- 入出力を多用するジョブよりもCPUを多用するジョブの処理優先度を上げた方が、CPUの待ち時間が少なくなるので、全体のスループットの向上が期待できる\
入出力処理はCPU処理よりも時間がかかる。入出力を多用するジョブの優先度を高くし、入出力処理に空き時間を生じないようにすると全体としてのスループットは向上する

---
多数のクライアントが、LANに接続された1台のプリンターを共同利用するときの印刷要求から印刷完了までの所要時間を、待ち行列理論を適用して見積もる場合について考える。プリンターの運用方法や利用状況に関する記述のうち、M/M/1の待ち行列モデルの条件に反しないもの

- A.**印刷の緊急性や印刷量の多少に関わらず、先着順に印刷する**\
先着順でサービスを受けるのを待っている場合、待ち行列に並ぶ時間を確率論的に求めるモデル。客の到着時間、サービス提供時間、サービスを提供する窓口数をa/b/cの順で表し、Mはランダムを意味する。M/M/1は到着時間とサービス提供時間がランダム、窓口が1つという条件。空いた窓口には直ぐに次の顧客を割り当ててサービスを開始することを条件としているので、窓口の状況に応じてサービス提供を待つことが条件となっている場合は正しい分析をできない

- 一部のクライアントは、プリンタの空き具合を見ながら印刷要求する\
処理状況によって処理要求の到着間隔を操作するのは、到着間隔がランダムという条件に反する

- 印刷待ちの文書データの総量がプリンタのバッファサイズを超える時は、一時的に受付を中断する\
印刷待ちのデータがあっても窓口が受付を中断することはない

- 1つの印刷要求から印刷完了までの所要時間は、印刷の準備に要する一定時間と、印刷量に比例する時間の合計である\
待ち行列に並ぶ時間、窓口で印刷の準備に要する時間、印刷をする時間の合計
...
- A.**ニモニックコード(*Mnemonic Code*)**\
表意コードとも呼び、値から対象のデータが容易に連想できる英数字・記号の組み合わせをコードとして割り当てる方法。他のコードよりも桁数が多くなるが、利用者が記憶しやすい利点があり、商品番号や製品の型番としてよく使用されている

- シーケンスコード\
順番コードとも呼び、連続した値をデータに1つずつ割り当てる方法

- デシマルコード\
10進コードとも呼び、全データを0 - 9の10グループに分割し、更にグループ毎に10個のサブグループに分割することを繰り返す方法。図書コード内の分類コードで使用されている

- ブロックコード\
区分コードとも呼び、部署毎に1000 - 1999、2000 - 2999のようにグループ毎に定められた範囲内でコードを割り当てる方法。上位桁で属しているグループが分かる
...
---
OpenFlowを使ったSDN(*Software-Defined Networking*)の説明

- A.**データ転送と経路制御の機能を論理的に分離し、データ転送に特化したネットワーク機器とソフトウェアによる経路制御の組合せで実現するネットワーク技術**\
`OpenFlow` : 既存のネットワーク機器がもつ制御処理と転送処理を分離したアーキテクチャ。制御部をネットワーク管理者が設計・実装することで、ネットワーク機器ベンダの設定範囲を超えた柔軟な制御機能を実現できる\
`SDN` : OpenFlow上でソフトウェア制御による動的で柔軟なネットワークを作り上げる技術全般を意味する。物理的に接続されたネットワーク上で、別途仮想的なネットワークを構築するなど柔軟な制御が可能になる

- RFIDを用いるIoT(Internet of Things)技術の1つであり、物流ネットワークを最適化するためのソフトウェアアーキテクチャ\
EPCglobalネットワークアーキテクチャの説明

- 音楽や動画、オンラインゲームなどの様々なソフトウェアコンテンツをインターネット経由で効率的に配信するために開発された、ネットワーク上のサーバの最適配置手法\
CDN(*Content Delivery Network*)の説明

- データフロー図やアクティビティ図などを活用して、業務プロセスの問題点を発見し改善を行うための、業務分析と可視化ソフトウェアの技術\
UML(*Unified Modeling Language*)の説明
...
---
パケットフィルタリング型ファイアウォールがルール一覧に基づいてパケットを制御する場合、パケットAに対する制御はどれか。ルール一覧の番号の1から順にルールの適用判断を行い、1つのルールが適用された時には残りのルールは適用しない
...
- A.**番号1によって、通過を禁止する**\
通過するパケットのヘッダ部分に記述されている、送信元(宛先)のIPアドレス(またはポート番号)、通信の方向などの情報をもとに通信を許可/禁止の判断するファイアウォールの制御方式。1から順にルールの適用判断を行い、最初に全ての情報がマッチするのがルール1、通過禁止が適用される

---
WAF(*Web Application Firewall*)を利用する目的

- A.**Webサーバ及びアプリケーションに起因する脆弱性への攻撃を遮断する**\
パケットフィルタ型ファイアウォールは、通過するパケットのIPアドレスとポート番号を見て通貨の可否を決める。XSSやSQLインジェクションなどの攻撃パケットは、正規のHTTP通信に則り、HTTPポート(80 / TCP)宛に送信されるため、パケットのヘッダ情報だけでは通常の通信との判別ができない。WAFではWebアプリケーションの防御に特化したファイアウォールで、パケットのヘッダ部に含まれる、IPアドレスやペイロード部(データ部分)をチェックし、攻撃の兆候の有無を検証する。これによりWebアプリケーションに対する攻撃を検知し、遮断することが可能

---
クライアントとWebサーバの間において、クライアントからWebサーバに送信されたデータを検査して、SQLインジェクションなどの攻撃を遮断するためのもの

- A.**WAF**\
Webアプリケーションの防御に特化したファイアウォール。パケットのヘッダ部に含まれるIPアドレスやポート番号だけでなく、ペイロード部(データ)をチェックすることで、XSSやSQLインジェクションなどのWebアプリケーションに対する攻撃を検知し、遮断が可能

- SSL-VPN機能\
TLS(*Transport Layer Security*)の暗号化技術を用いたVPNの形態。専用ソフトウェアを端末にインストールする必要がなく、Webブラウザを通じて利用でき、ユーザの利便性が高い
...
内部ネットワークからインターネットへのアクセスを代理して中継する装置またはそれを実現するソフトウェア。内部ネットワークの特定端末だけにインターネット接続を許可したり、外部ネットワークからのアクセスを制限するなど、ネットワークの境目で、ネットワーク間のアクセスを一元管理する。Webページアクセスはクライアントではなくプロキリサーバが代理して行うので、ネットワーク内部を秘匿にできセキュリティの確保が期待できる。Webページのアクセス時に内容をキャッシュし、次に同じページにアクセス要求があった場合にはインターネットに問い合わせることなくキャッシュをクライアントに返すキャッシュ機能によって回線負荷を軽減する機能も持っている

- 外部サーバのホスト名とIPアドレスの対応表をもち、クライアントからの問い合わせに対してホスト名に対応するIPアドレスを通知する\
DNSサーバの説明

- クライアントを内部ネットワークに接続する時に、クライアントに対してIPアドレスを動的に割り当てる\
DHCPサーバの説明

- 内部ネットワークで使っているプライベートIPアドレスとグローバルIPアドレスを相互変換し、外部サーバとの直接通信を実現する\
NAT機能の説明
...
---
4ブロック分のキャッシュメモリC0-C3が表に示す状態である。新たに別のブロックの内容をキャッシュメモリにロードする必要が生じた時、C2のブロックを置換の対象とするアルゴリズム(時刻は分:秒)

|キャッシュメモリ|ロード時刻|最終参照時刻(分:秒)|参照回数|
|-|-|-|-|
|C0|0:00|0:08|10|
|C1|0:03|0:06|1|
|C2|0:04|0:05|3|
...
最後に参照された時刻が最も昔のものを置換え対象とする(C2)

- FIFO(*First In First Out*)\
先入れ先出しのこと。最も昔にロードされたものを置換え対象とするアルゴリズム(C0)

- LFU(*Least Frequently Used*)\
最も使用頻度の少ないものを置換え対象とするアルゴリズム(C1)

- LIFO(*Last In First Out*)\
後入れ先出しのこと。最も新しくロードされたものを置換え対象とするアルゴリズム(C3)
...
---
キャッシュメモリと主記憶の間でブロックを置き換える方式にLRU(*Least Recently Used*)方式がある。この方式で置き換えの対象になるブロック

- A.**最後に参照されてから最も長い時間が経過したブロック**\
置き換え対象の中で最も長い時間参照されていないものを置き換え対象とする

- 一定時間参照されていないブロック\
NRU(*Not Recently Used*)方式の基準
...
---
記憶領域の動的な割当て及び解放を繰り返すことで、どこからも利用されない記憶領域が発生することがある。このような記憶領域を再び利用可能にする処理

- A.**ガーベジコレクション**\
記憶領域上の不要な部分を自動で回収し、再利用できるようにする機能。プログラムが動的に確保したメモリ領域のうち、不要になった部分を自動的に開放して、再び使用可能にする機能。従来のプログラム言語では、必要なメモリを確保する命令を記述し、使用後に明示的に開放する必要があったが、ガーベジコレクションが採用されていると、不要と判断された時点で自動的にメモリの開放が行われる

- スタック\
後入れ先出し方式でデータを入出力するデータ記憶構造

- ヒープ\
プログラム上から動的(任意)に確保できるメモリ領域

- フラグメンテーション\
実行されるプログラムに対して記憶領域を区画して割り当てた結果として生じる、主記憶上の断片的な未使用領域
...
---
OSの記憶管理機能に対応する適切な用語の組み合わせ\
a : 予めプログラムを幾つかの単位に分けて補助記憶に格納しておき、プログラムの指定に基づいて主記憶に読み込む\
b : 主記憶とプログラムを固定長の単位に分割し、効率よく記憶管理する。少ない主記憶で大きなプログラムの実行を可能にする\
c : プログラムを一時的に停止させ、世界中の主記憶の内容を補助記憶に退避する。再開時には、退避した内容を主記憶にロードし、元の状態に戻す

- A.**a : オーバレイ**\
データ量が大きく主記憶に格納しきれないプログラムを、幾つかのブロックに分解し、主記憶にはその時点で必要となる部分だけをロードする実記憶の管理方式

- A.**b : ページング**\
主記憶乗に存在するプログラムやデータと、補助記憶装置上にある必要なプログラムやデータの入れ替えを、ページと呼ばれる固定長の単位で行う仮想記憶方式
...
---
アドレス指定方法のうち、命令読出し後のメモリ参照を行わずにデータを取り出すもの

- A.**即値オペランド**\
アドレス部の値が有効なアドレスではなく、そのまま演算対象データとする。オペランド部の値にアドレスではなく演算対象のデータを直接指定するため、他の方式のように演算対象データを取り出すためのメモリ参照を行うことはない
...
<img width="400" alt="" src="./images/アドレス指定方式.png">

- A.**指標付きアドレス指定方式**\
命令が処理対象とするデータの主記憶状の位置を指定する方式。下記のようにアドレス修飾(有効アドレスの計算)を行う\
`指標付きアドレス指定方式` : アドレス部にインデックスレジスタ番号とアドレス値が入って、アドレス値と指定されたインデックスレジスタの値を加算したものを有効とする方式(インデックスアドレス方式)。命令に含まれるアドレス定数100に指定されたレジスタ値の50を加算した150が有効アドレスとなっている

- 相対アドレス指定方式\
命令のアドレス部の値にプログラムカウンタの値を加算したものを有効アドレスとする方式
...
粒度 : 表や行などのロックをかける単位のこと。粒度が小さいほど並行実行が可能になるが、ロックの回数が多くなり制御するためのオーバヘッドが増加する。粒度が大きいほど資源の開放待ち時間が増え、並行実行の多重度が減るためDBの効率が低下する
...
`共有ロック` : データを読込む時に使うロックで、資源がこの状態の場合は他のトランザクションによる更新処理ができなくなる(読込みは可能)。共有ロック時のみ、別のトランザクションが新たに共有ロックをかけることが可能\
`占有ロック` : データを更新するときに使うロックで、資源がこの状態の場合は他のトランザクションによる読込みや更新ができなくなる

---
DBMSの排他制御に関する記述

- A.**複数の人が同時に更新する可能性のあるデータは、排他制御が必要である**\
DBの一貫性を保つためにデータを更新する際に更新対象のデータに対する他のトランザクションからのアクセスを禁止する方法。同時更新による更新消失を防ぐためには更新時に排他制御を行う必要がある

- アクセス頻度の高いデータの処理速度を上げるためには、排他制御が必要である\
ロック制御のためのオーバヘッドやロックの解除待ちに追加の時間が必要になる。そのため排他制御は処理速度を上げるために行われるわけではない

- 処理速度を上げるため、排他制御を行うデータ範囲は極力大きくすべきである\
表、ブロック、列などの単位でかけられる。範囲が広いほどロック解除待ちが長くなり並行実行力が低下するので、処理速度を高めるにはロック範囲を狭める必要がある

- データアクセス時のデッドロックを防止するために、排他制御が必要である\
共有資源を使用する2つ以上のプロセスが、互いにプロセスを必要とする資源を排他的に使用している場合に発生する。排他制御はデッドロックの発生原因になる
...
---
出現頻度の異なるA, B, C, D, Eの5文字で構成される通信データを、ハフマン符号化を使って圧縮するために、符号表を作成した。`a`に入る符号
...
- A.**110**\
可変長の符号化方式で、出現頻度が高いデータに短い符号、低いデータに長い符号を与え、圧縮を効率的に行う方法。符号化のビット列に同じ符号の並びが存在しないように、ある文字に対応する符号が、他の文字に対応する符号の接頭辞にならないように設定される。これにより単純に先頭から読むだけででコードができる

- 001\
Aの符号`00`が接頭辞になっており、デコードの際に先頭部分がAに変換されてしまうため不適切

- 010\
Bの符号`01`が接頭辞になっており、デコードの際に先頭部分がAに変換されてしまうため不適切

- 101\
Cの符号`10`が接頭辞になっており、デコードの際に先頭部分がAに変換されてしまうため不適切

---
稼働率Pのシステムで構成された多重化システム全体の稼働率を表す式はどれか。並列部分は、どちらか一方が稼働していれば良いものとする

<img width="500" alt="" src="./images/システム稼働率.png">

- A.**$P(1-(1-P^2)^2)$**\
直列部分は $P^2$ 、稼働率Pの機器が並列に接続されている場合の稼働率は $1-(1-P)^2$ 、Pを実際の稼働率 $P^2$ に置き換え、直列接続されているPを掛ける

---
1台のサーバ、3台のクライアント及び2台のプリンターがLANで接続されている。このシステムはクライアントからの指示に基づいて、サーバにあるデータをプリンタに出力する。各装置の稼働率が表のとおりであるとき、このシステムの稼働率を表す計算式はどれか。クライアントは3台のうちどれか1台でも稼働していればよく、プリンターは2台のうちどちらかが稼働していればよい

<img width="500" alt="" src="./images/装置稼働率.png">

- A.**$a(1-(1-b)^3)(1-(1-c)^2)$**\
並列接続で少なくとも1つが稼働していればシステムの稼働が継続していればよい場合、 $1-(1-R_1)(1-R_2)$ 。今回のシステム構成はサーバ・クライアント・プリンタの各1台が稼働していれば稼働を続けられる
...
- A.**MTBFを変えずにMTTRを短くできれば、システムの稼働率は向上する**\
$\frac{MTBF(平均故障間隔)}{MTBF+MTTR(平均修復時間)}$\
稼働率は上記式で求められる。分子は変化せず、分母は小さくなると稼働率は向上する。故障の間隔はそのまま、修復時間が短くなる

- MTBFが異なってもMTTRが等しければ、システムの稼働率は等しい\
MTBFが長いと稼働率は向上する

- MTBFとMTTRの和が等しければ、システムの稼働率は等しい\
2指標の和が等しくても、MTBFが長いほど稼働率は高くなる

- MTTRが変わらずMTBFが長くなれば、システムの稼働率は低下する\
MTTRが等しければ、MTBFが長いほど稼働率は向上する
...
- A.**3**\
稼働率0.9の3つの機器が並列に接続されている場合のシステム全体の稼働率は、\
$1-(1-0.9)^3=1-0.001=0.999$

---
2台のコンピュータを並列に接続して使うシステムがある。どちらか1台が稼働していればよい場合、システムの全体の稼働率は何%か
...
- A.**99.8**\
コンピュータ1の稼働率は0.96、コンピュータ2の稼働率は0.95\
どちらか1台が稼働していればシステムは正常なので並列接続の稼働率を計算する\
$1-(1-0.96)(1-0.95)$\
$1-(0.04*0.05)$\
$1-0.002=0.998$

---
東京と福岡を結ぶ実線の回線がある。東京と福岡の間の信頼性を向上させるために、大阪を経由する破線の迂回回線を追加した。迂回回線追加後における、東京と福岡の間の稼働率は幾らか。回線の稼働率は、東京と福岡、東京と大阪、大阪と福岡の全てが0.9とする

<img width="400" alt="" src="./images/稼働率.png">

- A.**0.981**\
東京 - 大阪の回線稼働率は0.9、東京 - 大阪 - 福岡の回線稼働率は0.81(0.9の2乗)\
迂回回線追加後は、上記のどちらか一方が稼働していれば良いので並列構成と考えて計算する\
$1 - (1 - 0.9)(1 - 0.81)$\
$= 1 - 0.1 * 0.19$\
$= 1 - 0.019 = 0.981$

---
東京 - 大阪及び東京 - 名古屋がそれぞれ独立した通信回線で接続されている。東京 - 大阪の稼働率は0.9、東京 - 名古屋の稼働率は0.8あである。東京 - 大阪の稼働率を0.95以上に改善するために、大阪 - 名古屋にバックアップ回線を新設することを計画している。新設される回線の稼働率は最低限幾らか

- A.**0.625**\
東京 - 大阪の稼働率(0.9)と、東京 - 大阪(0.9)と大阪 - 名古屋のバックアップ回線(R)の並列接続\
```math
1-(1-0.9)(1-0.8R) \geqq 0.95 \\
1-(0.1-0.08R) \geqq 0.95\\
0.9+0.08R \geqq 0.95\\
0.08R \geqq 0.05\\
R \geqq 0.625
```
...
---
装置aとbの MTBF と MTTR が下記の時、直列に接続したシステムの稼働率
...
- A.**0.72**\
それぞれの稼働率は0.8と0.9。2台を直接に接続したシステムの稼働率は、積の0.72

---
MTBFが45時間でMTTRが5時間の装置がある。この装置を2つ直列に接続したシステムの稼働率

- A.**0.81**\
稼働率は、 $45/(45+5)=0.9$\
直列した時の稼働率は $0.9^2=0.81$

---
N個の観測値の平均値を算出する式はどれか。SはN個の観測値の和(S > 0)とし、[X]はX以下で最大の整数とする。平均値は小数第1位を四捨五入して整数値として求める

A.**$[\frac{S}{N}+0.5]$**\
平均値を四捨五入して整数値で求める。整数値で求められるので、小数部分が5以上は切り上げ、4以下は切り下げの処理を行う。計算された平均値(S / N)に0.5足してから小数部分を切り捨てることで、四捨五入と同様の効果を得ることができる。小数第1位が4以下の場合は5を足すと5から9になるので切り捨て、5以上の場合は整数部分が切り上がって小数第1位が0から4になり、小数部分を切り捨てる
...
---
CMMI(*Capability Maturity Model Integration* : 総合能力成熟度モデル)を説明したもの

- A.**ソフトウェア開発組織及びプロジェクトのプロセスの成熟度を評価するためのモデルである**\
組織におけるプロセス改善をガイドするモデル。プロジェクト管理、プロセス管理、エンジニアリング、支援に関するプロセスについて、効果的なプロセスであるために重要なゴール及びプラクティスという概念で整理されている。未成熟なプロセスから有効性を持った秩序ある成熟したプロセスへの進化の改善経路も記述しており、自らの組織の状況に合わせて適用内容を徐々に高速化できる\
...
- A.**システムテストにおいて、単体テストレベルのバグが多発して、開発が予定どおり進捗しない**\
プログラミング工程や設計工程に起因する問題。ソフトウェア全体がどのような品目の組み合わせで構成されているかを構成台帳などで一元管理することをいい、構成識別・バージョン管理、構成制御・プロセス制御、ビルド / リリース管理、変更管理などの作業が含まれる\
システムの構成要素は開発の進行や変化などにより随時変更される。構成管理でソースコード・設計書・マニュアル等の文書類の成果物の変更履歴を計画的に記録することで、ある製品バージョンと構成要素の紐付けが可能になる。不具合発生時には変更履歴を確認することで迅速な原因究明作業ができる

- 開発者がバグを定められた手続に従わずに修正したので、今まで動作していたプログラムが、突然に不正な動作をする\
構成管理が定める変更方法を守らなかったことに起因する問題

- 仕様書、設計書及びプログラムのそれぞれが一致していないので、プログラム修正時にソースプログラムを解析しないと、修正すべきプログラムが特定できない\
設計書・マニュアル類が計画的に管理されていないため発生する問題

- 1つのプログラムから多数の派生プログラムが作られているが、派生元のバグ修正が全ての派生プログラムに反映されない\
プログラム同士の親子関係の記録が疎かなため発生する問題
...
---
マルウェア対策ソフトでフォールスネガティブ(*False Negative*)に該当するもの

- A.**マルウェアに感染しているファイルを、マルウェアに感染していないと判断する**\
本来は検知すべき悪意のある活動を、誤って害のないものとして分類すること(検知漏れ)。多くなる程コンピュータに影響を与え得る攻撃を通過させてしまう可能性が高くなる。2つの発生数はトレードオフの傾向があるが、フィルタリングルールの調整によってどちらも最小になるように改善し続けることが求められる

- マルウェアに感染していないファイルを、マルウェアに感染していると判断する\
フォールスポジティブ(*False Positive*)に該当する。本来は通過させるべき害のない活動を、誤って悪意のあるものとして分類すること。いわゆる過剰検知で、多くなる程正常な操作の阻害回数や管理者の負担が増える
...
---
マルウェアについて、トロイの木馬とワーム(*Warm*)を比較した時、ワームの特徴
...
---
企業内ネットワークやサーバにおいて、侵入者が通常のアクセス経路以外で侵入するためにが組み込むもの
...
- フォレンジックス\
不正アクセスや情報漏洩などのセキュリティインシデントの発生時に、原因究明や法的証拠を保全するために必要な電子的記録を収集解析すること

---
クライアント上のアプリケーションがDB接続プログラム経由でサーバ上のDBのデータにアクセスする。アプリケーションとDB間で送受信されるコマンドや実行結果の漏洩を防止する対策

<img width="400" alt="" src="./images/データアクセス.png">

- A.**DB接続プログラム間の通信を暗号化する**\
この画像の通信経路上で攻撃者によって盗聴されれば、コマンドや実行結果などが漏洩する。端末を制限、パスワードの設定、ポート番号の変更をしても効果はない(認証済の通信を見れるため)。そのため通信経路上の盗聴に対する恒久的な対策は通信を暗号化して行うこと
...
---
コンピュータウイルス対策ソフトのパターンマッチング方式を説明したもの

- A.**既知ウイルスのシグネチャコードと比較して、ウイルスを検出する**\
コンピュータウイルスやワームを検出するための代表的な方式で、特徴的なパターン(シグネチャコード)として検査対象のファイルと比較することでウイルス検出を試みる。検出できるウイルスはパターンファイルに定義されているものに限るので、定義されていないものや亜種が作られるもの、未知のウイルスの検出は困難
...
---
コンピュータで使われている文字符号の説明

- A.**ASCII符号はアルファベット、数字、特殊文字及び制御文字からなり、漢字に関する規定はない**\
ASCII(アスキー)コードは、2進数7桁で表現できる128種類に英数字や記号、改行やタブなどの制御文字などを割り当てた文字コード

- *EUC*は文字符号の世界標準を作成しようとして考案された16ビット以上の符号体系であり、漢字に関する規定はない\
*Unicode*の説明

- *Unicode*は文字の1バイト目で漢字かどうかが分かるようにする目的で制定され、漢字をASCII符号と混在可能とした符号体系である\
SJIS(*Shift_JIS*)の説明
...
---
UNIXの階層的ファイルシステムにおいて、アカウントをもつ一般の利用者がファイルの保存などに使う階層で最上位のもの

- A.**ホームディレクトリ**\
マルチユーザ環境のUNIXのコンピュータにおいて、ユーザが自由にファイルやサブディレクトリを作成したり、ユーザ自身の作成したプログラムや書類、ユーザ独自の設定ファイルなどを保存できるディレクトリ。*UNIX/Linux*は、一般ユーザがログインした際のカレントディレクトリがホームディレクトリ

- カレントディレクトリ\
現在作業中のディレクトリ

- デスクトップディレクトリ\
デスクトップに表示されるファイルや環境情報が入っているディレクトリ

- ルートディレクトリ\
木構造で構成されるコンピュータのファイルシステムのうち、最上位層にあるディレクトリ
...
---
ファイルの格納に関する記述のうち、アーカイブ(*Archive*)の説明

- A.**複数のファイルを1つのファイルにまとめて、記憶装置に格納する**\
複数のファイルを1つのファイルにまとめること、またそのまとめられたファイルのこと。ソフトウェアの場合はアーカイバと呼ばれる

- 主記憶における特定のデータやレジスタの値などを一時的にほかの記憶装置に格納する\
スワッピングの説明

- 同一のファイルを2つのディスクにコピーし、データ保存の信頼性を確保する\
ミラーリング(RAID1)の説明
...
---
分解能が8ビットのD/A変換器に、デジタル値0を入力した時の出力電圧が0Vとなり、デジタル値128を入力した時の出力電圧が2.5Vとなる時、最下位の1ビットの変化によるこのD/A変換器の出力電圧の変化

- A.**2.5/128**\
分解能は、機械装置等がアナログ信号をどの程度細かく表現できるかの性能を表すもの。分解能8ビットなので、このD/A変換器は0~最大出力までの電圧を256階調で任意に出力可能。入力値を表すビット列の最下位1ビットが変化すると入力値は1だけ増減する。0を与えた時の出力電圧は0V、128を与えた時の出力電圧は2.5Vのため、1ビットの変化によって増減する出力電圧は、2.5Vを128等分した量

---
アナログ電圧をデジタル化した後に演算処理することの利点

- A.**演算結果が部品精度、温度変化及び外来雑音の影響を受けにくい**\
アナログ電圧は、値が時間的に連続的に変化する波として表される。デジタル値への変換は、一定周期毎にサンプリングした電圧値を8ビット(256段階)、10ビットなどで表現することで行う。アナログ値では外来ノイズが測定される値に影響するが、デジタル化では一定範囲内の値は全て同じデジタル値に変換される。範囲内に収まっていれば多少のノイズは変換後の値に変化がない

- アナログからデジタルへの変換では誤差が発生しない\
連続的な電圧値を非連続的な離散値で表すため誤差が生じる

- 数値演算において丸め誤差が発生することはない\
デジタルで表現できる有限の値にするために演算過程で丸め誤差が生じることがある

- 電圧が変化してから演算結果を得るまでの遅延時間が発生しない\
デジタル化に要する時間によって遅延が発生することがある

---
音声などのアナログデータをデジタル化するために用いられるPCMで、音の信号を一定の周期でアナログ値のまま切り出す処理

- A.**標本化**\
連続的なアナログ値(振幅、周波数、その他の測定可能な性質)を一定の時間間隔(サンプリング周波数)で測定して、時系列的に連続的でない値(離散値)として収集すること

- 量子化\
小数点以下の値を最も近い整数値で近似する。量子化ビット数が大きいほど細かな値の変化を表現できる
...
---
システム結合テストにおける状態遷移テストに関する記述

- A.**設計されたイベントと内部情報と組合せ通りにシステムが動作することを確認する手法**\
設計段階でシステム要求事項をもとに作成した、状態遷移図・状態遷移表を使用して、システムが正しく挙動するかを検証するホワイトボックステストの手法。状態遷移図は、文章のみの仕様書よりシステムの持つ機能や全体像を感覚的に把握しやすくする役割がある。状態遷移表はシステムの内部状態とイベントの組合せを漏れなくリストアップでき、正常な動作だけでなく不正な遷移についても可視化できるメリットがある。2つを組み合わせることで仕様書の漏れ・抜けをテストで発見しやすくし開発作業の手戻りを少なくする目的がある

- イベントの発生によって内部状態が変化しない計算処理システムのテストに適した手法\
内部状態が変化するシステムのテストで用いられる

- システムの内部情報に着目しないブラックボックステスト用の手法\
単体テストや結合テストで用いられるホワイトボックス用の手法

- データフロー図、決定表を使用してシステムの内部情報を解析する手法\
状態遷移図、状態遷移表を使用する
...
---
階層構造のモジュール群からなるソフトウェアの結合テストを、上位のモジュールから行う。この場合に使用する、下位モジュールの代替となるテスト用のモジュール

- A.**スタブ**\
下位のモジュールの代替となるテスト用モジュール。トップダウンテストにおいて未完成の下位モジュールからの呼び出しに対して適切な値を返す役割をもつ

- ドライバ\
上位のモジュールの代替となるテスト用モジュール。ボトムアップテストにおいて、未完成の上位モジュールの代わりとなるテスト用モジュール。上位モジュールの代わりに下位モジュール内の命令を呼び出す役割をもつ

- エミュレータ\
所定のコンピュータや機械装置を模倣するシミュレータの一種で、他のコンピュータ向けの命令プログラムを解読しながら実行するマイクロプログラム

- シミュレータ\
現実の事象や業務をモデル化して模擬試験を行う装置やプログラム
...
静的解析ツールの構文チェッカの説明。プログラムを実行しない状態で検証を行うためのツール

- テスト対象モジュールに必要なドライバ又はスタブを生成する\
テスト環境設定ツールであるテストベッドツールの説明。テスト環境を準備するためのツール

- テストによって実行した経路から網羅度を算出する\
動的解析ツールであるカバレージモニタの説明。プログラムを実行しながら検証を行うためのツール

- プログラムの特定の経路をテストするためのデータを生成する\
テスト環境設定ツールであるテスト自動生成ツールの説明

---
プログラムの静的解析ツールで検出できるもの

- A.**後に使用されない変数への代入**\
ソースコードを分析することで、定義されたまま使用されていない変数や、代入後使用されていない変数を検出できる。ソースコード中から分かる記述ミスが検出可能\
プログラムを実行しない状態で検証を行うためのツールで、構文チェッカ、コードオーディタ、モジュールインターフェイスチェックツールなどがある。ほとんどはソースコードに対して行われ、使用しているプログラム言語が定める構文通りに記述されているか、コーディング規約を満たしているかなどを検査捨

- 関数ごとの実行処理時間\
実際にプログラムを実行し、デバッガやベンチマークテストで測定する

- プログラム仕様に対する処理の記述漏れ\
ツールではなく、インスペクションやレビューなどの人的な解析で検出する

- 用意したテストケースでは実行されなかった命令\
テストの網羅率は、プログラムを実行してカバレージモニタなどで測定する
...
- A.**極小の集積回路とアンテナの組合せであり、無線自動認識技術によって対象の識別や位置確認などができ、電子荷札に利用される**\
乗車カード以外に、電子荷札、野生動物の調査、自動レジ、家畜の管理などに利用されている
...
- 人間の身体的認証としての生体情報を、個人の識別・認証に利用する技術であり、指紋認証、静脈認証などがある\
バイオメトリクス認証の説明

---
RFIDの活用事例

- A.**微小な無線チップによる人又は物の識別及び管理**\
ID情報を埋め込んだRFタグ(ICタグ)と電磁界や電波を用いることで数cm - 数mの範囲で情報のやり取りを行う技術。見えなくても読むことができ、RFタグが隠れた位置にあったり表面がホコリや泥で汚れていても読取可能

- 紙に印刷されたデジタルコードをリーダーで読み取ることによる情報の入力\
OMR(*Optical Mark Reader* : 光学式マーク読取装置)やバーコードリーダの活用事例

- 携帯電話とヘッドフォンとの間の音声データ通信\
*Bluetooth*の活用事例

- 赤外線を利用した近距離データ通信\
IrDA(*Infrared Data Association*)の活用事例。RFIDは赤外線ではなく電波を使う
...
---
関数や手続きを呼び出す際に、戻り番地や処理途中のデータを一時的に保存するのに適したデータ構造

- A.**スタック**\
後入れ先出し(*Last in First Out*)のデータ構造で、メインルーチンからサブルーチンを呼び出す時にメインルーチンの戻り番地や変数の値を一時的に格納しておく時などに使われる。ローカル変数や元の手続きの呼び出し位置などをスタックに積んでいき、関数が終了する度に前のデータをスタックから取り出して手続きを復帰させる

---
空の2分探索木に8, 12, 5, 3, 10, 7, 6の順にデータを与えた時にできる2分探索木
<img width="400" alt="" src="./images/探索木.png">
...
DEQ : 待ち行列からデータを取り出す\
空の待ち行列に対し、ENQ1, ENQ2, ENQ3, DEQ, ENQ4, ENQ5, DEQ, ENQ6, DEQ, DEQの操作を行った。次にDEQ操作を行った時、取り出される値

- A.**5**\
待ち行列(キュー)では、先入先出方式のデータ構造\
ENQ1 -> 1\
ENQ2 -> 2, 1\
ENQ3 -> 3, 2, 1\
DEQ -> 3, 2(1を取り出す)\
...
---
2つのスタック操作を定義する。空のスタックに対して、次の順序でスタックでスタック操作を行った操作\
push n : スタックにデータ(整数値n)をプッシュする\
pop : スタックからデータをポップする\
・push 1, push5, pop, push7, push6, push4, pop, pop, push3

- A.**(新しいデータから)3, 7, 1**\
最後に格納したデータから先に取り出す、後入れ先出し(*Last-in First-out*)構造をもつ。データ挿入をpush命令、取り出すpop命令を使用してデータ操作を行う\
push1, 5 -> 5, 1\
pop -> 1\
push7, 6, 4 -> 4, 6, 7, 1\
pop, pop -> 7, 1\
...
---
データ構造の1つであるリストは、配列を用いて実現する場合と、ポインタを用いて実現する場合とがある。配列を用いて実現する場合の特徴はどれか。配列を用いたリストは、配列に要素を連続して格納することによって構成し、ポインタを用いたリストは、要素から次の要素へポインタで連結することによって構成するものとする

- A.**位置を指定して、任意のデータに直接アクセスすることができる**\
各要素の位置を"先頭から何番目"のように指定して参照できる。リスト構造では、目的の要素に到達するまで先頭(後方)から順にポインタを辿る必要がある

- 並んでいるデータの先頭に任意のデータを効率的に挿入することができる\
配列の先頭にデータを挿入するには、現在の要素全体を挿入されるデータの分だけ後ろにずらす必要がある。要素の数が多ければその分挿入に伴う処理量は増加する。リスト構造では数個のポインタを付け替えるだけで容易に先頭への挿入ができる

- 任意のデータの参照は効率的ではないが、削除や挿入の操作を効率的に行える\
任意のデータ参照には直接アクセス可能な配列構造が有利。削除や挿入は要素の移動回数が多くなるため非効率

- 任意のデータを別の位置に移動する場合、隣接するデータを移動せずにできる\
配列内のデータ移動では、移動対象を一旦退避し、空いた箇所を詰めるように隣接データをずらす必要がある

---
リストを2つの1次元配列で実現する。配列要素box[i]とnext[i]の対がリストの1つの要素に対応し、box[i]に要素の値が入り、next[i]に次の要素の番号が入る。配列が下記の状態の場合、リストの3番目と4番目との間に値のHである要素を挿入した時のnext[8]の値はどれか。next[0]がリストの先頭(1番目)の要素を指し、next[i]の値が0である要素はリストの最後を示し、next[i]の値が空白である要素はリストに連結されていない

<img width="500" alt="" src="./images/配列要素.png">

- A.**7**\
元の配列はA5 -> E3 -> (3)C7 -> (7)G2 …となる。(3)C8 -> (8)H7 -> (7)G2と書き換えると辻褄が合う

---
多数のデータが単方向リスト構造で格納されている。このリスト構造には、先頭ポインタとは別に、末尾のデータを指し示す末尾ポインタがある。ポインタを参照する回数が最も多いのはどれか

- A.**リストの末尾のデータを削除する**\
末尾データを削除するには、末尾の1つ前のノードがもつ次ノードへの参照を空にし、末尾ポインタに末尾の1つ前のデータのポインタをセットしないといけない。単方向リストなので末尾のデータから1つ前のデータへの逆方向の参照はできず、先頭から末尾の1つ前まで順番にポインタを辿っていく必要がある。ポインタを辿る回数がリストが保持するデータ数にほぼ等しい回数になる

- リストの先頭にデータを挿入する\
挿入するデータの次ノードへの参照に現在の先頭ポインタの値をセットし、リストの先頭ポインタに挿入するデータポインタをセットするので、ポインタを参照する回数は1回

- リストの先頭のデータを削除する\
現在の先頭のデータが持つ次ノードへの参照を、先頭ポインタにセットすることで先頭のデータがリストから削除されたことになるので、先頭ポインタ -> 先頭データで1回

- リストの末尾にデータを挿入する\
末尾ポインタから末尾データに飛び、末尾データの次ノードへの参照を追加するデータにする。末尾ポインタに追加するデータの参照をセットするので、末尾ポインタ -> 末尾データで1回
...
---
A社では、1,000台のパソコンを使用している。これらのパソコンについて、平均故障台数を20日間で2台以下に抑えたい。これらのパソコンのMTBFは少なくとも何時間必要か。パソコンの平均使用時間は1日8時間とする

- A.**80,000**\
システムが修理され正常に稼働し始めてから次回故障するまでの平均故障間隔。1,000台のパソコンが1日8h20日間稼働するので、総稼働時間は、`160,000h`。システム稼働時間 / 故障回数で計算できるので、総稼働時間の半分(2台)となる

---
次の条件でITサービスを提供している。SLAを満たすには、サービス時間帯中の停止時間は1ヶ月以内に最大で何時間以内であればよいか。1ヶ月の営業日は30日とする
[SLAの条件]\
・サービス時間帯は営業日の午前8時から午後10時まで\
・可用性は99.5%以上とすること

- A.**2**\
1日14hのサービスで、30日間なので、1ヶ月のサービス時間は`420h`\
可用性99.5%以上を維持するには、420*0.005=2.1hを超えない時間の停止が許容される
...
---
システムの信頼性を表す指標であるRASのうち、可用性(*Availability*)を表す尺度

- A.**稼働率($\frac{MTBF}{MTBF+MTTR}$)**\
RAS(*Reliability* : 信頼性, *Availability* : 可用性, *Serviceability* : 保守性)の頭文字を合わせたもので、システムの信頼性を表す指標。可用性は、システムの必要な機能を必要な時に使うことができるかどうか度合いを表す\
稼働率は、前運転時間に占める正常に稼働していた時間の割合で、可用性の指標として用いられる。システムの修復が完了し正常稼働し始めてから、次回故障するまでの平均故障間隔(MTBF)とシステムの故障を修理するために要した平均修復時間(MTTR)を使った式で表される
...
---
RAID(*Redundant Arrays of Independent Disks*)1 - 5の各構成は、何に基づいて区別されるか

- A.**データ及び冗長ビットの記録方法と記録位置の組み合わせ**\
複数台の安価な磁気ディスクを組み合わせ、1つの仮想的なディスクとして扱うことで信頼性や性能を向上させる技術。レベルによって本来のデータとエラー訂正用のデータ(ハミング符号・パリティビット)を、どう書き込むかによって定義されている\
`RAID0` : 複数のディスクにデータを分散して書き込むストライピング\
`RAID1` : 2台の磁気ディスクに同じ内容を書き込むミラーリング\
`RAID2` : ストライピング + エラー訂正用のハミング符号をディスクに書き込む。最少でも5台構成\
`RAID3` : ストライピング + エラー修正用にパリティビットを1台のディスクに書き込む\
`RAID4` : RAID3のパリティビットの書き込みをブロック単位で行う\
`RAID5` : パリティビットもデータと同様に分散して書き込む

---
コンピュータシステムの高信頼化技術は、目標とする特性からRASISと呼ばれる。構成する5要素

- A.**信頼性, 可用性, 保守性, 保全性, 機密性**\
*Reliability*(信頼性) : 故障の起こしにくさの度合い。評価指標はMTBF(平均故障間隔)\
*Availability*(可用性) : 必要時に必要なだけシステムが使用可能状態である度合い。評価指標は稼働率\
*Serviceability*(保守性) : システム障害時の修理しやすさの度合い。評価指標はMTTR(平均修復時間)\
*Integrity*(保守性・安全性) : 保存されているデータが安全であるかどうかの度合い\
*Security*(安全性・機密性) : 保存されているデータが災害や障害などに対してもつ耐性の度合い

---
データを細分化して複数台の磁気ディスクに格納する名称は何か。 $b_0 - b_{15}$ はデータがビット毎にデータディスクに格納される順番を示す

<img width="400" alt="" src="./images/データディスク.png">

- A.**ストライピング**\
$b_0 - b_{15}$ の別のデータを各データディスクに分散して格納している。性能向上を目的としたRAID0に相当し、各データディスクに並列にアクセスを行うことで読み書き速度の向上が望める

- ディスクキャッシュ\
主記憶と補助記憶との速度差を緩和するために設置されるキャッシュメモリ

- ブロック化\
複数のレコードを一定数毎にまとめる。1レコードごと読み書きするよりも、ブロック単位でアクセスすることで、処理効率を向上させる

- ミラーリング\
RAID1に相当し、2台のディスクに同じデータを書き込む。片方のディスクに障害が起きても問題ないのでディスクの信頼性が向上する

---
運用テストにおける検査内容

- A.**利用者に提供するという視点で、システムが要求を満足していることを確認する**\
システムの運用部門が業務の流れに沿ってシステムのテストを行い、実際の稼働状況おいて不具合が発生しないか検証するために実施されるテスト

- 個々のソフトウェアユニットについて、仕様を満足していることを確認する
- ソフトウェア品目の中で使用しているアルゴリズムの妥当性を確認する\
単体テストの説明

- ソフトウェアユニット間のインターフェイスが整合していることを確認する\
結合テストの説明

---
システムの一部に修正を加えた時に、修正部分が他に悪影響を及ぼさずに正しい結果が得られることを検証するテスト

- **退行テスト**\
システム変更作業を実施した場合に、以前まで正常に機能していた部分に不具合や影響が出ていないか検証する。リグレッションテストや回帰テストとも呼ばれる

- 機能テスト\
システムが要求された全ての機能を満たしていることを確認するテスト

- 結合テスト\
単体テストが終了したモジュール同士を組み合わせて行うテストで、主にモジュール間のインターフェイス(接点)が設計通りに機能するか検証するために実施される

- 例外テスト\
例外ケースや異常ケースが入力された場合に、自動的に修正したりエラーメッセージを出力するなど例外ケースに対する耐性を検証するテスト
...
---
産業機器の機器制御に使われる、シーケンス制御の説明

- A.**あらかじめ定められた順序又は条件に従って、制御の各段階を逐次進めていく制御方法である**\
決められた順序通りに行う処理を自動制御する。スイッチを押すと予め瀬定した通りに材料を加工するNC機械、水の投入から脱水・乾燥までを行う全自動洗濯機などに活用されている
...
---
システム障害が発生した時にシステムを初期状態に戻して再開する方法で、更新前コピー又は更新後コピーの前処理を伴わないシステム開始のことで、初期プログラムロードとも呼ばれるもの

- A.**コールドスタート**\
システムの電源を入れ直し、初期状態に戻してからプログラムを起動して処理を再開する方法

- ウォームスタート\
システムの電源をOFFせず、そのままの状態でプログラムを再起動して処理を再開する方法

- ロールバック\
更新前ログを用いてDBをトランザクション開始直前の状態に戻す処理

- ロールフォワード\
更新後ログを用いて今まで処理していたトランザクションを再現しシステム障害直前までDB情報を復帰させる処理

---
バックアップ方式の説明のうち、増分バックアップはどれか。ここで、最初のバックアップでは、全てのファイルのバックアップを取得し、OSが管理しているファイル更新を示す情報はリセットされるものとする

- A.**直前に行ったバックアップの後、ファイル更新を示す情報があるファイルだけをバックアップし、ファイル更新を示す情報はリセットする**\
定期的にフルバックアップを行い、フルバックアップの期間は、**前回の増分バックアップ以降に変更されたファイルだけ**を記録する方式。バックアップ1回当たりのデータ量は少なく済むが、回復はフルバックアップに加え複数の増分バックアップの適用が必要なので、障害発生時の復旧時間は長くなる

- 最初のバックアップの後、ファイル更新を示す情報があるファイルだけをバックアップし、ファイル更新を示す情報は変更しないでそのまま残しておく\
差分バックアップの説明。定期的にフルバックアップを行う。フルバックアップの間の期間は、フルバックアップ以降に変更された全てのファイルを記録する。障害発生時にはフルバックアップと**直前の差分バックアップ1つ**を使用して復旧する
...
フルバックアップの説明。毎回前ファイルのバックアップを行う。復旧時間は短くなるが、バックアップ対象のデータ量が多いためバックアップに要する時間は長い
...
- A.**業務処理がバックアップ処理と重なると正常にバックアップされない可能性があるので、両方の処理が重ならないようにスケジュールを立てた方が良い**\
業務に影響が出るのであれば、バックアップを業務時間外に実施するのが適切な運用

- バックアップ作業時間を短くするためには、同一記憶媒体内に別のファイル名にしたバックアップデータを置いた方が良い\
同一記憶媒体にバックアップを保存すると媒体障害時に復旧ができない

- バックアップデータからの復旧時間を短くするためには、差分バックアップを採用した方が良い\
差分バックアップ方式における復旧手順は、フルバックアップを適用し、その後に時系列に沿って各差分を適用する。復旧時間に関しては、差分バックアップ方式よりフルバックアップ方式を採用した方が短くなる

- バックアップ用媒体は、ランダムアクセスが可能な媒体にした方が良い\
ランダムアクセスできる記憶媒体の寿命は比較的短く、長期間の保存には向かない。磁気テープはシーケンシャルアクセスしかできないが30年程度は記憶内容を保持できる。また、ランダムアクセスできる媒体は容量当たりのコストが高くなる

---
パソコンでの記憶媒体のフォーマット処理に関する記述

- A.**論理フォーマットでは、OS固有のファイルシステムの管理領域や実際に記録されるデータの論理的な位置に固定する**\
記録メディアをコンピュータなどの機器で利用可能にするために初期化(イニシャライズ)する\
`物理フォーマット` : 記録メディアやハードディスクに記録されている全てのデータを`00`や規定のパターンで上書きし初期化を行う処理\
`論理フォーマット` : 物理フォーマット後の記憶媒体に、パーティションやファイルシステムのための必要なデータを書き込む処理

- 磁気ディスクでは、物理フォーマットの処理に先立って、論理フォーマットを行う必要がある\
論理フォーマットは、物理フォーマットの後に行われる

- 物理フォーマットでは、磁気ディスク上にパーティションを作成し、装置として認識可能にする\
論理フォーマットの説明

- 論理フォーマットでは、不良セクタを同時に行う。エラー発生時には予備領域内の正常なセクタを不良セクタを代替することができる\
物理フォーマットの説明。物理フォーマットを実施すると、エラー検出や発見された不良セクタを予備のセクタへの置き換えなどの処理が行われる

---
500バイトのセクタ8個を1ブロックとして、ブロック単位でファイルの領域を割り当てて管理しているシステムがある。2,000バイト及び9,000バイトのファイルを保存する時、これら2つのファイルに割り当てられるセクタ数の合計は幾らか。ディレクトリなどの管理上方が占めるセクタは考慮しない

- A.**32**\
500バイトのセクタ8個を1ブロックとしている。どれだけ格納するデータサイズが小さくても、最低1ブロック(4,000バイト)の領域を割り当てないといけない。2,000バイトのファイルは1ブロック(4,000バイト)、9,000バイトのファイルは3ブロック(12,000バイト)が必要。合計は4ブロック(32セクタ)
...
各々の企業が提供しているWebサービスを組み合わせて、新しい1つのWebサービスのようにする機能・開発手法。ITの深い知識がなくても、既存のWebサービスを組み合わせて、短期間でアプリケーション開発ができ、新しい開発手法として注目されている
...
ソースコードを公開することなどの条件の下で、ソフトウェアの変更と派生ソフトウェアの作成を許可している

- 使用分野及び利用者を制限して再配布できる\
特定の個人やグループ、利用する分野に対する差別をしてはならない
...
---
フィッシング(*phishing*)の手口に該当するもの

- A.**電子メールを発信して受信者を誘導し、実在する会社などを装った偽のWebサイトにアクセスさせ、個人情報を騙し取る**\
...
- Webページに入力した内容をそのまま表示する部分がある場合、ページ内にスクリプトを埋め込み、ユーザとサーバに被害を与える\
クロスサイトスクリプティングの手口

- ウイルスに感染したコンピュータを、インターネットなどのネットワークを通じて外部から操る\
ボットの手口

- コンピュータ利用者のIPアドレスやWebの閲覧履歴などの個人情報を、密かに収集して外部へ送信する\
スパイウェアの手口
...
---
関係DBにおいて、外部キー定義を行う目的

- A.**関係する相互のテーブルにおいて、レコード間の参照一貫性が維持される制約をもたせる**\
関係DBの2テーブル間の参照整合性を保つための制約で、SQLでは設定する属性に対して*foreign key*句を指定する。正規化された表同士を外部キーで結合することで、データの一貫性・正当性を保つことができる

---
関係DBの主キー制約の条件として、キー値が重複していないことの他に、主キーを構成する列に必要な条件

- A.**キー値(*primary key*)が空でないこと**\
表内のある行を一意に決められる特性をもつ属性。候補キーには*null*が許されるが、主キーでは*null*の存在が許されないので、主キー制約は一意性制約に*not null*制約を加えたものとも言える

- 構成する列が1つであること\
複数の属性を合わせて主キーとすることもある(複合キー)
...
- A.**保険に加入するなど資金面での対策を講じること**\
他社に金銭的なリスクを移転することで、リスクファイナンスのリスク移転に該当する。リスクコントロールのリスク移転では、保険をかけたりリスクのある業務を委託したりすることでリスクを他者に転嫁する対策
