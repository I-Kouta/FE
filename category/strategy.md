...
---
いずれも時価100円の株式A-Dのうち、1つの株式に投資したい。経済の成長を高、中、低の3つに区分した時のそれぞれの株式の予想値上がり幅は表の通り。マクシミン原理に従う時、どの株式に投資することになるか
...
- A.**A**\
将来の起こり得る状態は予想できても、発生確率が不明な場合の意思決定の判断基準として用いられる。マクシミン原理は、各戦略を選択した時に得られる最小利得が最大となる戦略を選ぶ保守的な考え方。各株式の最小利得は下記の通り\
A.成長率・中の時に10円\
B.成長率・中の時に5円\
C.成長率・低の時に5円\
D.成長率・低の時に−10円\
マクシミン原理に従う場合、それぞれの最小利得を比較して最も利益が大きくなるAに投資すると結論づけられる

---
昨年度と今年度の入社試験問題を比較するために、多数の社員に両年度の問題を解答させた。昨年度の問題の得点をx軸、今年度の問題の得点をy軸にとり、相関係数と回帰直線を求めた。この結果から分かること\
結果\
・相関係数は0.8であった\
・回帰直線の傾きは1.1であった\
・回帰直線のy切片の値は10であった

- A.**回帰直線の傾きとy切片の値から、今年度の問題は昨年度の問題に比べて得点しやすい傾向にあることが分かる**\
回帰直線の式は`y=1.1x+10`の一次関数と分かる。$x \geqq 0$ であれば、昨年度の点数より今年度の点数の方が高くなる関係になり、得点しやすいと言える

- 回帰直線のy切片の値から、今年度の問題の得点が0点の人でも、昨年度の問題では10点程度とれることが分かる\
説明とは逆で、昨年度は0点でも今年度の問題は10点程度取れる

- 回帰直線の傾きから、今年度の問題の平均点は、昨年度の問題の平均点の1.1倍であることが分かる\
今年度の平均点は昨年度の得点の1.1倍に10点を加算した程度になる

- 回帰直線の傾きと相関係数の値から、今年度の問題は質が高いことが分かる\
相関係数と回帰直線から問題の質は判断できない
...
---
企業の事業活動を機能ごとに主活動と支援活動に分け、企業が顧客に提供する製品やサービスの利益は、どの活動で生み出されているか分析する手法

- A.**バリューチェーン分析**\
業務を購買物流・製造・出荷物流・販売/マーケティング・サービスの5主活動と、調達・技術開発・人事/労務管理・全般管理の4つの支援活動に分類し、製品の付加価値がどの部分(機能)で生み出されているか分析する手法。自業活動を価値創造活動の集合と捉え、価値の連鎖を分析することで競合他社と比較した自社の強みと弱みと明らかにし、業務最適化を図る目的がある

- 3C分析\
マーケティング分析に必要不可欠な3要素、*Customer*(顧客)、*Company*(自社)、*Competitor*(競合他社)について自社の置かれている状況を分析する手法

- SWOT分析\
企業の置かれている経営環境を分析し、今後の戦略立案に活かす方法の1つ。内部要因の強み・弱み、外部要因の機会・脅威に分けて分析する

- ファイブフォース分析\
業界の収益性を決める5つの競争要因から、業界の構造分析を行う手法。供給企業の交渉力・買い手の交渉力・競争企業間の敵対関係の3つの内的要因と、新規参入者の脅威・代替品の脅威の2つの外的要因、系5つの要因から業界全体の魅力度を測る
...
---
ビッグデータ分析の前段階として、非構造化データを構造化データに加工する処理を記述している事例

- A.**ソーシャルメディアの口コミを機械学習によって単語ごとに分解し、要約を作り、分析可能なデータに加工し、関係DBに保管する**\
関係DBや表計算ソフトのデータなど、データの格納形式が定まっているデータが構造化データに該当する。特定の形式がなく、元の形のまま保存されるデータが非構造化データで、画像、動画、音声などが含まれるJSONやXMLのようにデータの形式が決まっているものの、様々なデータを含めることができるものは半構造化データ

- 関係DBに蓄積された大量の財務データから必要な条件に合致するデータを抽出し、利用者が扱いやすい表計算ソフトウェアデータに加工する\
関係DB内のデータ、表計算ソフトウェアデータはどちらも構造化データなので、構造化データから構造化データに加工している

- 個人情報を含むビッグデータを更に利活用するために、特定の個人を識別することができないように匿名化加工する\
匿名化加工はデータ内容を変更するもので、格納形式は変更されないため、データ形式は非構造化データのままで変わらない

- 住所データ項目の中にある、"ヶ"と"が"の混在や、丁番地の表記不統一を、標準化された表記へ統一するために加工する\
事前に定義された形式のデータなので構造化データ。表記方法の統一加工はデータの内容を変更するものであり、格納形式は変更されないため、データ形式は構造化データのままで変わらない

---
ビッグデータ活用の発展過程を次の4段階に分類した場合、第4段階に該当する活用事例\
[ビッグデータ活用の発展段階]\
第1段階 : 過去や現在の事実の確認(どうだったのか)\
第2段階 : 過去や現在の状況の解釈(なぜそうだったのか)\
第3段階 : 将来生じる可能性がある事象の予測(なぜそうなのか)\
第4段階 : 将来の施策への展開(どうしたら良いのか)

- A.**ビッグデータを基に、利用者の誤操作の原因と、それによる故障率の増加を推定し、利用者の誤操作を招きにくいユーザーインタフェースに改良する**\
第4段階に該当する

- 製品のインターネット接続機能を用いて、販売後の製品からの多数の利用者による操作履歴をビッグデータに蓄積し、機能の使用割合を明らかにする\
第1段階(事実分析)に該当する

- 多数の利用者による操作履歴が蓄積されたビッグデータの分析結果を基に、当初、メーカーが想定していなかった利用者の誤操作とその原因を見つけ出す\
第2段階(新たな知見の獲得)に該当する
...
---
1回の発注量を40個とする場合を、1回の発注量を100個とする場合を比べた時、仕入額、発注費、保管費用の年間総額はどうなるか。在庫は一定の割合で減少し、在庫がなくなると同時に入荷するものとする
...
- A.**182万円高い**\
年間発注量は400個なので、1回の発注量は40個の場合は10回、100個の場合は4回。在庫は一定の割合で減少し、在庫がなくなると同時に入荷するので、保管している在庫量は発注量 / 2となる\
1回の発注量が40個の場合 :\
...
改善提案1 : 改善額10万円未満N, 期間短縮1週間未満Yなので、左から3列目のケースに該当し、賞金は1,000円\
改善提案2 : 改善額10万円未満Y, 期間短縮1週間未満Nなので、左から2列目のケースに該当し、賞金は1,000円

---
CGM(*Consumer Generated Media*)の説明

- A.**個人が制作したデジタルコンテンツの閲覧者・視聴者への配信や利用者同士の共有を可能とするもの**\
主にインターネットを利用して消費者やユーザがその内容を生成する携帯のメディア。不特定多数の個人によって書き込まれる記事や作品をDB化してコンテンツとするWebサイトがこれに該当し、ブログやSNS、口コミサイト、Q&Aコミュニティ、写真共有サイトや動画共有サイトなどが代表

- オークション形式による物品の売買機能を提供することによって、消費者同士の個人売買の仲介役を果たすもの\
インターネットオークションやインターネットフリーマーケットの説明
...
---
クラウドソーシング(*Crowd Sourcing*)の説明

- A.**発注者がインターネット上で発注対象の業務内容や発注条件を告知し、受注者を募集する**\
マッチングを行うWebサイトを通じ、不特定多数の人に仕事を依頼できるサービス。Web制作、デザイン、システム開発、メディアコンテンツ制作などの専門スキルが求められる仕事だけでなく、データ入力・テープ起こし、体験談・感想の執筆などの比較的簡単な作業まで様々。外注費用を抑えスピーディな仕事の完成を求める企業において利用が広がるほか、余暇や隙間時間を活かして副収入を得たり、フリーランスが仕事を受託するための手段として活用されている
...
- 商売の売手がインターネット上で対象商品の内容や希望する販売条件を告知し、入札者が価格を競い落札する\
インターネットオークションの説明
...
システムやソフトウェアにおける性能や信頼性、使いやすさなど、機能そのもの以外の要件を指す。品質に関する要求や制約条件などが該当する。非機能要件は、システムがどのように動作するかを決定する要素で、ユーザにとっての満足度や体験に影響を与える。システムの応答時間や同時接続数、セキュリティの強度、可用性などがある
...
ISO 10646で制定されている世界統一文字コードの規格。1文字を2バイトで表現するので、最大65536文字を割り当てられる。現在は1文字4バイトで表現し、最大21億文字を割り当てることができるUCS - 4が定義されている
...
---
携帯電話端末の機能の1つであるテザリング(*Tethering*)の説明

- A.**携帯電話端末をモデム又はアクセスポイントのように用いて、PC、ゲーム機などから、インターネットなどを利用したデータ通信をすること**\
モバイル情報通信端末(スマホなど)がもつ携帯回線などのインターネット接続機能を介して、モバイル端末に接続された他のコンピュータをインターネットに接続すること。ADSLや光ファイバなどの環境や、データ通信カードなどを新たに契約せず、電話機からの接続のみでインターネットが利用できるメリットがある

- 携帯電話端末に、異なる通信事業者のSIMカードを挿して使用すること\
SIMフリーの説明。SIMカードは、携帯電話で使われている電話番号を特定するための固有のID番号が記録されたICカードのこと
...
---
車載機器の性能の向上に関する記述のうち、ディープラーニング(*Deep Learning*)を用いているもの

- A.**システムが大量の画像を取得し処理することによって、歩行者と車をより確実に見分けることができるようになった**\
人間や動物の脳神経をモデル化したアルゴリズム(ニュートラルネットワーク)を多層化したものを用意し、十分な量のデータを与えることで、人間の力なしに自動的に特徴点やパターンを学習させることを呼ぶ。人工知能分野における要素技術の1つで、真相学習とも呼ぶ。中間層の多層化によって複雑なパターンの表現と計算を可能にしていることが特徴

- 車の壁への衝突を加速度センサーが検知し、エアバッグを膨らませて搭乗者をケガから守った
- 自動でアイドリングストップする装置を搭載することによって、運転経験が豊富な運転者が運転する場合よりも燃費を向上させた
- ナビゲーションシステムが、携帯電話回線を通してソフトウェアのアップデートを行い、地図を更新した\
大量のデータからシステム自身が学習する仕組みではない
...
---
A社がシステム開発を行うに当たり、外部業者B社を利用する場合の契約に関する記述

- A.**請負契約によるシステム開発では、特に契約に定めない限り、開発されたプログラムの著作権はB社に帰属する**\
請負契約で特段の取り決めがない場合には著作物は受託側に帰属する

- 委任契約ではB社に成果物の完成責任がないので、A社がB社の従業員に対して直接指揮命令権を行使する\
委任契約では、B社の従業員はB社の指揮命令の下で業務に従事する

- 請負契約、派遣契約によらず、いずれの場合のシステム開発でも、B社にはシステムの完成責任がある\
完成責任は請負契約では生じるが、派遣契約ではない

- 派遣契約では、開発されたプログラムに重大な欠陥が発生した場合、B社に契約不適合責任がある\
派遣契約ではB社の従業員がA社の指揮命令の下で作業することになるため、欠陥の責任を負うのは指揮命令を行なったA社にある
...
---
コンカレントエンジニアリング(*Concurrent Engineering*)の説明

- A.**製品開発において、設計、生産計画などの工程を同時並行的に行う手法**\
主に製造業において、設計から生産に至るまでの各プロセスを同時並行に行うことで、開発機関や納期の短縮・生産コストの削減を図る手法
...
- A.**経営戦略を立てるために、自社の強みと弱み、機会と脅威を分析する手法である**\
*Strength*(強み)、*Weakness*(弱み)、*Opportunity*(機会)、*Threat*(脅威)の各単語の頭文字をとったもの。企業の置かれている経営環境を分析し、今後の戦略立案に活かす手法の1つ。強みと弱みは企業の内部要因、機会と脅威は外部環境要因に分類される
...
---
リスクや投資価値の類似性で分けたカテゴリ毎の情報化投資について、最適な資源分配を行う手法

- A.**ITポートフォリオ**\
ポートフォリオの考え方を情報化投資戦略に応用したもの。IT投資を目的やリスクの特性毎にカテゴライズし、カテゴリ毎に投資割合を管理することで、限りある経営資源を有効に配分することが可能。戦略目標達成型・業務効率化型・インフラ構築型の3カテゴリに分類した後、幾つかの評価項目を用いて優先順位を決定する

- 3C分析\
マーケティング分析に必要不可欠な3要素、*Customer*(顧客)、*Company*(自社)、*Competitor*(競合他社)について自社の置かれている状況を分析する手法

- エンタープライズアーキテクチャ\
組織の全体最適化の観点より、業務及びシステム双方の改革を実践するために、統一的な手法でモデル化し改善することを目的とした、設計・管理手法

- ベンチマーキング\
自社製品・サービス及びプロセスを定量的・定性的に測定し、業界で最も成功を収めている企業(ベスト企業)のものと比較しギャップを把握する分析手法

---
IT投資マネジメントを、プロジェクト単位での最適化を目的とする個別プロジェクトマネジメントと企業レベルの最適化を目的とする戦略マネジメントの2つに分類した場合、戦略マネジメントでの実施項目

- A.**全社規模でのIT投資評価の方法や、複数のプロジェクトから成るIT投資ポートフォリオの選択基準を決定し、全社IT投資テーマを起案する**\
複数のプロジェクトを横断してIT投資の実行管理を行っているため、戦略マネジメントに位置付けられる業務内容。戦略マネジメントは組織全体のIT投資を実行・管理し、個別プロジェクトマネジメントは戦略マネジメントで選定された各プロジェクトを実行管理をする関係<br><br>
`戦略マネジメント` : 企業全体または事業部門レベルでのIT投資を俯瞰的に捉え、経営戦略と整合した投資の方向性や優先度を決定する役割を担う。ITポートフォリオの管理、投資プログラムの実行管理を行い、複数のプロジェクトを全社的視点で選別・調整する。投資対象プロジェクトの選別(P) -> 個別プロジェクトマネジメント(D) -> 経営者の視点での目標実現度の評価・課題抽出(C) -> マネジメント・プロセスの見直し、ポートフォリオや投資内容の見直し(A)\
`個別プロジェクトマネジメント` : 戦略マネジメントの決定も基づき、選定された各プロジェクトを実際に実行・管理する役割を持つ。個々のプロジェクトを適切に運営し、計画した投資効果の実現を目指すことが目的。個別投資計画の作成(P) -> 開発プロジェクトマネジメント(D) -> プロジェクト実行の進捗、変更管理、投資の事後評価、課題抽出(C) -> マネジメント・プロセスの見直し、他の投資計画への反映(A)

- 実施中のプロジェクトの評価を行い、全社IT統括部門に進捗状況などを報告した上で、必要に応じて実施計画を修正する\
特定のプロジェクトの実行管理に関する内容なので、個別プロジェクトマネジメントの実施項目

- 全社IT投資計画を基にプロジェクトの実施計画を策定し、投資目的・目標の設定と、投資額の見積りを行い、予算の配分を判断する\
特定のプロジェクトの実行計画に関する内容なので、個別プロジェクトマネジメントの実施項目

- プロジェクトが完了してから一定期間が経過した後、実施計画段階で設定した効果目標が達成されているか否かを実績に基づいて検証する\
特定のプロジェクトの事後評価に関する内容なので、個別プロジェクトマネジメントの実施項目

---
IT投資評価を、個別プロジェクトの計画・実施・完了に応じて、事前評価・中間評価・事後評価として実施する。事前評価について説明したもの

- A.**投資目的に基づいた効果目標を設定し、実施可否判断に必要な情報を上位マネジメントに提供する**\
効果目標の瀬定・実施可否判断という内容から、事前評価に該当する。軽お悪段階でITプロジェクトの投資評価を行い、実施の可否判断を行うための判断材料を上位マネジメントに提供する

- 事前に設定した効果目標の達成状況を評価し、必要に応じて目標を達成するための改善策を検討する\
変更の要否を判断する目的で実施されるため、中間評価に該当する。実施中のITプロジェクトの実績を評価し、継続・中断・中止などの判断をするための情報を上位マネジメントに提供する

- 実施計画と実績との差異及び原因を詳細に分析し、投資額や効果目標の変更が必要かどうかを判断する\
目標の達成状況を確認する目的で実施されるため、事後評価に該当する。システム稼働後にITプロジェクトが事前に設定した目的や効果を達成したかどうかを評価し、結果を上位マネジメントに報告する
...
- A.**製品1単位当たりの達成すべき原価を設定し、それを基準に計算した結果と実際原価との原価差異を分析する**\
実際の製造で必要となった実際原価ではなく、統計などに基づいて算定した標準原価によって製品原価を計算する方式。標準原価と実際に掛かった原価を比較することで、原因を分析(差異を把握)し原価管理を効率的に行うことが可能

- 原価要素を変動費と固定費に分類し、変動費を製品原価として計算し、固定費は発生した期間の費用(期間原価)として計算する\
直接原価計算の説明

- 新製品の企画・開発段階において、目標販売価格から目標利益を差し引いた許容原価を基に目標原価を設定する\
目標原価の設定法の1つである割付法の説明

- 発生した原価を、活動単位に集計して活動コストを求め、それを製品やサービスに割り当てる\
ABC(*Activity Based Costing* : 活動基準原価計算)の説明
...
---
工場Xでは、ある原料から3種類の製品A, B及びCを生産している。各製品の単位量当たりの製造時間と原料所要量及び利益額は表に示す通りである。工場の月間合計製造時間は最大240時間であり、投入可能な原料は月間150kgである。この時、製品A, B, Cをどれだけ作ると最も高い利益が出るかも知りたい。この問題を解くのに適切な手法
...
- A.**線形計画法**\
1次式を満たす変数の値の中で式を最大化(最小化)する値を求める方法。在庫としてもつ原材料を使用して最大の利益を得るための販売量や、機械の稼働時間を最大限に生かして製造する製品など、資源を最大限に活用したい場合に組み合わせを考えるために用いられる

- 移動平均法\
時系列のデータを平滑化することで売上予測などに用いられる手法

- 最小二乗法\
散布図にプロットされた複数の点を基に数学的に回帰直線を導く方法
...
- A.**収集した情報を相互の関連によってグループ化し、解決すべき問題点を明確にする方法である**\
あるテーマに基づいて集めたデータを親和性によってグループ分けすることで項目を整理する手法

- 事態の進展とともに様々な事象が想定される問題について、対応策を検討し望ましい結果に至るプロセスを定める方法である\
PDPC法(*Process Decision Program Chart*)の説明
...
BPRと呼ばれる手法。業務プロセス・組織・規程など全ての要素をゼロベースで見直し、十分な検討により設定された将来のあるべき姿を目標とし、実現するためにそれらを最良のものに再構築する

- 業務改善を積み重ねるために、ビジネルモデルの将来像にはこだわらず、現場レベルのニーズや課題への対応を重視して業務プロセスを再設計する\
...
---
個人情報保護委員会"個人情報の保護に関する法律についてのガイドライン(通則編)平成28年11月(平成29年度3月一部改正)"によれば、個人情報に該当しないもの

- A.**匿名加工情報に加工された利用者アンケート情報**\
氏名・生年月日その他記述等により特定の個人を識別できる、個人識別符号が含まされるもののいずれかに該当する生存する個人に関する情報が個人情報に該当する\
匿名加工情報は、個人情報を区分に応じて定められた措置を講じて特定の個人を識別することができないように加工して得られる個人に関する情報で、復元して特定の個人を再識別できないようにしたもの。個人情報に関するルールは適用されず、一定の条件の下、本人の同意を取らなくても自由に再利用できる。これにより新事業の新サービスの創出や、国民生活の利便性の向上が期待される

- 受付に設置した監視カメラに録画された、本人が判別できる映像データ\
容貌や歩行の際の姿勢及び両腕の動作、歩幅その他の歩行の態様を収めた電子データは個人情報に該当する

- 個人番号の記載がない、社員に交付する源泉徴収票\
住所・氏名・勤務先の名称(住所)などが記載されているため個人を特定可能なので個人情報に該当する

- 指紋認証のための指紋データのバックアップデータ\
指紋、掌紋、静脈の形状を収めた電子データは個人情報に該当する
...
- A.**競合他社が真似できないような、自動車エンジンのアイドリングストップ技術**\
長年の企業活動により蓄積され、市場競争力の源であると同時に企業経営の核となる技術のこと。大きな経済的利益を生み出し、希少性があり、模倣されにくい3要素の特徴を有する技術。市場において持続的な優位性をもつことができる。企業が展開すべき方向性明確になるので、選択と集中の戦略により効率の良い投資効果を得やすくなるメリットがある

---
サプライチェーンマネジメントの改善指標となるもの

- **不良在庫の減少率**\
生産・在庫・購買・販売・物流など、全ての情報をリアルタイムに交換することでサプライチェーン全体の効率を大幅に向上させる手法。素材・部品・製品・物流・販売までの連鎖(サプライチェーン)を最適化することで無駄な在庫や生産を減らし効率化を図ることができる

---
サプライチェーンマネジメントを説明したもの

- A.**購買・生産・販売及び物流を結ぶ一連の業務を、企業内・企業間で全体最適の視点から見直し、納期短縮や在庫削減を図る**\
納期短縮や在庫削減などの効果が期待できる

- 個人が持っているノウハウや経験などの知的資産を組織全体で共有して、創造的な仕事につなげていく\
ナレッジマネジメントの説明

- 社員のスキルや行動特性を把握し、人事戦略の視点から適切な人員配置・評価などのマネジメントを行う\
HRM(*Human Resource Management* : 人的資源管理)の説明

- 多様なチャネルを通じて集められた顧客情報を一元化し、活用することによって、顧客との関係を密接にしていく\
...
---
アマゾフが提唱した成長マトリクスにおいて、既存市場に対して既存製品で事業拡大する場合の戦略

- A.**市場浸透**\
既存市場に対して既存製品で事業拡大する場合の戦略。市場と製品を縦と横軸にとり、それぞれに既存・新規の2区分を設け、4象限のマトリクスとしたもの。事業が成長・発展できる経営戦略を検討するために適したフレームワーク

- 市場開発\
新規市場に対して既存製品で事業拡大する場合の戦略

- 製品開発\
既存市場に対して新規製品で事業拡大する場合の戦略

- 多角化\
新規市場に対して新規製品で事業拡大する場合の戦略
...
---
デビットカード(*Debit Card*)の決済方式

- A.**利用金額を預金口座から即時に引き落とす**\
商品購入時に銀行などの預金口座から即時(あるいは数日後までに)引き落として支払うクレジットカードに似たカード。申込時の審査が無く、クレジットカードの取得困難な人も、口座開設と共に発行されるメリットがあるが、取引時に講座の暗証番号の入力が必要でありセキュリティ面での心配もある

- 後払い方式の決済を行う\
クレジットカードの決済方法

- カード内で残高処理を行い、財布のように利用できる\
電子マネーの決済方法
...
---
システム管理基準において、情報システムの費用、スケジュール、開発体制、投資効果などを明確にする計画

- A.**開発計画**\
情報戦略を立案し、効果的な情報システム投資とリスクを低減するためのコントロールを適切に整備・運用するための287項目をとりまとめてもの。開発計画は、目的・対象業務・費用・スケジュール・開発体制・投資効果等を明確にすることが定義されている
...
---
図は2つの会社の損益分岐点を示したものである。A社とB社の損益分析に関する記述
...
---
コストプラス法による価格設定方法を表すもの

- A.**製造原価、営業費を基準にし、希望マージンを織り込んで価格を決める**\
製品のコストに一定の利幅を加えて製品価格とするコスト指向型の価格設定法。直接費、間接費、利益によって価格が決定する

- 価格分析によって、利益最大、リスク最小を考慮し、段階的に価格を決める\
需要指向的価格決定法の説明

- 顧客に対する値引きを前提にし、当初からマージンを加えて価格を決める\
景品表示法の「希望小売価格を比較対照価格とする二重価格表示」に抵触するおそれがある
...
---
プロジェクトを準独立的な事業として遂行し、成果に対して全面的な責任を負う起業者としての権限と責任を与えられる組織構造

- A.**社内ベンチャー組織**\
新事業の開発のために、社内に自由にベンチャーを作らせ、小さな独立企業のように運営させる仕組みにより、下記のようなメリットがある\
...
---
企業経営で用いられるベンチマーキングの説明

- A.**経営目標設定の際のベストプラクティスを求めるために、最強の競合相手又は先進企業と比較して、製品、サービス及び実践方法を定性的・定量的に把握することである**\
自社の製品・サービス及びプロセスを定量的・定性的に測定し、業界で最も成功を収めている企業(ベスト企業)のものと比較しギャップを把握する企業経営で用いられる分析手法。分析により明らかになったギャップを埋めるためにベスト企業のベストプラクティスを参考に業務改善を進める
...
- A.**3,900**\
先に仕入れたものから順次払い出されたと想定して棚卸資産を評価する方法。2回ある払出を、在庫として古い方から払い出して最後に残った商品在庫の金額を計算する。5日の払出の30個は、先月繰越の10個と4日仕入の20個を足した分が払い出される。30日の払出の30個は、4日仕入の20個と7日仕入の10個が払い出される。月末時点での商品Aの在庫は、7日仕入の20個と10日仕入の10の合計30個\
$(20*140)+(10*110)=2,800+1,100=3,900$
...
---
あるメーカーがビールと清涼飲料水を生産する場合、表に示すように6種類のケース(A-F)によって異なるコストが掛かる。このメーカーの両製品の生産活動におけるスケールメリットとシナジー効果についての記述で適切なもの
...
- A.**スケールメリットはないが、シナジー効果はある**\
`スケールメリット(規模の経済性)` : 生産規模を拡大するほど生産性や経済効率が向上し、利益率が高くなる\
ビール : AよりBの方が大量に生産するが、製品当たりのコストは割高になっている\
清涼飲料水 : CよりDの方が大量に生産するが、製品当たりのコストは割高になっている => スケールメリットはない<br><br>
`シナジー効果(相乗効果)` : 2つ以上の要素が組み合わさることで、それぞれ単体で得られる効果の合計より大きな効果を得ること\
ケースE : ビール20万本 + 清涼飲料水10万本をそれぞれ単体で生産するケースA + ケースCのコストよりも割安となっている\
ケースF : ビール40万本 + 清涼飲料水20万本をそれぞれ単体で生産するケースB + ケースDのコストよりも割安となっている
...
---
刑法における、いわゆるコンピュータウイルスに関する罪となるもの

- A.**他人が作成したウイルスを発見し、後日これを第三者のコンピュータで動作させる目的で保管した**\
正当な理由がなく他人のコンピュータで実行させる目的で、ウイルスのプログラムやウイルスのソースコード等が記述された電磁的記録・媒体を作成・提供・保管した場合に罰せられる

- 自分に送られてきたウイルスに感染した電子メールを、それとは知らずに他者に転送した\
他人のコンピュータで実行させる目的ではないので罪にならない
...
---
コトラーの競争戦略によると、業界でのシェアは高くないが、特定の製品・サービスに経営資源を集中することで収益性を高め、独自の地位を獲得することを戦略目標とする企業

- A.**マーケットニッチャ**\
小さいながらの特定の市場で、独自の地位を築いている企業。潜在的な需要がありながら、大手企業が参入しないような専門特化した市場に、限られた経営資源を集中するニッチ戦略を採る

- マーケットチャレンジャ\
リーダに次ぐシェアを保持し、競争を仕掛ける2・3番手の企業。上位企業の市場シェアを奪うことを目標に、製品・サービス・販促・流通チャネルなどの方面での差別化戦略を採る

- マーケットフォロワ\
リーダやチャレンジャの戦略を模倣することで、市場での地位を維持している企業。目標とする企業の戦略を観察し、迅速に模倣することで、開発や広告のコストを抑制し、市場での存続を図るコスト・リーダシップ戦略を採る

- マーケットリーダ\
市場においてナンバー1のシェアを誇る企業。利潤、名声の維持向上と最適市場シェアの確保を目標として、市場内の全ての顧客をターゲットとした全方位戦略を採る

---
特定顧客、特定製品のセグメントに資源を集中し、専門化を図る戦略

- A.**ニッチ戦略**\
特定のニーズ(需要)を持つ規模の小さい市場(ニッチ市場、隙間市場)をターゲットとし、新たなニーズの発掘や市場に特化・集中した製品・サービスの提供などを行う販売戦略

- リーダ戦略\
市場において1番のシェアを誇る企業が採る戦略で、消費者へ新商品や使い方を提案し、市場規模の拡大を図り品揃えを拡充しシェアを維持・拡大する戦略

- フォロワー戦略\
市場シェアが3, 4番手以降の企業が採る戦略で、市場における自社の実力を見極め、ヒット商品を模倣するなどリーダ企業に追従することでシェアよりも安定的な利益確保を優先する戦略

- チャレンジャ戦略\
リーダに次ぐシェアを保持し、競争を仕掛ける企業が採る戦略で、製品やサービスなどのあらゆる面で徹底した差別化を図ることで、他企業からの売上・顧客を奪い、市場シェア拡大を目指す戦略
...
---
派遣元会社A社と派遣先会社B社が派遣契約を結び、A社は社員であるN氏を派遣した。労働者派遣法に照らして適切な行為

- A.**N氏は派遣期間中の仕事に関する指示を、B社の担当者から直接受けることにした**\
派遣契約では、派遣元企業の従業員が派遣先企業の責任者の下に業務を行う。労働者派遣法では、対象業務の範囲、二重派遣の禁止、派遣元事業主や派遣先事業主の講ずべき措置などについて定められている

- B社の繁忙期とN氏の休暇申請が重なったので、B社から直接N氏に休暇の変更を指示した\
有給休暇は原則として派遣労働者の希望日に与えなければならないが、業務を妨げるような場合は磁気を変更する権利が派遣元企業に認められている。派遣先企業には時期変更権は認められていない

- N氏からの作業環境に関する苦情に対し、B社は雇用関係にないので、対応はA社だけで行った\
苦情の申出を受けた時は、派遣先企業が苦情の処理を図る必要がある

- 派遣期間中にN氏の作業時間が空いたので、B社は派遣取決め以外の作業を依頼した\
労働者派遣契約には、従事する業務の種類を定めることになっている。派遣先企業ではこの定めに反しないようにしなければならない
...
労働基準法などの各種労働法が適用される。週40時間、1日8時間の法定労働時間が定められ、これを超える労働を行うには労使間で36協定を締結し、所轄の労働基準監督署長に届出を行う必要がある。派遣労働者と雇用関係にあるのは派遣元事業者なので、派遣労働者が派遣先で時間外労働を行うためには、労働者派遣契約に時間外労働や休日労働の条項を記載し、派遣元事業者が派遣労働者と36協定を締結し届出を行う必要がある

---
労働者派遣法に基づいた労働者の派遣において、労働者派遣契約関係が存在する関係
...
1人(少人数)の作業者チームで製品の組み立て工程を完成(または検査)まで行う。作業者の周囲に機械、組付工具、部品が配置され、多能工が歩き回り作業をする。ライン生産方式と比較して、作業者1人が受け持つ範囲が広く多品種を生産する時に切り替えが容易なのが特徴

- 生産性を上げるために、大量生産が必要なもの
- 製品の仕様が長期間変わらないもの\
大量生産や仕様が長期間変わらない製品はライン生産方式が適している

- 標準化、単純化、専門化による分業が必要なもの\
作業工程ごとの専門家を必要とする場面には向いていない
...
- 著作権は、プログラムには認められるが、DBについては認められていない\
...
---
ASP(*Application Service Provider*)とは、どのようなサービスを提供する事業者か

- A.**汎用的なアプリケーションシステムの機能をネットワーク経由で複数の顧客に提供するサービス**\
インターネットを通じて、主に業務用のソフトウェアやアプリケーションの利用環境を顧客に提供する事業者。SaaSはクラウドコンピューティングの一形態を表すのに対し、ASPはSaaSを提供する事業者という違いがある。サービス利用側は、インターネット上のWebサービスベースのソフトウェアを利用する形なので、自社内にソフトウェアを導入したり、ソフトウェアのメンテナンスを行ったりする必要がなく、購入や新規開発よりもコストを抑えられることがメリット
...
- A.**観測回数・観測時刻を設定し、実地観測による観測点数の比率から、統計的理論に基づいて作業時間を見積もる**\
ある時点での観測対象が、作業内容のどの状態にあったかという瞬間観測を何度か行い、観察記録の回数の割合から、各作業時間がどのくらいかを推定する

- 作業動作を基本動作にまで分解して、基本動作の時間基準テーブルから、構成される基本動作の時間を合計して作業時間を求める\
PTS法(規定時間標準法)の説明
...
- A.**ある時点での観測対象が、作業内容のどの状態にあったかという瞬間観測を何回か行い、観察記録の回数の割合から、各作業時間がどのくらいであるかを推定する**\
瞬間観測法とも呼ばれ、観測者が計画に従って決められた時間に対象となる作業者は設備を巡回して稼働状況を記録を繰り返し、集まった観測結果を分析することで稼働時間や作業時間を推定する手法
...
---
MRP(*Materials Requirements Planning* : 資材所要量計画)の特徴

- A.**製品の基準生産計画を基に、部品の手配数量を算出する**\
部品表と生産計画をもとに必要な資材所要量を求め、在庫、発注、納入の管理を支援する計画方式、またはそれを実現するシステム。製品に組み込まれる部品の構成や、それぞれの部品毎の使用量、製造リードタイム、発注リードタイムをもとに、全ての製品の需要に対応した個々の部品の必要量、製造指示日、発注日時を決めるように、製品の生産に必要な部品の発注量をコンピュータで算出することを目的としている

- 顧客の注文を受けてから製品の生産を行う\
BTO(*Build to Order* : 受注生産)の特徴
...
---
共通フレーム2007によれば、システム化計画を立案する時に考慮すべき事項

- A.**情報システムの有効性及び投資効果を明確にする**\
ソフトウェア産業界においての共通の物差しとなることを目的として作成された規格。取得者と供給者双方又はシステム開発に関わる全ての人がソフトウェアを中心としたシステムの企画・要件定義・開発・運用・保守の作業内容を共通に参照できるよう詳細に表現したり、ソフトウェア取引を明確化するための基準が明記されている。システム化計画の立案は企画プロセスに含まれ、基本要件・業務要件・機能要件の定義や業務モデルの作成・経営戦略との整合性を確認する作業群

---
システム化計画を立案する時に考慮すべき事項

- A.**投資、保守、運用に関する費用と投資効果を明確にする**\
企画プロセスを構成するアクティビティの1つで、業務モデルの作成・費用対効果の予測・プロジェクトの目標設定・プロジェクト計画の作成と承認など18のタスクがある。システム実現時に定量的・定性的効果予測を行い、開発・運用・保守に関する期間・体制・工数の大枠を予測し、システム実現のための費用を見積もる。費用と効果を対比させ、システムへの投資効果と時期などを明確にする
...
---
事業年度初日の平成21年4月1日に、事務所用のエアコンを60万円で購入した。平成24年3月31日現在の帳簿価額は何万円か。耐用年数は5年、減価償却は定額法、定額法の償却率は0.200、残存価格は0円とする
...
---
事業年度初日の平成21年4月1日に、事務所用のエアコンを100万円で購入した。平成23年3月31日現在の帳簿価額は何万円か。耐用年数は6年、減価償却は定額法、定額法の償却率は0.167、残存価格は0円とする

- A.**666,000**\
毎年一定の額を償却していく。取得原価が100万円、耐用年数6年、残存価格0円なので取得原価に償却率を乗じて、\
$1,000,000*0.167=167,000円$\
現在の帳簿価額は取得原価から2年分を償却した額なので、\
$100万円-(16.7万円*2年=33.4)=66.6万円$
...
- A.**商品毎の販売金額や粗利益額から、売れ筋商品を把握したい**\
商品在庫や売上高などをパレート図で重点管理する手法。分析する要素・項目群を大きい順に並べ、上位70%を占める要素群をA、70 - 90%の要素群をB、それ以外の要素群をCとしてグルーピングすることで重点的に管理すべきグループがどれかを明らかにする。この分析結果をもとにして限りある経営資源を効率的に配分することが可能

---
不良品の個数を製品別に集計すると表のようになった。ABC分析を行い、A群の製品に対策を講じることにした。A群の製品は何種類か。A群は70%以上とする
...
- A.**5**\
重要度や優先度の高い要素・項目を明らかにするために行われる分析手法。個数合計の70%以上の閾値となる個数を求める\
$875*0.7 \fallingdotseq 600$\
$182+136=318$\
$318+120=438$\
$438+98=536$\
$536+91=627$
...
---
取扱商品をABC分析した場合、Aグループの管理対象となる商品の商品番号

<img width="300" alt="" src="./images/ABC分析.png">

- A.**2と5**\
重要度や優先度の高い要素・項目を明らかにするために行われる。パレート図を使って分析する要素・項目群を大きい順に並べ、上位70%を占める要素群をA、20%をB、それ以外をCとしてグルーピングする。売上高の多い取扱商品順に足し合わせると上位2点が該当する

---
QCにおける管理図を説明したもの

- A.**中央線と上下一対の境界線を引いて、製品などの特性値をプロットし、品質不良や製造工程の異常を検出して不良原因の除去や再発防止に役立てる**\
工程の状態や品質を時系列に表した図で、工程が安定した状態か否かを判断するために用いる。不良品の個数を記録し、統計的に求めた上方管理限界と下方管理限界を超えた位置に値が記録された場合、その日に何らかの異常が発生していると判断できる
...
- A.**時系列的に発生するデータのばらつきを折れ線グラフで表し、上限と下限を設定して異常の発見に用いる**\
中央線・上限・下限を示す境界線を引いて、製品等の特性値を打点することで、工程の状態や品質を時系列に表した図で、工程が安定した状態にあるかどうかを判断するために使用される
...
---
ある企業が、AIなどの情報技術を利用した自動応答システムを導入して、コールセンターにおける顧客対応を無人化しようとしている。この企業が、システム化構想の立案プロセスで行うべきこと

- A.**AIなどの情報技術の動向を調査し、顧客対応における省力化と品質向上など、競争優位を生み出すための情報技術の利用方法について分析する**\
企画プロセスを構成するプロセスの1つで、経営上のニーズ、課題を実現・解決するために、置かれた経営環境を踏まえて、新たな業務の全体像とそれを実現するためのシステム化構想・推進体制を得ることを目的とするプロセス。経営上のニーズ、課題の確認・事業環境、業務環境の調査分析・原稿業務、システムの調査分析・情報技術動向の調査分析・対象業務の明確化・業務の新全体像の作成・対象選定と投資目標の策定の7タスクが含まれる
...
- 自動応答を実現するソフトウェア製品又はシステムの要件定義を行い、AIなどを利用した実現方式やインターフェイス設計を行う\
システム(ソフトウェア)要件定義の作業

---
ICタグ(*Radio Frequency IDentification* : RFID)の特徴

- A.**汚れに強く、記録された情報を梱包の外から読むことができる**\
ID情報を埋め込んだRFタグ(ICタグ)から電磁界や電波を用いて情報のやり取りを行う技術。非接触型ICカードや電子マネー、無人レジ、在庫管理などに活用されている。バーコードやQRコードと比較し、見えなくても読むことができ、RFタグが目に見えない隠れた位置にあっても、タグ表面がホコリや泥で汚れていても読み取りが可能

- GPSを利用し、現在地の位置情報や属性情報を表示する\
GPSは未利用

- 専用の磁気読取り装置に挿入して使用する\
ICタグとRFIDリーダとの間で非接触型の無線通信で情報をやり取りするため、読取り機器に挿入する必要はない

- 大量の情報を扱うので、情報の記憶には外部記憶装置を使用する\
情報は内部のIC(集積回路)に記憶しておく

---
インターネットで利用される技術の標準化を図り、技術仕様をRFCとして策定している組織

- A.**IETF**(*Internet Engineering Task Force*)\
TCP / IP・HTTP・SMTPなどのインターネット上で開発される技術やプロトコルなどを標準化する組織。標準化が行われた規格はRFCとしてインターネット上に公開され自由に閲覧できる

- ANSI(*American National Standards Institute*)\
アメリカ合衆国の工業的な分野の標準化組織であり公の合意形成のために、様々な規格開発を行う団体

- IEEE\
電気・電子技術に関わる非営利の団体で、主に学会活動、書籍発行、IEEE規格の標準化を行っている

- NIST(アメリカ国立標準技術研究所)\
アメリカの技術革新や産業競争力を強化するために、経済保障を強化して生活の質を高めるように計測学・規格・産業技術を促進することを目的とする機関

---
日本産業標準調査会(*Japanese Industrial Standards Committee*)を説明したもの

- A.**経済産業省に設置されている審議会で、産業標準化法に基づいて産業標準化に関する調査・審議を行っており、JISの制定、改正などに関する審議を行っている**\
産業標準化法に基づいて設置され、日本産業規格(JIS)の制定・改正・確認・廃止に関する審議などを行う機関。鉱工業品、プログラムその他の電磁的記録、建築物、役務等が標準化対象。HPではJIS規格がPDF形式で閲覧可能

- 電気・電子技術に関する非営利の団体であり、主な活動内容としては、学会活動、書籍の発行、IEEE規格の標準化を行っている\
IEEE(アイ・トリプルイー)の説明

- 電気機械器具・材料などの標準化に関する事項を調査審議し、JEC規格の制定及び普及の事業を行っている\
電気規格調査会(*Japanese Electrotechnical Committee*)の説明
