...
---
ITサービスマネジメント活動のうち、インシデント管理及びサービス要求管理として行うもの

- A.**利用者からの障害報告を受けて、既知の誤りに該当するかどうかを照合する**\
既知のエラーであれば、前回発生時の対応策を採ることで迅速な障害回復が可能\
サービスに対する計画外の中断、サービスの品質低下、顧客へのサービスにまだ影響していない事象が報告された時に、サービスの中断時間を最小限に抑え速やかに回復することを目指すプロセス。ITILではインシデント管理が独立したプロセスとして定義されているが、インシデントとサービスの要求の両方を合わせた、インシデント及びサービス要求管理として定義されているものもある
...
---
情報システムのセキュリティコントロールを予防・検知・復旧の3つに分けた場合、復旧に該当するもの

- A.**コンティンジェンシープラン**\
復旧(回復)の機能。障害からの復旧、回復手順の明確化、不測事態への対応訓練など。システムが抱える潜在的なリスクが現実化した際に、組織やシステムが被る損害を最小限に留め、速やかに緊急事態の克服を図るための手続きが記述された対応計画のこと

- オペレータとプログラマの職務分離
- パスワードの利用\
予防(防止)の機能。アクセス制御、データの暗号化、入退室管理、パッチの適用など

- メッセージ認証\
検知の機能。アクセスログの記録、コンピュータウイルスの常時検査、システムの稼働状況の監視など
...
---
全部で100画面から構成されるシステムの画面作成作業において、規模が小かつ複雑度が単純な画面が30、中規模かつ普通の画面が40、大規模かつ普通の画面が20、大規模かつ複雑な画面が10である場合の工数を、表の標準作業日数を用いて標準タスク法で見積もると何人日になるか。全画面のレビューに5人日を要し、作業の管理にレビューを含めた作業工数の20%を要するものとする
...
規模大・普通の画面 : $20*1.0=20$\
規模大・複雑な画面 : $10*1.2=12$\
それぞれの和は80、画面のレビューを加味して85、作業管理のレビューを加味して、 $85*1.2=102$
...
利用時の職務権限、利用権限を定めるのは、データ改ざんや消失を防ぐためのコントロール
...
---
JIS Q 27001:2014(情報セキュリティマネジメントシステム-要求事項)に基づいてISMS内部監査を行った結果として判明した状況のうち、監査人が指摘事項として監査報告書に記載すべきもの

- A.**リスクアセスメントを実施した後に、リスク受容基準を決めた**\
リスク需要基準の決定時期が遅いので指摘事項に該当する。リスクマネジメントプロセスでは、リスクアセスメントを実施する前にリスク需要基準を確立することになっている。リスクアセスメントに含まれるリスク評価プロセスにおいて、リスク分析の結果とリスク需要基準を比較することになっている

<img width="500" alt="" src="./images/リスクアセスメント.png">

- USBメモリの使用を、定められた手順に従って許可していた\
資産の取り扱いに関する手順は、組織が採用した情報分類体系に従って策定し実施することとされているので問題ない

- 個人情報の誤廃棄事故を主務官庁などに、規定されたとおりに報告していた\
関係当局との適切な連絡体制を維持し、法が破られたと疑われる場合に、特定した情報セキュリティインシデントをいかにして時機を失せずに報告するかの手順を備えることとされているので問題ない

- マルウェアスキャンでスパイウェアが検知され、駆除されていた\
不適合が発生した場合、是正のための処置を取ることとされており、問題ない
...
---
工程管理図表に関する記述のうち、ガントチャート(*Gantt chart*)の特徴

- A.**作業開始と作業終了の予定と実績や、作業中の項目などが把握できる**\
縦軸に作業項目、横軸に時間をとり、作業に必要な期間を横棒の長さで表記する手法。作業項目同士の日程的な重なりが視覚的に把握しやすいので日程計画の策定に使用されたり、計画と実績を並べて表記することで進捗度を管理する方法として使用されたりする
...
EVM(アーンドバリューマネジメント)やトレンドチャートの特徴

---
プロジェクトで発生した課題の傾向を分析するために、ステークホルダ、コスト、スケジュール、品質などの管理項目別の課題件数を棒グラフとして件数が多い順に並べ、この順で累積した課題件数を折れ線グラフとして重ね合わせた図を作成した図

- A.**パレート図**\
値の大きい順に分析対象の項目を並べた棒グラフと、累積構成比を表す折れ線グラフを組み合わせた複合グラフで、主に複数の分析対象の中から管理対象とすべき重要な要素を識別するために使用される

- 管理図\
中央線と上限・下限を示す境界線を引いて、製品などの特性値を打点することで、工程の状態や品質を時系列に表した図。工程が安定した状態にあるかを判断するために用いる

- 散布図\
横軸縦軸に2項目の量や大きさ等を対応させて、分析対象のデータを打点した図。2項目間の分布・相関関係を把握するのに使用される

- 特性要因図\
現れた特性(結果)と影響を及ぼしたと思われる要因の関係を体系的に表した図。多くの要因が複雑に絡み合っている時に、直接的な原因と間接的な原因に分別したり、真の問題点を明らかにしたりすることができる
...
---
サービスマネジメントにおいて、サービスレベル管理の要求事項

- A.**提供するサービスのサービスカタログとSLAを作成し、顧客と合意する**\
顧客とサービス提供者の間でSLAを締結し、合意したサービスレベルを維持管理する一連の活動。締結後は、PDCAサイクルによってサービスの維持向上に努める。JIS Q 20000-1で定義されるサービスレベル管理の役割は下記5点\
提供するサービスについての合意\
サービスカタログ(品目)についての合意\
SLAの作成及び合意\
サービスとSLAの定期的なレビュー\
傾向とパフォーマンスの定期的な監視・記録
...
16人 / 0.8 = A.**20**\
現在各月の作業人員は10名なので各月10人を追加する必要がある
...
---
金融庁"財務報告に係る内部統制の評価及び監査の基準(令和5年)"に基づいて、内部統制の基本的要素を、統制環境、リスクの評価と対応、統制活動、情報と伝達、モニタリング、ITへの対応の6つに分類したときに、統制活動に該当するもの

- A.**発注業務と検収業務をそれぞれ別の者に担当させること**\
`統制活動` : 経営者の命令及び指示が適切に実行されることを確保するために定められる方針及び手続きのこと。権限・職責の付与、職務の分掌等の広範な方針・手続きが含まれる<br><br>
`情報と伝達` : 必要な情報が識別・把握・処理され、組織内外及び関係者相互に正しく伝えられることを確保すること\
`ITへの対応` : 組織目標を達成するために予め適切な方針及び手続きを決め、業務の実施において、組織の内外のITに対し適切に対応すること

- 経営者が自らの意思としての経営方針を全社的に明示していること\
`統制環境` : 組織の気風を決定し、組織内の全ての者の統制に対する意識に影響を与え、他の基本的要素の基礎・基盤のこと

- 情報システムの故障・不具合に備えて保険契約に加入しておくこと\
`リスクの評価と対応` : 組織目標の達成に影響を与える事象について、組織目標の達成を阻害する要因をリスクとして識別、分析・評価するプロセス
...
- A.**情報システム部門は、データを入力する利用部門からの独立を保ち、利用部門がデータの正確性を維持できるようにする**\
不正や誤謬の発生を防ぐため、開発と運よの責任を分離して、相互牽制が働く仕組みとなっている\
内部統制では、ITを取り入れた情報システムに関する統制(IT統制)を基本的要素の1つとしており、目標としては会計上の取引記録の正当性、完全性及び正確性を画することがある。インテグリティ(*Integrity* : 完全性)は記録した取引や漏れに重複がないことを呼ぶ

- 業務ニーズにそった効率の良いデータ入力システムを実現するため、情報システム部門がデータ入力システムを開発してデータ入力する\
システムの開発と運用が1つの部門によって行われているため、相互牽制の仕組みになっていない。相互牽制が働かない体制では、不正や誤謬の行為が発生するリスクが大きくなり、データの完全性が脅かされる

- 情報システム部門の担当者は、その経験を生かし、システム開発においてデータの整合性が保てるように、長期間、同一部署に配置する\
同一人物が長期間同一部署に存在すると、ある業務が特定の者に一身専属的に属することとなり、組織としての継続的な対応が困難となる等の問題点がある。不正や誤謬の温床となることがあるため、データの完全性を確保する観点から不適切

- 情報システム部門の要員が他部門に異動する場合は、関連する資料をもたせ、システムトラブルなどの緊急時に戦力となるようにする\
職務の分掌を明確化し、権限や職責を担当者に適切に分担させていることが肝要。情報システム部門の職務であるシステムトラブルへの対応を他部門の要員にさせることは、職務の分掌の観点から不適切
...
- A.**外部入出力や内部論理ファイル、照会、インターフェイスなどの個数や特性などから開発規模を見積もる**\
ソフトウェアに含まれる機能と複雑さを基準に論理的に開発工数を見積もる方法。外部入出力やファイル、外部インターフェイスなどの数と複雑さなどの因子からファンクションポイント数を算出し、それを基にして開発工数を見積もる

- 開発するプロジェクト毎のステップ数を積算し、開発規模を見積もる\
プログラムステップ法の説明
...
---
ファンクションポイント法で、システムの開発規模を見積もる際に必要となる情報

- A.**画面数**\
外部入出力や内部ファイルの数と難易度の高さから論理的にファンクションポイントを算出し、開発規模を見積もる手法。画面数、帳票の数、ファイル数などを基準に見積もるので、依頼者からのコンセンサスが得やすい長所がある。画面数は、システムのユーザインターフェイス上で必要となる画面数のこと

---
システムを構成するプログラムの本数とプログラム1本当たりのコーディング所要工数が下記の時、システムを95日間で開発するには少なくとも何人の要因が必要か。システムの開発にはコーディングの他に、設計及びテストの作業が必要であり、それらの作業にはコーディングの所要工数の8倍の工数が掛かるものとする

||プログラムの本数|プログラム1本あたりのコーディング所要日数(人日)|
|-|-|-|
|入力処理|20|1|
|出力処理|10|3|
|計算処理|5|9|
...
- A.**9人**\
実際の所要工数は、コーディングに要する工数+(コーディングに要する工数*8)\
実際の所要工数は、プログラム1本当たり、\
入力処理(9人日)、出力処理(27人日)、計算処理(81人日)\
総工数は、 $(20*9)+(10*27)+(5*81)=855(人日)$\
95日で割ると、 $855/95=9人$
...
---
プロジェクトチームが実行すべき作業を上位から下位へ段階的に分解したもの

<img width="400" alt="" src="./images/WBS.png">

- A.**WBS(*Work Breakdown Structure*)**\
プロジェクト目標を達成し、必要な成果物を過不足なく作成するために、プロジェクトチームが実行すべき作業を、成果物を主体に階層的に要素分解したもの。作業漏れや抜けを防ぎ、プロジェクトの範囲を明確にすると同時に、作業単位毎に内容・日程・目標を設定することでコントロールしやすくなる目的がある

- CPM(*Critical Path Method*)\
クリティカルパスを求めることで、プロジェクト完了までの最短時間を決定したり、遅延が許されないタスクの繋がりをはっきりさせることができる

- EVM(*Earned Value Management*)\
プロジェクトにおける作業を金銭の価値に置き換えて実績管理する進捗管理手法

- PERT(*Program Evaluation and Review Technique*)\
アローダイアグラムと同義で対象とするプロジェクトの完遂に必要なタスクを図で分析する手法

---
ソフトウェア開発プロジェクトにおいてWBS(*Work Breakdown Structure*)を使用する目的

- A.**作業を断層的に詳細化して、管理可能な大きさに細分化する**
...
- 作業の日程を横棒で表して、作業の開始や終了地点、現時点の進捗を明確にする\
ガントチャートを使用する目的
...
---
プロジェクトのコンティンジェンシー計画において決定すること

- A.**予め定義された、ある条件の時にだけ実行する対応策**\
プロジェクトが抱える潜在的なリスクが現実化した際に、プロジェクトが被る損害を最小限に留め、速やかに緊急事態の克服を図るための手続きが記述された対応計画のこと。既知のリスクに対する対応計画の一部として策定され、ある事象が起きた場合に限り使用するので、発動条件となる事象とその時実行する対応策を記述する
...
監査手続の実施内容や発見した問題点、収集した情報や分析結果、監査人の結論などをまとめた文書。監査報告書に記載される監査結果、監査意見、改善勧告などの根拠となる
...
---
システム監査で実施するヒアリングに関する記述

- A.**ヒアリングで被監査部門から得た情報を裏付けるための文書や記録を入手するよう努める**\
監査人が被監査部門の従業員に対して実施する聞き取り調査。情報収集や問題発見に有効な手段だが、見聞きしただけの情報では監査根拠として不十分。ヒアリングを通して問題と思われる事項を発見した監査人は、裏付けとなる記録の入手や現場確認を行うことが望まれる

- 監査対象業務に精通した被監査部門の管理者の中からヒアリングの対象者を選ぶ\
管理者のみを対象とするのではなく、日常の業務を行っている担当者からもヒアリングを行うことが望ましい

- ヒアリングの中で気が付いた不備事項について、その場で被監査部門に改善を指示する\
改善勧告は、その場で指摘するのではなく監査終了後に作成するシステム監査報告書にまとめて記載する

- 複数人でヒアリングを行うと記録内容に相違が出ることがあるので、1人のシステム監査人が行う\
複数人で行うことで監査人ごとの見解のばらつきを抑える利点があるため、1人行うことは必須ではない
...
- エディタ、コンパイラ、デバッガなどプログラミングに必要なツールが1つに統合され、1つのインタフェースで扱えるようになっている開発環境である\
...
- A.**インベントリ(*inventory*)収集**\
企業などで使用されている情報機器の資産情報のこと。企業のPC、インストールされているソフトウェア、プリンタ、CPU、メモリなどは全て該当する。インベントリ収集ツールは、利用者側のPCを個別操作をせずに、PCの情報を得ることができるソフトウェア。ソフトウェアのバージョンも収集できるため、ソフトウェアアップデートが行われず脆弱性が放置されているPCなども洗い出すことができる

---
DBシステムと業務アプリケーションが稼働しているサーバのOSのバージョンアップの案内が届いた。バージョンアップを行うか否かの判断で適切なもの

- A.**バージョンアップされたOSでのDBシステムの稼働を確認した後に、業務アプリケーションの稼働を確認し、問題がなければバージョンアップをする**\
新環境に移行する前に、現在稼働している・ソフトウェアの動作を確認することが重要

- 業務アプリケーションは長期間使用しているが、DBシステムは比較的新しいので、OSとDBシステムの相性をチェックしバージョンアップをする\
新OSで長期間使用している業務アプリケーションの動作確認が取れなければ、バージョンアップした後の障害に繋がる

- 今回のバージョンアップに伴い現在使用しているOSはサポート終了となるので、すぐにバージョンアップをする\
現在稼働しているシステムが正常に稼働していれば、急いでバージョンアップする必要はない。現システムと新OSの相性を確認しながら段階的に移行する判断

- DBシステムは、OSのメーカーが提供するDB管理機能を使っているのでトラブルはないと判断し、業務アプリケーションとOSの関係を調査し、問題がなければバージョンアップをする\
OSがバージョンアップする時に従来の機能について仕様変更されることがある。旧OSで稼働したシステムでも、新OSで全ての機能が正常に動作する保証はない
...
- A.**バックアップしたデータの整合性を保証するために、バックアップ処理と業務処理が重ならないようにスケジューリングする**\
就業 - 始業までの時間にバッチ処理で行われることが推奨される

- バックアップからの復旧時間を最短にするために、差分バックアップ方式を採用する\
復旧時間を最短にするには、毎回フルバックアップ方式を取る方法が有効。ただしバックアップの作成に要する時間は差分バックアップ方式よりも長くなる

- バックアップからの復旧処理でランダムアクセスを可能にするために、磁気テープにバックアップする\
磁気テープはランダムアクセスができない

- バックアップ処理の時間を最短にするために、同一記憶媒体内にバックアップする\
バックアップ内容を複数の記憶媒体に分割し同時に書き込みを行った方が、バックアップに要する時間は短くなる
...
- A.**保守作業が完了しないまま放置されるのを防ぐためにも、保守の完了を記録する**

- テスト終了後は速やかに本稼働中のライブラリにプログラムを登録し、保守承認者に報告する\
本家用環境への移行作業は、保守承認者の承認を得た後に実施する

- 変更内容が簡単であると判断できるときは、本稼働用のライブラリを直接更新する\
移行中または移行後に予想外の事象が起こり本稼働環境に影響を与える可能性ある。本稼働環境に移す前にはテストを必ず実施すべき
...
---
図のアローダイアグラムから読み取れることで適切なものはどれか。プロジェクトの開始日を1日目とする

<img width="500" alt="" src="./images/アローダイアグラム4.png">

- A.**作業Eの総余裕日数は30日である**\
クリティカルパスは、B, C, G, Hの**60日**。完了前に行われる作業Hはプロジェクト開始から51日目までには作業を開始しなければならない。作業Hの開始条件には作業Eの完了も含まれるので、作業Eは開始から50日目までに完了していればクリティカルパスに影響を与えない。最短で開始できるのは11日目、再遅の場合は41日目までに開始していればよいので、その差の30日

- 作業Cを最も早く開始できるのは6日目である\
先行作業は作業AとB。作業Bが完了するまでに10日を要するので、作業Cを最も早く開始できるのは11日目

- 作業Dはクリティカルパス上の作業である\
作業Dは栗ティルパス上の作業ではない

- 作業Fを最も遅く開始できるのは11日目である\
11日目は最早開始日。再遅の場合は41日目
...
---
情報システムの障害対策の1つである縮退運用(*fallback*)の説明

- A.**障害箇所を切り離し、機能又は性能が低下してもシステムを稼働させ続けること**\
システムの一部に障害が発生した時に、障害部分を切り離し、処理速度や一部機能の制限をしてもシステム全体としては停止することなく稼働させ続ける方式。2台のサーバで構成される負荷分散システムにおいて、片方のサーバが故障した時に、処理能力の低下を許容して残った片方のみで処理を続けることが該当する
...
- A.**生産性**\
開発工数は、`開発規模 / 生産性`で計算されるので、見積もりには単位時間当たりの生産量(生産性)が必要な情報
...
---
システムに関わるドキュメントが漏洩・改ざん・不正利用されるリスクに対するコントロールを監査する際のチェックポイント

- A.**ドキュメントの機密性を確保するための対策を講じていること**\
機密性は、許可された正規のユーザのみが情報にアクセスできる度合いを示す。ドキュメントの漏洩・改ざん・不正利用に対しては暗号化やアクセス制御、物理的な隔離によって正規の利用者以外からのアクセスを遮断する対策が有効
...
---
ウォーターフォール型のソフトウェア開発において、運用テストで発見された誤りの修復に要するコストに関する記述

- A.**外部設計の誤りは、プログラムだけでなく、マニュアルなどにも影響を与えるので、コーディングの誤りに比べて修復コストは高い**\
開発プロジェクトを時系列、要件定義・外部設計(概要設計)・内部設計(詳細設計)・開発(プログラミング)・テスト・運用などの作業工程に分割し、原則前工程が完了しないと次に進まない(並行しない)ことで、前工程の成果物の品質を確保し、前工程への後戻りを最小限にする開発モデル。大規模プロジェクトにおいて進捗を管理しやすいメリットがあるが、上流工程での誤りを発見する手順が組み込まれていないこともあり、上流工程での誤りは下流工程にいくにつれて大きなものになる

- テストケースの誤りは、テストケースの修正とテストのやり直しだけでは済まないので、外部設計の誤りに比べて修復コストは高い\
テストケースの作成は、外部設計・内部設計よりも下流工程のため、修復コストは低くなる

- 内部設計の誤りは、設計レビューによってほとんど除去できるので、もし発見されでも、コーディングの誤りに比べて修復コストは安い
- コーディングの誤りは、修復のための作業範囲がその後の全工程に及ぶので、要求定義の誤りに比べて修復コストは高い\
コーディングの誤りであれば前工程の設計工程への影響はないが、要件定義の誤りはその後の全工程に影響するので、修復コストは要求定義での誤りの方が高くなる
