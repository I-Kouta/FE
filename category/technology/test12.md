...
システム開発の各設計段階において、設計仕様書やプロトタイプの内容を確認して問題点や改善点を発見するための活動。システムに実装される機能、システム方式、インタフェース、UI、セキュリティなどが適切に設計されているかを審査する。システムの問題点を早い段階で発見することで、後工程の作業をスムーズにし、システムの最終的な品質を高めること
...
- A.**再利用可能な部品の開発は、同一規模の通常のソフトウェアを開発する場合よりも工数がかかる**  
再利用可能な部品の開発では、パラメータやデータ個数を可変にしたり様々な場面を想定してエラー処理を組み込むなど再利用を前提とした設計を行う必要があるため、同規模のソフトウェアを開発するのと比較して工数がかかる。新しくソースコードを書くことは同時にバグとセキュリティ脆弱性が入るよりを作ることもある。検証済みで信頼できる既存のコードなら再利用した方が工数をかけずに高い信頼性を確保できる。コードの再利用は問題を起こしにくいソフトウェアを構築する上で重要な手法だが、当初から部品かを考慮に入れた開発手法が必要

- 大きい部品を再利用するよりも、小さい部品を再利用する方が、単位規模当たりの開発工数の削減効果は大きい  
小さな部品を組み合わせるのにも工数がかかるため、同じ機能を実現できるのであれば大きな部品1つを使用した方が工数削減を期待できる

- 部品の再利用を促進するための表彰制度などのインセンティブの効果は、初期においては低いが、時間の経過とともに高くなる  
インセンティブの効果は初期が最も高くなる

- 部品を再利用したときに削減できる工数の比率は、部品の大きさに反比例する  
一般に部品の大きさに比例した工数の削減が期待できる

---
51.オブジェクト指向における、開かれた(ホワイトボックス型)再利用とは、基底クラスに対して、サブクラスを作ることによって、基底クラスのデータや機能を再利用することである。この方式のオブジェクト指向の再利用技術に関する記述

- A.**基底クラスで定義したデータや機能に対する差異をサブクラスに記述すればよく、開発効率が良い**  
差分プログラミングでは基底クラスと共通した部分の定義は不要で、付加する部分のみの定義だけで良いのでプログラミング効率が向上する

- 基底クラスで定義したデータが保護されるので、安全性の高いプログラムが開発できる  
サブクラスでは基底クラスのデータを再定義して仕様を追加できる

- 基底クラスの変更は、サブクラスに影響しない  
基底クラスの変更は全てのサブクラスに影響する

- 基底クラスを複数のアプリケーション開発に利用することはできるが、そのサブクラスを再利用することはできない  
サブクラスを基底クラスとしてさらにそのサブクラスを作成したり再利用することができる

---
55.プロセス中心設計と比較した時、データ中心設計の特徴

- A.**データを共有資源と見なし、一元的に管理できる**  
データが業務内容が変化しても変わらない最も安定した共有資源であることに着目し、データ中心で業務を分析したりシステムを設計する手法。業務で扱いデータとその関係をER図などを用いてモデリングし、元にしたDBを設計する。業務システムはDBをもとに設計されるため、複数のシステム間でデータの一貫性や整合性が保たれ、一元的な管理も容易になる

- 業務のモデリングに先だって、データモデリングを行う  
データ中心設計では業務のモデリングは行われない
...
---
56.ソフトウェアのモジュール設計において、信頼性・保守性を向上させるためのアプローチ

- A.**モジュール強度を強く、結合度を弱くする**  
`モジュール強度` : モジュールを構成する機能同士の結びつきの強さ(モジュール内部の関連性の強さ)を表し、強いほど独立性が高く良いとされる(`暗号的強度(弱) > 論理的強度 > 時間的強度 > 手順的強度 > 連絡的強度 > 情報的強度 > 機能的強度(強)`)
`モジュール結合度` : モジュール同士がどのように影響し合うかの関連性の強さを表す。弱いほど関連するモジュールに変更があった場合の影響を受けにくいので、モジュールの独立性が高まる(`データ結合(弱) > スタンプ結合 > 制御結合 > 外部結合 > 共通結合 > 内部結合(強)`)

---
57.外部設計の成果物に基づいて、実現方法や処理効率を考慮しながらシステム開発者の立場から進める設計作業

- A.**機能分割・構造化**  
内部設計のことを指す。外部設計の要件をコンピュータ・システム上で効率よく動作させるかのシステム開発側の視点で行われる工程  
1.外部設計で作成した機能をプログラム単位に分割する  
2.外部設計で作成した論理データを具体的なファイル・DBへのへ変換する  
...
---
3.流れ図において、分岐網羅を満たし条件網羅も満たすテストデータの組み

<img width="300" alt="" src="./images/流れ図.png">

- A.**{x, y} = {1, 2}, {0, 1}, {0, 2}**  
`分岐網羅(判定条件網羅)` : 判定条件の真偽を少なくとも1回は実行するテストケースを設計する  
`条件網羅` : 判定条件が複数ある場合に、それぞれの条件が真偽の場合を組み合わせたテストケースを設計する  
この流れ図で分岐網羅を満たすには、2つある分岐についてyes / noの処理をどちらも実行する必要がある。条件網羅を満たすためには判定条件が複数である1つ目の分岐において、"x >= 1が真", "y = 1が真", "x >= 1が偽", "y = 1が偽"の4つが含まれている必要がある

- {x, y} = {2, 2}, {1, 2}  
どちらの分岐もnoの処理を行っていないので分岐網羅を満たさない。"x >= 1が偽", "y = 1が真"が含まれていないので条件網羅も満たさない

- {x, y} = {1, 2}, {0, 0}  
分岐網羅を満たすが、"y = 1が真"が含まれていないので条件網羅を満たさない

- {x, y} = {1, 2}, {1, 1}, {0, 1}  
条件網羅を満たすが、"X >= 1 or y = 1"の分岐がnoの処理を行っていないので分岐網羅を満たさない

---
4.エラー埋込法において、埋め込まれたエラー数をS、埋め込まれたエラーのうち発見されたエラー数をm、埋め込まれたエラーを含まないテスト開始前の潜在エラー数をT、発見された総エラー数をnとしたとき、S, T, m, nの関係を表す式

- A.**m / S = (n - m) / T**  
埋め込まれた発見されたエラー : 埋め込まれたエラー = 埋め込みではない発見されたエラー : 埋込を含まない潜在エラーが成り立つので、m : S = (n - m) : Tが成り立つ

---
5.単一の入り口をもち、入力項目を用いた複数の判断を含むプログラムのテストケースを設計する。命令網羅と判定条件網羅の関係

- A.**判定条件網羅を満足するならば、命令網羅も満足する**  
`命令網羅` : 全ての命令を少なくとも1回は実行するテストケースの設計。最も網羅性が低い  
`判定条件網羅(分岐網羅)` : 全ての分岐について分岐先のすべての経路を少なくとも1回は実行するようにテストケースを設計する

- 判定条件網羅を満足しても命令網羅を満足しない場合がある  
判定条件網羅を満足すれば、どの分岐先についても命令が実行されるということになり、同時に命令網羅も満足する

- 命令網羅を満足しなくても、判定条件網羅を満足する場合がある  
命令網羅を満足しないならば、必ず判定条件網羅も満足しない

- 命令網羅を満足するならば、判定条件網羅も満足する  
命令網羅がそのまま判定条件網羅になるケースもあるが、命令網羅を満足していても判定条件網羅を満たすとは限らない

---
6.ブラックボックステストに関する記述

- A.**被テストプログラムに冗長なコードがあっても検出できない**  
内部構造を考慮せず、システムに与える入力と出力が仕様書通りになっているかを検証する。内部での処理は無視するため、システム内部をブラックボックス(中身が見えない)とし、同値分析、限界値分析、原因 - 結果グラフ、エラー推測などがある

- テストデータの作成基準として、プログラムの命令や分岐に対する網羅率を使用する  
命令や分岐のうちどの程度の割合をテストしたか基準とするには、ソースコードが明らかになっていないといけないため、ホワイトボックステストの記述

- プログラムの内部構造に着目し、必要な部分が実行されたかどうかを検証する  
プログラムの内部構造に着目しているので、ホワイトボックステストの記述

- 分岐命令やモジュールの数が増えると、テストデータが急増する  
ホワイトボックステストの記述。仕様書の機能通りに動くかどうかのみを検証するので、テストケースの多さはプログラムの実装方法とは無関係

---
7.ソフトウェアのテストツールの説明で、静的テストを支援する静的解析ツール

- A.**プログラム中に文法上の誤りや論理的な誤りなどがあるかどうかを、ソースコードを分析して調べる**  
静的解析ツールの構文チェッカの説明。ソースコードを分析しているので、プログラムを実行せずに行う解析  
`静的テストツール` : 構文チェッカ、コードオーディタ、モジュールインタフェースチェックツールなどのプログラムを実行しない状態でプログラムを検証する

- 指定された条件のテストデータや、プログラムの入力ファイルを自動的に生成する  
環境設定ツールのテストケース作成ツールの説明  
`環境設定ツール` : テストベッドツール、テストデータ生成ツールなどのテスト環境を準備するためのツール

- テストの実行結果を基に、命令の網羅率や分岐の網羅率を自動的に計測し、分析する  
動的解析ツールのカバレッジモニタの説明  
`動的テストツール` : インスペクタ、トレーサ、スナップショット、メモリダンプ、カバレッジモニタ、アサーションチェッカなどのプログラムを実行しながら検証する

- モジュールの呼出し回数や実行時間、実行文の実行回数などの、プログラム実行時の動作特性に関するデータを計測する  
動的解析ツールのプロファイラの説明

---
8.ブラックボックステストのテストデータの作成方法

- A.**機能仕様から同値クラスや限界値を識別し、テストデータを作成する**  
有効データと無効データのそれぞれに対して正しい出力が得られるかどうか検証可能なテストデータである必要がある

- 稼動中のシステムから実データを無作為に抽出し、テストデータを作成する  
有効なデータが正しく処理されるだけでなく、無効なデータがエラーとして処理されるか否かも検証する必要がある

- 業務で発生するデータの発生頻度を分析し、テストデータを作成する  
発生頻度ではなく入力データの範囲を分析してテストデータを作成する
