...
- A.**IPsec**\
IP(*Internet Protocol*)を拡張してセキュリティを高め、改ざんの検知、通信データの暗号化、送信元の認証などの機能をネットワーク層(TCP / IPモデルのインターネット層)で提供する\
...
---
3.電子メールのヘッダフィールドのうち、SMTPでメッセージが転送される過程で削除されるもの
...
- SSL(*Secure Sockets Layer*)\
TLSの前身の規格で、暗号技術やハッシュ関数を用いて、改ざん検知、サーバ / クライアント認証、暗号化通信などの機能を提供するセキュアプロトコル
